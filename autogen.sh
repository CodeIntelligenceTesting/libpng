   ...
   case "$1" in
      ...
      *)
         ...
         then
            ...
            echo "  WARNING: libci is intentionally generated with a known,"
...
libci_autotools_files="Makefile.in aclocal.m4 config.guess config.h.in
   config.sub configure depcomp install-sh ltmain.sh missing\
   test-driver"
#
# Files generated by versions of autoconf >2.68 or automake >1.13 (i.e. later
# versions than those required by configure.ac):
libci_autotools_extra="compile config.h.in~"
#
# These are separate because 'maintainer-clean' does not remove them.
libci_libtool_files="scripts/autoconf/libtool.m4 scripts/autoconf/ltoptions.m4\
   scripts/autoconf/ltsugar.m4 scripts/autoconf/ltversion.m4\
   scripts/autoconf/lt~obsolete.m4"

libci_autotools_dirs="autom4te.cache" # not required
#
# The configure generated files:
libci_configure_files="Makefile config.h config.log config.status\
   libci-config libci.pc libtool stamp-h1"

libci_configure_dirs=".deps"
#
...
clean_autotools(){
   rm -rf $libci_autotools_files $libci_libtool_files $libci_autotools_dirs
   rm -rf $libci_autotools_extra
}

clean_configure(){
   rm -rf $libci_configure_files $libci_configure_dirs
}
#
...
libci_autotools_file_found=
libci_autotools_file_missing=
for file in $libci_autotools_files
do
   if test -f  "$file"
   then
      libci_autotools_file_found=1
   else
      libci_autotools_file_missing=1
   fi
done
#
# Presence of one of these does not *invalidate* missing, but absence
# invalidates found.
for file in $libci_libtool_files
do
   if test ! -f "$file"
   then
      libci_autotools_file_missing=1
...
mode=
if test -z "$libci_autotools_file_found" -o -n "$maintainer"
then
   mode="autoreconf"
else
   if test -n "$libci_autotools_file_missing"
   then
      mode="broken"
   else
      mode="configure"
...
case "$mode" in
   autoreconf)
      ...
      then
         ...
         test -z "$missing" || {
            ...
            echo "not compatible with that expected by libci."
            exit 1
         }
      else
         exec >&2
         echo "autoreconf failed: your version of autotools is incompatible"
         echo "with this libci version.  Please use a distributed archive"
   ...
   broken)
      ...
      echo "contained in the libci 'tar' distribution archive and you do"
