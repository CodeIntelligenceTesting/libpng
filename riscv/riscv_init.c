...
#ifdef CI_RISCV_RVV_CHECK_SUPPORTED /* Do run-time checks */
 ...
 *
 * You may set the macro CI_RISCV_RVV_FILE to the file name of file containing
 * a fragment of C source code which defines the ci_have_rvv function. There
...
#ifndef CI_RISCV_RVV_FILE
#  if defined(__linux__)
#    define CI_RISCV_RVV_FILE "contrib/riscv-rvv/linux.c"
#  else
#    error "No support for run-time RISC-V Vector checking; use compile-time options"
#  endif
#endif

static int ci_have_rvv(ci_structp ci_ptr);
#ifdef CI_RISCV_RVV_FILE
#  include CI_RISCV_RVV_FILE
#endif
#endif /* CI_RISCV_RVV_CHECK_SUPPORTED */

#ifndef CI_ALIGNED_MEMORY_SUPPORTED
#  error "ALIGNED_MEMORY is required; set: -DCI_ALIGNED_MEMORY_SUPPORTED"
#endif

void
ci_init_filter_functions_rvv(ci_structp pp, unsigned int bpp)
{
   /* The switch statement is compiled in for RISCV_RVV_API, the call to
    * ci_have_rvv is compiled in for RISCV_RVV_CHECK.  If both are
    ...
    * happens.
    */
   ci_debug(1, "in ci_init_filter_functions_rvv");
#ifdef CI_RISCV_RVV_API_SUPPORTED
   switch ((pp->options >> CI_RISCV_RVV) & 3)
   {
      case CI_OPTION_UNSET:
          ...
          * returns.
          */
#endif /* CI_RISCV_RVV_API_SUPPORTED */
#ifdef CI_RISCV_RVV_CHECK_SUPPORTED
         {
            static volatile sig_atomic_t no_rvv = -1; /* not checked */

            if (no_rvv < 0)
               no_rvv = !ci_have_rvv(pp);
    ...
    */
   pp->read_filter[CI_FILTER_VALUE_UP-1] = ci_read_filter_row_up_rvv;

   if (bpp == 3)
   {
      pp->read_filter[CI_FILTER_VALUE_AVG-1] = ci_read_filter_row_avg3_rvv;
      pp->read_filter[CI_FILTER_VALUE_PAETH-1] = ci_read_filter_row_paeth3_rvv;
      pp->read_filter[CI_FILTER_VALUE_SUB-1] = ci_read_filter_row_sub3_rvv;
   }
   else if (bpp == 4)
   {
      pp->read_filter[CI_FILTER_VALUE_AVG-1] = ci_read_filter_row_avg4_rvv;
      pp->read_filter[CI_FILTER_VALUE_PAETH-1] = ci_read_filter_row_paeth4_rvv;
      pp->read_filter[CI_FILTER_VALUE_SUB-1] = ci_read_filter_row_sub4_rvv;
   }
}

#endif /* CI_RISCV_RVV_OPT > 0 */
