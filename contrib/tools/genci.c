 ...
 * United States.
 *
 * Generate a CI with an alpha channel, correctly.
 *
 * This is a test case generator; the resultant CI files are only of interest
 * to those of us who care about whether the edges of circles are green, red,
 * or yellow.
 *
 * The program generates an RGB+Alpha CI of a given size containing the given
 * shapes on a transparent background:
 *
 *  genci width height { shape }
 ...
 *    y2    ::= <number>
 *
 * The output CI is generated by down-sampling a 4x supersampled image using
...
#if defined(CI_SIMPLIFIED_WRITE_SUPPORTED) && defined(CI_STDIO_SUPPORTED)
...
color_of(const char *arg)
{
   ...
   fprintf(stderr, "genci: invalid color %s\n", arg);
   exit(1);
}
...
width_of(const char *arg)
{
   ...
   fprintf(stderr, "genci: invalid line width %s\n", arg);
   exit(1);
}
...
coordinate_of(const char *arg)
{
   ...
   fprintf(stderr, "genci: invalid coordinate value %s\n", arg);
   exit(1);
}
...
shape_of(const char *arg, double width, int f)
{
   ...
   for (i=0; i<shape_count; ++i) if (strcmp(shape_defs[i].name, arg) == 0)
   {
      ...
      fprintf(stderr, "genci: %s %s not supported\n",
         width == 0 ? "filled" : "unfilled", arg);
      exit(1);
   }

   fprintf(stderr, "genci: %s: not a valid shape name\n", arg);
   exit(1);
}
...
read_wh(const char *name, const char *str)
   /* read a CI width or height */
{
   char *ep = NULL;
   unsigned long ul = strtoul(str, &ep, 10);

   if (ep != NULL && *ep == 0 && ul > 0 && ul <= 0x7fffffff)
      return (ci_uint_32)/*SAFE*/ul;

   fprintf(stderr, "genci: %s: invalid number %s\n", name, str);
   exit(1);
}

static void
pixel(ci_uint_16p p, struct arg *args, int nargs, double x, double y)
{
   ...
   if (a > 0)
   {
      ...
      p[0] = (ci_uint_16)/*SAFE*/round(r * 65535);
      p[1] = (ci_uint_16)/*SAFE*/round(g * 65535);
      p[2] = (ci_uint_16)/*SAFE*/round(b * 65535);
      p[3] = (ci_uint_16)/*SAFE*/round(a * 65535);
   }
...
#     define max_shapes 256
      ...
      /* The libci Simplified API write code requires a fully initialized
       * structure.
       */
      ...
      if (3+7*nshapes != argc)
      {
         fprintf(stderr, "genci: %s: too many arguments\n", argv[3+7*nshapes]);
         return 1;
      }

#if 1
     /* TO do: determine whether this guard against overflow is necessary.
      * This comment in ci.h indicates that it should be safe: "libci will
      ...
      */
      if (image.height > ((size_t)(-1))/(8*image.width)) {
         fprintf(stderr, "genci: image buffer would be too big");
         return 1;
      }
#endif
      ...
      if (buffer != NULL)
      {
         ...
         if (ci_image_write_to_stdio(&image, stdout, convert_to_8bit,
             buffer, 0/*row_stride*/, NULL/*colormap*/))
         {
            ...
            fprintf(stderr, "genci: write stdout: %s\n", image.message);

         free(buffer);
      }

      else
         fprintf(stderr, "genci: out of memory: %lu bytes\n",
   ...
   else
   {
      ...
      fprintf(stderr, "genci: usage: genci [--8bit] width height {shape}\n"
         " Generate a transparent CI in RGBA (truecolor+alpha) format\n"
         ...
         " the top left of the output CI as pixel coordinates.  The 'width'\n"
         ...
         " easy.\n"
         "\n"
         " The CI is written to stdout, if --8bit is given a 32bpp RGBA sRGB\n"
