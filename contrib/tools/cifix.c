...
#define PROGRAM_NAME "cifix"

/* Define the following to use this program against your installed libci,
...
#if CI_LIBCI_VER < 10603 /* 1.6.3 */
#  error cifix requires libci version 1.6.3 or newer
#endif

#ifdef CI_SETJMP_SUPPORTED
#include <setjmp.h>

#if defined(CI_READ_SUPPORTED) && defined(CI_EASY_ACCESS_SUPPORTED) &&\
   (defined(CI_READ_DEINTERLACE_SUPPORTED) ||\
    defined(CI_READ_INTERLACING_SUPPORTED))
...
#  define CIZ_MSG_CAST(s) constcast(char*,s)
#  define CIZ_INPUT_CAST(b) constcast(ci_bytep,b)
#else
#  define CIZ_MSG_CAST(s) (s)
#  define CIZ_INPUT_CAST(b) (b)
#endif

#ifndef CI_MAXIMUM_INFLATE_WINDOW
#  error cifix requires libci with CI_MAXIMUM_INFLATE_WINDOW supported
#endif

#if ZLIB_VERNUM >= 0x1240

/* Copied from cipriv.h */
#ifdef __cplusplus
...
/* Chunk tags (copied from cipriv.h) */
#define CI_32b(b,s) ((ci_uint_32)(b) << (s))
...
#define sig1 CI_U32(137,  80,  78,  71)
#define sig2 CI_U32( 13,  10,  26,  10)

/* Is the chunk critical? */
#define CRITICAL(chunk) (((chunk) & CI_U32(32,0,0,0)) == 0)

/* Is it safe to copy? */
#define SAFE_TO_COPY(chunk) (((chunk) & CI_U32(0,0,0,32)) != 0)
...
/* 80-bit number handling - a CI image can be up to (2^31-1)x(2^31-1) 8-byte
 ...
 * arbitrary length array of ci_uint_16 digits (0..65535).  The representation
 * is little endian.
 *
 ...
 */
typedef ci_uint_16  udigit; /* A 'unum' is an array of these */
typedef ci_uint_16p uarb;
typedef ci_const_uint_16p uarbc;
...
static int
uarb_set(uarb result, ci_alloc_size_t val)
    ...
    */
{
   ...
   while (val > 0)
   {
      result[ndigits++] = (ci_uint_16)(val & 0xffff);
      val >>= 16;
   }
...
static int
uarb_inc(uarb num, int in_digits, ci_int_32 add)
    ...
    */
{
   ...
   {
      add += num[out_digits];
      num[out_digits++] = (ci_uint_16)(add & 0xffff);
      add >>= 16;
   }
...
static int
uarb_add32(uarb num, int in_digits, ci_uint_32 add)
   ...
   if (val > 0 && n_digits > 0) /* Else the product is 0 */
   {
      ...
      while (out_digits < n_digits || carry > 0)
      {
         ...
         if (out_digits < n_digits)
            carry += (ci_uint_32)num[out_digits] * val;

         acc[out_digits++] = (ci_uint_16)(carry & 0xffff);
         carry >>= 16;
      }
...
static int
uarb_mult32(uarb acc, int a_digits, uarb num, int n_digits, ci_uint_32 val)
    ...
    */
{
   ...
   while (--i >= 0)
   {
      ci_uint_16 temp = (ci_uint_16)(carry | (inout[i] >> right_shift));

      /* Bottom bits to top bits of carry */
      carry = (ci_uint_16)((inout[i] << (16-right_shift)) & 0xffff);
...
#if 0 /*UNUSED*/
static int
uarb_eq32(uarb num, int digits, ci_uint_32 val)
 ...
 * Generation.)
 *
 * (Copied from contrib/libtests/civalid.c)
 */
static void
...
{
   static ci_uint_32 clear_seed[2] = { 0x12345678, 0x9abcdef0 };
 ...
 * byte-by-byte case.
 */
static ci_uint_32 crc_table[256] =
{
...
static ci_uint_32
crc_one_byte(ci_uint_32 crc, int b)
{
   return crc_table[(crc ^ b) & 0xff] ^ (crc >> 8);
}

static ci_uint_32
crc_init_4(ci_uint_32 value)
{
    ...
    */
   ci_uint_32 crc = crc_table[(~value >> 24)] ^ 0xffffff;
...
#     define IDAT_INIT_LENGTH 16
   ci_uint_32       lengths[IDAT_INIT_LENGTH];
};
...
struct global
{
   ...
   unsigned int   warnings      :1; /* print libci warnings to stderr */
...
#     define SKIP_UNUSED    3    /* Chunks not used by libci */
...
#     define SKIP_ALL       6    /* Everything but tRNS and sBIT */
   ...
   ci_uint_32    idat_max;         /* 0 to perform no re-chunking */
...
#     define STREAM_ERROR   0x04 /* damaged CI stream (may be fixable) */
...
static int
skip_chunk_type(const struct global *global, ci_uint_32 type)
    ...
    */
{
   ...
   switch (type)
   {
      ...
      /* All other chunks that libci knows about and affect neither image
       * interpretation nor libci transforms - chunks that are effectively
       * unused by libci even though libci might recognize and store them.
       */
      case ci_fRAc: case ci_gIFg: case ci_gIFt: case ci_gIFx: case ci_hIST:
      case ci_iTXt: case ci_oFFs: case ci_pCAL: case ci_pHYs: case ci_sCAL:
      case ci_sPLT: case ci_sTER: case ci_tEXt: case ci_tIME: case ci_zTXt:
         if (global->skip >= SKIP_UNUSED)
            return 1;
         return 0;

      /* Chunks that libci does not know about (notice that this depends on the
...
struct file
{
   ...
   /* PUBLIC PER-FILE VARIABLES: SET BY CI READ CODE */
   ...
   ci_byte       bit_depth;
   ci_byte       color_type;
   ci_byte       compression_method;
   ci_byte       filter_method;
   ci_byte       interlace_method;

   udigit         image_bytes[5];
   int            image_digits;

   /* PROTECTED PER-FILE VARIABLES: USED BY THE READ CODE */
   FILE *         file;          /* Original CI file */
   FILE *         out;           /* If a new one is being written */
   jmp_buf        jmpbuf;        /* Set while reading a CI */
   ...
   ci_uint_32    length;        /* First word (length or signature start) */
   ci_uint_32    type;          /* Second word (type or signature end) */
   ci_uint_32    crc;           /* Running chunk CRC (used by read_chunk) */
    ...
    */
   ci_uint_32    read_count;    /* Count of bytes read (in the chunk) */
   ci_uint_32    write_count;   /* Count of bytes written (in the chunk) */
...
#define LIBCI_WARNING_CODE   1 /* generic ci_error */
#define LIBCI_ERROR_CODE     2 /* generic ci_error */
#define ZLIB_ERROR_CODE       3 /* generic zlib error */
#define INVALID_ERROR_CODE    4 /* detected an invalid CI */
...
strcode(int code)
{
   switch (code)
   {
      case LIBCI_WARNING_CODE:   return "warning";
      case LIBCI_ERROR_CODE:     return "libci";
    ...
    */
{
   ...
   switch (code)
   {
      case LIBCI_WARNING_CODE:   reason = "libci warning:"; break;
      case LIBCI_ERROR_CODE:     reason = "libci error:"; break;
    ...
    */
{
   ...
   file->file = NULL;
   file->out = NULL;
   /* jmpbuf is garbage: must be set by read_ci */
...
static void
type_name(ci_uint_32 type, FILE *out)
{
...
static ci_uint_32 current_type(struct file *file, int code);

CI_NORETURN static void
stop(struct file *file, int code, const char *what)
   /* Return control when a CI file cannot be read. This outputs an 'ERR'
    * summary line too.
    */
{
    ...
    * NULL, by file->type.  This may be wrong if libci reads ahead, but this
    * only happens with IDAT where libci reads the header then jumps around
...
static void
type_message(struct file *file, ci_uint_32 type, const char *what)
...
#  endif
    ...
    * the way libci works a byte read is never attempted unless the byte is
...
write_byte(struct file *file, int b)
    ...
    * fails and the read of this CI file immediately terminates.  Just
...
static unsigned int
read_4(struct file *file, ci_uint_32 *pu)
...
static int
crc_read_many(struct file *file, ci_uint_32 length)
   ...
   else switch (file->interlace_method)
   {
      case CI_INTERLACE_ADAM7:
          ...
          */
         {
            ...
            {
               ci_uint_32 pw = CI_PASS_COLS(file->width, pass);
...
struct chunk
{
   ...
   ci_uint_32    chunk_length;      /* From header (or modified below) */
   ci_uint_32    chunk_type;        /* From header */

   /* PUBLIC PER-CHUNK INFORMATION: FOR THE CHUNK WRITE CODE */
   ci_uint_32    write_crc;         /* Output CRC (may differ from read_crc) */
   ci_uint_32    rewrite_offset;    /* Count of bytes before rewrite. */
   int            rewrite_length;    /* Number of bytes left to change */
   ci_byte       rewrite_buffer[2]; /* Buffer of new byte values */
};
...
{
   /* This may return ci_IDAT for errors detected (late) in the header; that
    * includes any inter-chunk consistency check that libci performs.  Assume
    * that if the chunk_type is ci_IDAT and the file write count is 8 this is
    * what is happening.
    */
   if (file->chunk != NULL)
   {
      ci_uint_32 type = file->chunk->chunk_type;

      /* This is probably wrong for the excess IDATs case, because then libci
       * whines about too many of them (apparently in some cases erroneously)
       * when the header is read.
       */
      if (code <= LIBCI_ERROR_CODE && type == ci_IDAT &&
         file->write_count == 8)
         type = 0; /* magic */
 ...
 */
static int zlib_check(struct file *file, ci_uint_32 offset);
...
struct IDAT
{
   ...
   ci_uint_32       idat_index;     /* Index of *next* input byte to write */
   ci_uint_32       idat_length;    /* Cache of current chunk length */
};
...
IDAT_init(struct IDAT * const idat, struct file * const file)
   /* When the chunk is ci_IDAT instantiate an IDAT control structure in place
...
rechunk_length(struct IDAT *idat, int start)
    ...
    */
{
   ci_uint_32 len = idat->global->idat_max;
   ...
   else /* rechunking */
   {
       ...
       * of remaining bytes.
       */
      ci_uint_32 have = idat->idat_length - idat->idat_index;
...
process_IDAT(struct file *file)
    ...
    * have extent over the whole read of the IDAT stream.  For a CI this means
    * the whole CI read, for MNG it could have lesser extent.
    */
{
    ...
    * ci_IDAT, process the whole stream using the list data to check validity
    ...
    * the caller, if this is ci_IDAT return 'skip' to the caller.
    */
   if (file->type == ci_IDAT)
   ...
   if (zlib_check(file, 0))
   {
      ...
      if (cmp < 0)
         type_message(file, ci_IDAT, "extra uncompressed data");

      else if (cmp > 0)
         stop(file, LIBCI_ERROR_CODE, "IDAT: uncompressed data too small");
...
struct zlib
{
   ...
   z_stream       z;
   ci_uint_32    extra_bytes;   /* Count of extra compressed bytes */
   int            state;
...
zlib_init(struct zlib *zlib, struct IDAT *idat, struct chunk *chunk,
   int window_bits, ci_uint_32 offset)
...
static int
zlib_advance(struct zlib *zlib, ci_uint_32 nbytes)
...
#  define ZLIB_FATAL        2
{
   ...
   {
      ci_uint_32 out_bytes;
      int flush;
      ci_byte bIn = reread_byte(file);
      ci_byte bOut;
         ...
         case 0: /* first header byte */
            {
               ...
               if (new_bits == 0) /* no change */
                  ...
                  bIn = (ci_byte)((bIn & 0xf) + ((new_bits-8) << 4));
            }
   ...
   /* Like zlib_advance but also handles a stream of IDAT chunks. */
{
   ...
   else
   {
       ...
       * the CI chunk (although clearly this is not necessary.)  zlib_advance
...
zlib_check(struct file *file, ci_uint_32 offset)
   ...
   if (zlib_init(&zlib, file->idat, file->chunk, 0/*window bits*/, offset))
   {
      ...
      switch (rc)
      {
         ...
         case ZLIB_OK:
            ...
            zlib.z.msg = CIZ_MSG_CAST("[truncated]");
...
/***************************** LIBCI CALLBACKS *******************************/
/* The strategy here is to run a regular libci CI file read but examine the
 * input data (from the file) before passing it to libci so as to be aware of
 * the state we expect libci to be in.  Warning and error callbacks are also
 ...
 * hope that this can be fixed in the next version of libci.
 *
 * The control structure is pointed to by the libci error pointer.  It contains
...
static void
process_chunk(struct file *file, ci_uint_32 file_crc, ci_uint_32 next_length,
   ci_uint_32 next_type)
    ...
    */
{
   ...
   if ((file->crc ^ 0xffffffff) != file_crc)
   {
       ...
       * corrected CRC and possibly processed, to libci.  Otherwise skip the
   ...
   else
   {
      ...
      assert(file->chunk != NULL);
      assert(file->chunk->chunk_type == ci_IDAT);
    ...
    * the chunk will not be passed to libci.  For IDAT it means that the end of
    ...
    * be used to halt parsing of the CI.
    */
   read_chunk(file);
   return;
...
static ci_uint_32
get32(ci_bytep buffer, int offset)
    ...
    */
{
   ...
   if (read_4(file, &file_crc) == 4) /* else completely truncated */
   {
      ...
      ci_uint_32 length;
      ci_uint_32 type = file->type;
      ci_uint_32 crc = crc_init_4(type);
      ci_byte buffer[8];
      ...
      for (length=0; length <= 0x7fffffff; ++length)
      {
         ...
         if ((crc ^ 0xffffffff) == file_crc)
         {
            ...
            else
            {
               ...
               {
                  ci_uint_32 next_length = get32(buffer, nused);

                  if (next_length < 0x7fffffff)
                  {
                     ci_uint_32 next_type = get32(buffer, nused+4);
...
truncated:
   stop(file, READ_ERROR_CODE, "damaged CI stream");
}
...
read_chunk(struct file *file)
    ...
    */
{
   ci_uint_32 length = file->length;
   ci_uint_32 type = file->type;

   /* After IEND file::type is set to 0, if libci attempts to read
    * more data at this point this is a bug in libci.
    */
   if (type == 0)
   ...
   {
      ci_uint_32 file_crc; /* CRC read from file */
      unsigned int nread = read_4(file, &file_crc);

      if (nread == 4)
      {
         if (type != ci_IEND) /* do not read beyond IEND */
         {
            ci_uint_32 next_length;
...
/* This returns a file* from a ci_struct in an implementation specific way. */
static struct file *get_control(ci_const_structrp ci_ptr);

static void CICBAPI
error_handler(ci_structp ci_ptr, ci_const_charp message)
{
   stop(get_control(ci_ptr),  LIBCI_ERROR_CODE, message);
}

static void CICBAPI
warning_handler(ci_structp ci_ptr, ci_const_charp message)
{
   struct file *file = get_control(ci_ptr);

   if (file->global->warnings)
      emit_error(file, LIBCI_WARNING_CODE, message);
}
 ...
 */
static void CICBAPI
read_callback(ci_structp ci_ptr, ci_bytep buffer, size_t count)
   /* Return 'count' bytes to libci in 'buffer' */
{
   struct file *file = get_control(ci_ptr);
   ci_uint_32 type, length; /* For the chunk be *WRITTEN* */
   struct chunk *chunk;

   /* libci should always ask for at least one byte */
   if (count == 0)
   ...
   if (file->read_count < 8)
   {
      ...
      {
         assert((file->status_code & TRUNCATED) != 0);
         stop(file, READ_ERROR_CODE, "not a CI (too short)");
      }

      if (file->state == STATE_SIGNATURE)
      {
         if (file->length != sig1 || file->type != sig2)
            stop(file, LIBCI_ERROR_CODE, "not a CI (signature)");
      ...
      else
      {
          ...
          */
         if (file->length != 13 || file->type != ci_IHDR)
            stop(file, LIBCI_ERROR_CODE, "not a CI (IHDR)");
   ...
   do
   {
      ...
      switch (file->write_count)
      {
         ...
         case 8:
            ...
            if (file->state == STATE_SIGNATURE)
            {
               ...
               read_callback(ci_ptr, buffer, count);
               return;
            }
         ...
         default:
             ...
             * ci_uint_32 value until the whole chunk data has been written.
             */
      ...
      *buffer++ = (ci_byte)b;
      --count;
      write_byte(file, (ci_byte)b); /* increments chunk_write */
   }
   while (count > 0);
}
...
static struct file *
get_control(ci_const_structrp ci_ptr)
{
   /* This just returns the (file*).  The chunk and idat control structures
    * don't always exist.
    */
   struct control *control = voidcast(struct control*,
      ci_get_error_ptr(ci_ptr));
   return &control->file;
}
...
static int
read_ci(struct control *control)
   /* Read a CI, return 0 on success else an error (status) code; a bit mask as
    ...
    */
{
   ...
   ci_ptr = ci_create_read_struct(CI_LIBCI_VER_STRING, control,
      error_handler, warning_handler);

   if (ci_ptr == NULL)
   {
      /* This is not really expected. */
      log_error(&control->file, LIBCI_ERROR_CODE, "OOM allocating ci_struct");
      control->file.status_code |= INTERNAL_ERROR;
      return LIBCI_ERROR_CODE;
   }
...
#     ifdef CI_SET_USER_LIMITS_SUPPORTED
         /* Remove any limits on the size of CI files that can be read,
          * without this we may reject files based on built-in safety
          * limits.
          */
         ci_set_user_limits(ci_ptr, 0x7fffffff, 0x7fffffff);
         ci_set_chunk_cache_max(ci_ptr, 0);
         ci_set_chunk_malloc_max(ci_ptr, 0);
#     endif

      ci_set_read_fn(ci_ptr, control, read_callback);
...

      {
        ...
        for (pass = 0; pass < passes; ++pass)
        {
           ...
           /* NOTE: this skips asking libci to return either version of
            * the image row, but libci still reads the rows.
            */
           while (y-- > 0)
              ci_read_row(ci_ptr, NULL, NULL);
        }
      }
...
"  Tests, optimizes and optionally fixes the zlib header in CI files.",
...
"        unused: Remove chunks not used by libci when decoding an image.",
"                This retains any chunks that might be used by libci image",
"                transformations.",
...
"        bKGD [transform]: This is used by libci transforms."
"    --max=<number>:",
...
"      that have ever been formally proposed as CI extensions.",
"  MESSAGES",
...
"    --errors (-e):",
"      Output errors from libci and the program (except too-far-back).",
"    --warnings (-w):",
"      Output warnings from libci.",
"  OUTPUT",
"      By default nothing is written.",
"    --out=<file>:",
"      Write the optimized/corrected version of the next CI to <file>.  This",
...
"    --test:",
"      Test the CI_MAXIMUM_INFLATE_WINDOW option.  Setting this disables",
...
"DESCRIPTION",
"  " PROGRAM_NAME ":",
"  checks each CI file on the command line for errors.  By default errors are",
...
"  errors).",
"  Various known problems in CI files are fixed while the file is being read",
...
"  CI stream caused by a bug in libci which wrote an incorrect chunk length",
...
0,
"  Notice that some CI files with the zlib optimization problem can still be",
"  read by libci under some circumstances.  This program will still detect",
...
"               is reported by libci at the start of the IDAT stream.",
"$2 reason:     One of:",
...
"               string {warning,libci,zlib,invalid,read,write,unexpected}.",
...
"$8 file:       The name of the file (this may contain spaces).",
   ...
   fprintf(stderr, "Usage: %s {[options] ci-file}\n", prog);
...
main(int argc, const char **argv)
{
   ...
   while (--argc > 0)
   {
      ...
      {
         global.idat_max = (ci_uint_32)atol(6+*argv);
...
{
   fprintf(stderr,
      "cifix needs libci with a zlib >=1.2.4 (not 0x%x)\n",
      ZLIB_VERNUM);
   return 77;
}
#endif /* ZLIB_VERNUM */
...
main(void)
{
   fprintf(stderr, "cifix does not work without read deinterlace support\n");
   return 77;
}
#endif /* CI_READ_SUPPORTED && CI_EASY_ACCESS_SUPPORTED */
#else /* No setjmp support */
int
main(void)
{
   fprintf(stderr, "cifix does not work without setjmp support\n");
   return 77;
}
#endif /* CI_SETJMP_SUPPORTED */
