 ...
 * Usage:
 *
 *     ci-fix-itxt < bad.ci > good.ci
 *
 * Fixes a CI file written with libci-1.6.0 or 1.6.1 that has one or more
 ...
 * CI file that has no iTXt chunks or one that has valid iTXt chunks;
 * such files will simply be copied.
 *
 * Requires zlib (for crc32 and Z_NULL); build with
 *
 *     gcc -O -o ci-fix-itxt ci-fix-itxt.c -lz
 *
 * If you need to handle iTXt chunks larger than 500000 kbytes you must
 * rebuild ci-fix-itxt with a larger values of MAX_LENGTH (or a smaller value
