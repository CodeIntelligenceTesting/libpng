 ...
 * and license in ci.h
 *
 * This is an example of copying a CI without changes using the ci_read_ci
 * and ci_write_ci interfaces.  A considerable number of options are provided
 * to manipulate the compression of the CI data and other compressed chunks.
 *
 * For a more extensive example that uses the transforms see
 * contrib/libtests/ciimage.c in the libci distribution.
 *
 ...
 * testing libci performance, both speed and compression.
 */

#include "cilibconf.h" /* To find how libci was configured. */

#ifdef CI_CICP_TIMING_SUPPORTED
   /* WARNING:
    *
...
#  include "../../ci.h"
   ...
   /* READ_CI and WRITE_CI were not defined, so: */
#  ifdef CI_INFO_IMAGE_SUPPORTED
#     ifdef CI_SEQUENTIAL_READ_SUPPORTED
#        define CI_READ_CI_SUPPORTED
#     endif /* SEQUENTIAL_READ */
#     ifdef CI_WRITE_SUPPORTED
#        define CI_WRITE_CI_SUPPORTED
#     endif /* WRITE */
#  endif /* INFO_IMAGE */
#endif /* pre 1.7.0 */

#if (defined(CI_READ_CI_SUPPORTED)) && (defined(CI_WRITE_CI_SUPPORTED))
#include <stdarg.h>
#include <stdlib.h>
#include <string.h>
#include <errno.h>
...
#ifndef CI_SETJMP_SUPPORTED
#  include <setjmp.h> /* because ci.h did *not* include this */
#endif
 ...
 * the implementation takes to read a CI ignoring the potentially very large IO
 * overhead.
 */
#ifdef CI_CICP_TIMING_SUPPORTED
...
#     undef CI_CICP_TIMING_SUPPORTED
#  endif
#endif /* CICP_TIMING */
...
typedef enum
{
   ...
   LIBCI_ERROR,   /* this and higher cause a longjmp */
   LIBCI_BUG,     /* erroneous behavior in libci */
...
vl_compression[] =
{
    ...
    * then high speed.
    */
   { "low-memory",      CI_COMPRESSION_LOW_MEMORY },
   { "high-speed",      CI_COMPRESSION_HIGH_SPEED },
   { "high-read-speed", CI_COMPRESSION_HIGH_READ_SPEED },
   { "low",             CI_COMPRESSION_LOW },
   { "medium",          CI_COMPRESSION_MEDIUM },
   { "old",             CI_COMPRESSION_COMPAT },
   { "high",            CI_COMPRESSION_HIGH },
   { all, 0 }
},
#endif /* SW_COMPRESS_ci_level */

#if defined(CI_WRITE_CUSTOMIZE_COMPRESSION_SUPPORTED) ||\
    defined(CI_WRITE_CUSTOMIZE_ZTXT_COMPRESSION_SUPPORTED)
vl_strategy[] =
{
...
vl_filter[] =
{
   ...
   { "up",     CI_FILTER_UP     },
   { "avg",    CI_FILTER_AVG    },
   { "paeth",  CI_FILTER_PAETH  }
},
#endif /* WRITE_FILTER */
#ifdef CI_CICP_TIMING_SUPPORTED
#  define CICP_TIME_READ  1
#  define CICP_TIME_WRITE 2
vl_time[] =
{
   { "both",  CICP_TIME_READ+CICP_TIME_WRITE },
   { "off",   0 },
   { "read",  CICP_TIME_READ },
   { "write", CICP_TIME_WRITE }
},
#endif /* CICP_TIMING */
vl_IDAT_size[] = /* for ci_set_IDAT_size */
{
   { "default", 0x7FFFFFFF },
   { "minimal", 1 },
   RANGE(1, 0x7FFFFFFF)
},
#ifndef CI_SW_IDAT_size
   /* Pre 1.7 API: */
#  define ci_set_IDAT_size(p,v) ci_set_compression_buffer_size(p, v)
#endif /* !SW_IDAT_size */
...
typedef struct option
{
   ...
   ci_uint_32       opt;          /* an option, or OPTION or LIST */
   ci_byte          search;       /* Search on --search */
   ci_byte          value_count;  /* length of the list of values: */
...
#  define VLSIZE(name) voidcast(ci_byte,\
...
#  ifdef CI_SW_COMPRESS_ci_level
      /* The libci compression level isn't searched because it just sets the
...
#  ifdef CI_SW_COMPRESS_windowBits
...
cts(int ct)
{
   switch (ct)
   {
      case CI_COLOR_TYPE_PALETTE:     return "P";
      case CI_COLOR_TYPE_GRAY:        return "G";
      case CI_COLOR_TYPE_GRAY_ALPHA:  return "GA";
      case CI_COLOR_TYPE_RGB:         return "RGB";
      case CI_COLOR_TYPE_RGB_ALPHA:   return "RGBA";
...
struct display
{
    ...
    * image.
    */
   ci_alloc_size_t read_size;
   ci_structp      read_pp;
   ci_infop        ip;
#  if CI_LIBCI_VER < 10700 && defined CI_TEXT_SUPPORTED
      ci_textp     text_ptr; /* stash of text chunks */
      int           num_text;
      int           text_stashed;
#  endif /* pre 1.7 */
...
#  define MAX_SIZE ((ci_alloc_size_t)(-1))
   ci_alloc_size_t write_size;
   ci_alloc_size_t best_size;
   ci_structp      write_pp;
   ...
   int              bpp;
   ci_byte         ct;
   int              no_warnings;       /* Do not output libci warnings */
   int              min_windowBits;    /* The windowBits range is 8..8 */

   /* Options handling */
   ci_uint_32      results;             /* A mask of errors seen */
   ci_uint_32      options;             /* See display_log below */
   ci_byte         entry[option_count]; /* The selected entry+1 of an option
   ...
   struct stack
   {
      ci_alloc_size_t best_size;      /* Best so far for this option */
      ci_alloc_size_t lo_size;
      ci_alloc_size_t hi_size;
      ...
      ci_byte         opt;            /* The option being tested */
      ci_byte         entry;          /* The next value entry to be tested */
      ci_byte         end;            /* This is the last entry */
...
{
   if (dp->read_pp != NULL)
      ci_destroy_read_struct(&dp->read_pp, freeinfo ? &dp->ip : NULL, NULL);
...
display_clean_write(struct display *dp, int freeinfo)
{
   ...
   if (dp->write_pp != NULL)
      ci_destroy_write_struct(&dp->write_pp, freeinfo ? &dp->ip : NULL);
}
...
get_dp(ci_structp pp)
   /* The display pointer is always stored in the ci_struct error pointer */
{
   struct display *dp = (struct display*)ci_get_error_ptr(pp);

   if (dp == NULL)
   {
      fprintf(stderr, "cicp: internal error (no display)\n");
...
display_log(struct display *dp, error_level level, const char *fmt, ...)
    ...
    * does not return if level is above LIBCI_WARNING
    */
{
   dp->results |= 1U << level;
   ...
   if (level > (error_level)(dp->options & LEVEL_MASK))
   {
      ...
      switch (level)
      {
         ...
         case LIBCI_WARNING: lp = "warning(libci)"; break;
         case APP_WARNING:    lp = "warning(cicp)"; break;
         case APP_FAIL:       lp = "error(continuable)"; break;
         case LIBCI_ERROR:   lp = "error(libci)"; break;
         case LIBCI_BUG:     lp = "bug(libci)"; break;
         case APP_ERROR:      lp = "error(cicp)"; break;
...
{
   /* libci 1.6 and earlier fixed a bug whereby text chunks were written
    * multiple times by ci_write_ci; the issue was that ci_write_ci passed
    * the same ci_info to both ci_write_info and ci_write_end.  Rather than
    * fixing it by recording the information in the ci_struct, or by recording
    * where to write the chunks, the fix made was to change the 'compression'
    * field of the chunk to invalid values, rendering the ci_info somewhat
    * useless.
    *
    * The only fix for this given that we use the ci_info more than once is to
    * make a copy of the text chunks and ci_set_text it each time.  This adds a
    ...
    * useless chunks from the ci_info.
    *
    ...
    */
   ci_textp chunks = NULL;

   dp->num_text = ci_get_text(dp->write_pp, dp->ip, &chunks, NULL);

   if (dp->num_text > 0)
   {
      dp->text_ptr = voidcast(ci_textp, malloc(dp->num_text * sizeof *chunks));
...
{
   /* libci makes a copy, so this is fine: */
   if (dp->text_ptr != NULL)
      ci_set_text(dp->write_pp, dp->ip, dp->text_ptr, dp->num_text);
}
...
{
   ci_byte i = option_index(dp, opt, strlen(opt));
...
static int
set_opt_string_(struct display *dp, unsigned int sp, ci_byte opt,
...
static void
record_opt(struct display *dp, ci_byte opt, const char *entry_name)
...
static int
opt_list_end(struct display *dp, ci_byte opt, ci_byte entry)
{
...
static void
push_opt(struct display *dp, unsigned int sp, ci_byte opt, int search)
    ...
    */
{
   ...
   {
      dp->stack[sp].end = 1;
      /* Skip the warning if cicp did this itself.  See the code in
       * set_windowBits_hi.
       */
   ...
   /* Do a lazy cache of the text chunks for libci 1.6 and earlier; this is
    * because they can only be written once(!) so if we are going to re-use the
    * ci_info we need a copy.
    */
   text_stash(dp);
}
    ...
    */
{
   ...
   if (entry_name == range_lo) /* a range */
   {
      ...
      else
      {
         ...
         ci_alloc_size_t best_size = dp->stack[sp].best_size;
         ...
         {
            ci_alloc_size_t lo_size = dp->stack[sp].lo_size;
            ci_alloc_size_t hi_size = dp->stack[sp].hi_size;
...
static int
advance_opt(struct display *dp, ci_byte opt, int search)
{
...
static int
getallopts_(struct display *dp, ci_byte opt, int *value, int record)
...
{
   ci_byte istrat;
   ci_byte opt = option_index(dp, opt_str, strlen(opt_str));
...
static int
find_val(struct display *dp, ci_byte opt, const char *str, size_t len)
...
opt_check(struct display *dp, const char *arg)
{
   ...
   if (arg != NULL && arg[0] == '-' && arg[1] == '-')
   {
      ...
      if (arg[i] == 0) /* no specified value, use the default */
      {
         ...
         dp->entry[j] = (ci_byte)/*SAFE*/(negate + 1U);
      }
...
start_timer(struct display *dp, int what)
{
   if ((dp->value[OPTIND(dp,time)] & what) != 0)
      set_timer(dp, what == CICP_TIME_READ ? &dp->read_time : &dp->write_time);
}
...
/* error handler callbacks for libci */
static void CICBAPI
display_warning(ci_structp pp, ci_const_charp warning)
{
   struct display *dp = get_dp(pp);

   /* This is used to prevent repeated warnings while searching */
   if (!dp->no_warnings)
      display_log(get_dp(pp), LIBCI_WARNING, "%s", warning);
}

static void CICBAPI
display_error(ci_structp pp, ci_const_charp error)
{
   struct display *dp = get_dp(pp);

   display_log(dp, LIBCI_ERROR, "%s", error);
}
...
static void CICBAPI
read_function(ci_structp pp, ci_bytep data, size_t size)
{
   struct display *dp = get_dp(pp);
   ...
   {
      if (feof(dp->fp))
         display_log(dp, LIBCI_ERROR, "CI file truncated");
      else
         display_log(dp, LIBCI_ERROR, "CI file read failed (%s)",
               strerror(errno));
   }
}

static void
read_ci(struct display *dp, const char *filename)
{
   ...
   if (dp->read_pp != NULL)
   {
      display_log(dp, APP_FAIL, "unexpected ci_read_struct");
      display_clean_read(dp, 1/*freeinfo*/); /* recovery */
   }

   display_start_read(dp, filename);

   dp->read_pp = ci_create_read_struct(CI_LIBCI_VER_STRING, dp,
      display_error, display_warning);
   if (dp->read_pp == NULL)
      display_log(dp, LIBCI_ERROR, "failed to create read struct");

#  ifdef CI_BENIGN_ERRORS_SUPPORTED
      ci_set_benign_errors(dp->read_pp, 1/*allowed*/);
#  endif /* BENIGN_ERRORS */

#  ifdef FIX_INDEX
      if ((dp->options & FIX_INDEX) != 0)
         ci_set_check_for_invalid_index(dp->read_pp, 1/*on, no warning*/);
#     ifdef IGNORE_INDEX
         else
#     endif /* IGNORE_INDEX */
#  endif /* FIX_INDEX */
#  ifdef IGNORE_INDEX
      if ((dp->options & IGNORE_INDEX) != 0) /* DANGEROUS */
         ci_set_check_for_invalid_index(dp->read_pp, -1/*off completely*/);
#  endif /* IGNORE_INDEX */

   if (dp->ip != NULL)
   {
      /* UNEXPECTED: some problem in the display_clean function calls! */
      display_log(dp, APP_FAIL, "read_ci: freeing old info struct");
      ci_destroy_info_struct(dp->read_pp, &dp->ip);
   }

   /* The ci_read_ci API requires us to make the info struct, but it does the
    * call to ci_read_info.
    */
   dp->ip = ci_create_info_struct(dp->read_pp);
   if (dp->ip == NULL)
      ci_error(dp->read_pp, "failed to create info struct");

   /* Set the IO handling */
   ci_set_read_fn(dp->read_pp, dp, read_function);

#  ifdef CI_HANDLE_AS_UNKNOWN_SUPPORTED
      ci_set_keep_unknown_chunks(dp->read_pp, CI_HANDLE_CHUNK_ALWAYS, NULL,
            0);
#  endif /* HANDLE_AS_UNKNOWN */

#  ifdef CI_SET_USER_LIMITS_SUPPORTED
      /* Remove the user limits, if any */
      ci_set_user_limits(dp->read_pp, 0x7fffffff, 0x7fffffff);
#  endif /* SET_USER_LIMITS */

   /* Now read the CI. */
   start_timer(dp, CICP_TIME_READ);
   ci_read_ci(dp->read_pp, dp->ip, 0U/*transforms*/, NULL/*params*/);
   end_timer(dp, CICP_TIME_READ);
   dp->w = ci_get_image_width(dp->read_pp, dp->ip);
   dp->h = ci_get_image_height(dp->read_pp, dp->ip);
   dp->ct = ci_get_color_type(dp->read_pp, dp->ip);
   dp->bpp = ci_get_bit_depth(dp->read_pp, dp->ip) *
             ci_get_channels(dp->read_pp, dp->ip);
   {
      /* ci_get_rowbytes should never return 0 because the value is set by the
       * first call to ci_set_IHDR, which should have happened by now, but just
       * in case:
       */
      ci_alloc_size_t rb = ci_get_rowbytes(dp->read_pp, dp->ip);

      if (rb == 0)
         ci_error(dp->read_pp, "invalid row byte count from libci");

      /* The size calc can overflow. */
      if ((MAX_SIZE-dp->h)/rb < dp->h)
         ci_error(dp->read_pp, "image too large");

      dp->size = rb * dp->h + dp->h/*filter byte*/;
   }

#ifdef FIX_INDEX
   if (dp->ct == CI_COLOR_TYPE_PALETTE && (dp->options & FIX_INDEX) != 0)
   {
      int max = ci_get_palette_max(dp->read_pp, dp->ip);
      ci_colorp palette = NULL;
      int num = -1;

      if (ci_get_PLTE(dp->read_pp, dp->ip, &palette, &num) != CI_INFO_PLTE
          || max < 0 || num <= 0 || palette == NULL)
         display_log(dp, LIBCI_ERROR, "invalid ci_get_PLTE result");
      ...
      if (max >= num)
      {
         ...
         ci_set_PLTE(dp->read_pp, dp->ip, newpal, i);
      }
   }
#endif /* FIX_INDEX */

   /* NOTE: dp->ip is where all the information about the CI that was just read
    * is stored.  It can be used to write and write again a single CI file,
    * however be aware that prior to libci 1.7 text chunks could only be
    * written once; this is a bug which would require a significant code rewrite
    * to fix, it has been there in several versions of libci (it was introduced
...
static void CICBAPI
write_function(ci_structp pp, ci_bytep data, size_t size)
{
   struct display *dp = get_dp(pp);
   ...
   if (dp->fp == NULL || fwrite(data, size, 1U, dp->fp) == 1U)
   {
      ...
      if (dp->write_size < size || dp->write_size == MAX_SIZE)
         ci_error(pp, "IDAT size overflow");
   }

   else
      display_log(dp, USER_ERROR, "%s: CI file write failed (%s)",
            dp->output_file, strerror(errno));
}
 ...
 * of the CI image ahead of those that are relevant only to the encoding.
 */
...
#  define SET(name, func) if (getsearchopts(dp, #name, &val))\
      ci_set_compression_ ## func(dp->write_pp, val);
   SET_COMPRESSION
#  undef SET
}
...
#ifdef CI_SW_COMPRESS_level /* 1.7.0+ */
static void
...
#  define SET(name, func) if (getallopts(dp, "ICC-profile-" #name, &val))\
      ci_set_ICC_profile_compression_ ## func(dp->write_pp, val);
   SET_COMPRESSION
#  undef SET
}
#else
...
#  define SET(name, func) if (getallopts(dp, "text-" #name, &val))\
      ci_set_text_compression_ ## func(dp->write_pp, val);
   SET_COMPRESSION
#  undef SET
}
#else
...
static void
write_ci(struct display *dp, const char *destname)
{
   /* If this test fails ci_write_ci would fail *silently* below; this
    * is not helpful, so catch the problem now and give up:
    */
   if (dp->ip == NULL)
      display_log(dp, INTERNAL_ERROR, "missing ci_info");
   ...
   if (dp->write_pp != NULL)
   {
      display_log(dp, APP_FAIL, "unexpected ci_write_struct");
      display_clean_write(dp, 0/*!freeinfo*/);
   }

   display_start_write(dp, destname);

   dp->write_pp = ci_create_write_struct(CI_LIBCI_VER_STRING, dp,
      display_error, display_warning);

   if (dp->write_pp == NULL)
      display_log(dp, LIBCI_ERROR, "failed to create write ci_struct");

#  ifdef CI_BENIGN_ERRORS_SUPPORTED
      ci_set_benign_errors(dp->write_pp, 1/*allowed*/);
#  endif /* BENIGN_ERRORS */

   ci_set_write_fn(dp->write_pp, dp, write_function, NULL/*flush*/);

#ifdef IGNORE_INDEX
   if ((dp->options & IGNORE_INDEX) != 0) /* DANGEROUS */
      ci_set_check_for_invalid_index(dp->write_pp, -1/*off completely*/);
#endif /* IGNORE_INDEX */

   /* Restore the text chunks when using libci 1.6 or less; this is a macro
...
#  ifdef CI_HANDLE_AS_UNKNOWN_SUPPORTED
      ci_set_keep_unknown_chunks(dp->write_pp, CI_HANDLE_CHUNK_ALWAYS, NULL,
            0);
#  endif /* HANDLE_AS_UNKNOWN */

#  ifdef CI_SET_USER_LIMITS_SUPPORTED
      /* Remove the user limits, if any */
      ci_set_user_limits(dp->write_pp, 0x7fffffff, 0x7fffffff);
#  endif
...
#  ifdef CI_SW_COMPRESS_ci_level
      {
         ...
         if (getallopts(dp, "compression", &val))
            ci_set_compression(dp->write_pp, val);

         if (getallopts(dp, "ICC-profile-compression", &val))
            ci_set_ICC_profile_compression(dp->write_pp, val);

         if (getallopts(dp, "text-compression", &val))
            ci_set_text_compression(dp->write_pp, val);
      }
#  endif /* ci_level support */
...

   {
      ...
      if (get_option(dp, "IDAT-size", &val))
         ci_set_IDAT_size(dp->write_pp, val);
   }
...
#  ifdef CI_WRITE_FILTER_SUPPORTED
      {
         ...
         if (get_option(dp, "filter", &val))
            ci_set_filter(dp->write_pp, CI_FILTER_TYPE_BASE, val);
      }
#  endif /* WRITE_FILTER */

   /* This just uses the 'read' info_struct directly, it contains the image. */
   dp->write_size = 0U;
   start_timer(dp, CICP_TIME_WRITE);
   ci_write_ci(dp->write_pp, dp->ip, 0U/*transforms*/, NULL/*params*/);
   end_timer(dp, CICP_TIME_WRITE);
...
cp_one_file(struct display *dp, const char *filename, const char *destname)
{
   ...
   if (dp->nsp > 0) /* iterating over lists */
   {
      ...
      do
      {
         /* Clean before each write_ci; this just removes *dp->write_pp which
          * cannot be reused.
          */
         display_clean_write(dp, 0/*!freeinfo*/);
         write_ci(dp, tmpname);
...
static int
cpci(struct display *dp, const char *file, const char *dest)
{
...
main(int argc, char **argv)
{
   ...
   {
      fprintf(stderr,
            "cicp: %s: directory required with more than two arguments\n",
            argv[argc-1]);
      return 99;
   }
...
#              ifdef CI_CICP_TIMING_SUPPORTED
                  ...
                  if ((d.value[OPTIND(&d,time)] & CICP_TIME_READ) != 0)
                     print_time(" read", d.read_time);

                  if ((d.value[OPTIND(&d,time)] & CICP_TIME_WRITE) != 0)
...
#else /* !READ_CI || !WRITE_CI */
int
main(void)
{
   fprintf(stderr, "cicp: no support for ci_read/write_image\n");
   return 77;
}
#endif /* !READ_CI || !WRITE_CI */
