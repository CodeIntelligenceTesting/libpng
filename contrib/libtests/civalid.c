/* civalid.c - validate libci by constructing then reading ci files.
 *
 ...
 *   generates bitmaps internally, stores them as CI files (using the
 *   sequential write code) then reads them back (using the sequential
 *   read code) and validates that the result has the correct data.
 *
 *   The program can be modified and extended to test the correctness of
 *   transformations performed by libci.
 */
...
/* civalid requires write support and one of the fixed or floating point APIs.
 * progressive read is also required currently as the progressive read pointer
 * is used to record the 'display' structure.
 */
#if defined CI_WRITE_SUPPORTED &&\
   (defined CI_PROGRESSIVE_READ_SUPPORTED) &&\
   (defined CI_FIXED_POINT_SUPPORTED || defined CI_FLOATING_POINT_SUPPORTED)
...
#  define RELEASE_BUILD (CI_LIBCI_BUILD_BASE_TYPE >= CI_LIBCI_BUILD_RC)
#endif
#if RELEASE_BUILD
...
struct ci_store;
define_exception_type(struct ci_store*);
...
#define context(ps,fault) anon_context(ps); ci_store *fault
 ...
 */
static void
make_random_bytes(ci_uint_32* seed, void* pv, size_t size)
{
   ci_uint_32 u0 = seed[0], u1 = seed[1];
   ci_bytep bytes = voidcast(ci_bytep, pv);
   ...
   for (i=0; i<size; ++i)
   {
      ...
      ci_uint_32 u = ((u0 >> (20-8)) ^ ((u1 << 7) | (u0 >> (32-7)))) & 0xff;
      u1 <<= 8;
      u1 |= u0 >> 24;
      u0 <<= 8;
      u0 |= u;
...
static void
make_four_random_bytes(ci_uint_32* seed, ci_bytep bytes)
{
   make_random_bytes(seed, bytes, 4);
}

#if defined CI_READ_SUPPORTED || defined CI_WRITE_tRNS_SUPPORTED ||\
    defined CI_WRITE_FILTER_SUPPORTED
static void
randomize_bytes(void *pv, size_t size)
{
   static ci_uint_32 random_seed[2] = {0x56789abc, 0xd};
...
random_u16(void)
{
   ...
   return ci_get_uint_16(b2);
}

#if defined CI_READ_RGB_TO_GRAY_SUPPORTED ||\
    defined CI_READ_FILLER_SUPPORTED
static ci_uint_32
random_u32(void)
{
   unsigned char b4[4];
   randomize_bytes(b4, sizeof b4);
   return ci_get_uint_32(b4);
}
#endif /* READ_FILLER || READ_RGB_TO_GRAY */

#endif /* READ || WRITE_tRNS || WRITE_FILTER */

#if defined CI_READ_TRANSFORMS_SUPPORTED ||\
    defined CI_WRITE_FILTER_SUPPORTED
static unsigned int
...
/* A numeric ID based on CI file characteristics.  The 'do_interlace' field
 * simply records whether civalid did the interlace itself or whether it
 * was done by libci.  Width and height must be less than 256.  'palette' is an
...
#define FILEID(col, depth, palette, interlace, width, height, do_interlace) \
   ((ci_uint_32)((col) + ((depth)<<3) + ((palette)<<8) + ((interlace)<<13) + \
    (((do_interlace)!=0)<<15) + ((width)<<16) + ((height)<<24)))

#define COL_FROM_ID(id) ((ci_byte)((id)& 0x7U))
#define DEPTH_FROM_ID(id) ((ci_byte)(((id) >> 3) & 0x1fU))
#define PALETTE_FROM_ID(id) (((id) >> 8) & 0x1f)
#define INTERLACE_FROM_ID(id) ((ci_byte)(((id) >> 13) & 0x3))
...
static size_t
standard_name(char *buffer, size_t bufsize, size_t pos, ci_byte colour_type,
    int bit_depth, unsigned int npalette, int interlace_type,
    ci_uint_32 w, ci_uint_32 h, int do_interlace)
{
   ...
   if (interlace_type != CI_INTERLACE_NONE)
   {
      pos = safecat(buffer, bufsize, pos, " interlaced");
      if (do_interlace)
         pos = safecat(buffer, bufsize, pos, "(civalid)");
      else
         pos = safecat(buffer, bufsize, pos, "(libci)");
   }
...
static size_t
standard_name_from_id(char *buffer, size_t bufsize, size_t pos, ci_uint_32 id)
{
...
static int
next_format(ci_bytep colour_type, ci_bytep bit_depth,
...
#ifdef CI_READ_TRANSFORMS_SUPPORTED
static unsigned int
sample(ci_const_bytep row, ci_byte colour_type, ci_byte bit_depth,
    ci_uint_32 x, unsigned int sample_index, int swap16, int littleendian)
{
   ci_uint_32 bit_index, result;
   ...
   /* Less than 8 bits per sample.  By default CI has the big end of
...
#endif /* CI_READ_TRANSFORMS_SUPPORTED */
 ...
 */
static void
pixel_copy(ci_bytep toBuffer, ci_uint_32 toIndex,
   ci_const_bytep fromBuffer, ci_uint_32 fromIndex, unsigned int pixelSize,
   int littleendian)
{
   ...
   if (pixelSize < 8) /* Sub-byte */
   {
      ...
      toBuffer[toIndex >> 3] = (ci_byte)(destByte | (sourceByte & destMask));
   }
 ...
 */
static void
row_copy(ci_bytep toBuffer, ci_const_bytep fromBuffer, unsigned int bitWidth,
      int littleendian)
{
   ...
   if ((bitWidth & 7) != 0)
   {
      ...
      if (littleendian)
         ...
         mask = 0xff >> (bitWidth & 7);
      *toBuffer = (ci_byte)((*toBuffer & mask) | (*fromBuffer & ~mask));
   }
}
 ...
 */
static int
pixel_cmp(ci_const_bytep pa, ci_const_bytep pb, ci_uint_32 bit_width)
{
#if CI_LIBCI_VER < 10506
   if (memcmp(pa, pb, bit_width>>3) == 0)
   {
       ...
       * after the end (the low order bits in CI.)
       */
      bit_width &= 7;
      p >>= 8-bit_width;
...
#else
   /* From libci-1.5.6 the overwrite should be fixed, so compare the trailing
    * bits too:
    */
...
/*************************** BASIC CI FILE WRITING ***************************/
/* A ci_store takes data from the sequential writer or provides data
 * to the sequential reader.  It can also store the result of a CI
 * write for later retrieval.
 */
#define STORE_BUFFER_SIZE 500 /* arbitrary */
typedef struct ci_store_buffer
{
   struct ci_store_buffer*  prev;    /* NOTE: stored in reverse order */
   ci_byte                  buffer[STORE_BUFFER_SIZE];
} ci_store_buffer;
...
typedef struct ci_store_file
{
   struct ci_store_file*  next;      /* as many as you like... */
   char                    name[FILE_NAME_SIZE];
   unsigned int            IDAT_bits; /* Number of bits in IDAT size */
   ci_uint_32             IDAT_size; /* Total size of IDAT data */
   ci_uint_32             id;        /* must be correct (see FILEID) */
   size_t                  datacount; /* In this (the last) buffer */
   ci_store_buffer        data;      /* Last buffer in file */
   int                     npalette;  /* Number of entries in palette */
   store_palette_entry*    palette;   /* May be NULL */
} ci_store_file;

/* The following is a pool of memory allocated by a single libci read or write
 * operation.
 */
typedef struct store_pool
{
   struct ci_store    *store;   /* Back pointer */
   struct store_memory *list;    /* List of allocated memory */
   ci_byte             mark[4]; /* Before and after data */

   /* Statistics for this run. */
   ci_alloc_size_t     max;     /* Maximum single allocation */
   ci_alloc_size_t     current; /* Current allocation */
   ci_alloc_size_t     limit;   /* Highest current allocation */
   ci_alloc_size_t     total;   /* Total allocation */

   /* Overall statistics (retained across successive runs). */
   ci_alloc_size_t     max_max;
   ci_alloc_size_t     max_limit;
   ci_alloc_size_t     max_total;
} store_pool;

typedef struct ci_store
{
   /* For cexcept.h exception handling - simply store one of these;
    * the context is a self pointer but it may point to a different
    * ci_store (in fact it never does in this program.)
    */
   ...
   /* Share fields */
   ci_uint_32        chunklen; /* Length of chunk+overhead (chunkpos >= 8) */
   ci_uint_32        chunktype;/* Type of chunk (valid if chunkpos >= 4) */
   ci_uint_32        chunkpos; /* Position in chunk */
   ci_uint_32        IDAT_size;/* Accumulated IDAT size in .new */
   unsigned int       IDAT_bits;/* Cache of the file store value */

   /* Read fields */
   ci_structp        pread;    /* Used to read a saved file */
   ci_infop          piread;
   ci_store_file*    current;  /* Set when reading */
   ci_store_buffer*  next;     /* Set when reading */
   size_t             readpos;  /* Position in *next */
   ci_byte*          image;    /* Buffer for reading interlaced images */
   size_t             cb_image; /* Size of this buffer */
   size_t             cb_row;   /* Row size of the image(s) */
   uLong              IDAT_crc;
   ci_uint_32        IDAT_len; /* Used when re-chunking IDAT chunks */
   ci_uint_32        IDAT_pos; /* Used when re-chunking IDAT chunks */
   ci_uint_32        image_h;  /* Number of rows in a single image */
   store_pool         read_memory_pool;

   /* Write fields */
   ci_store_file*    saved;
   ci_structp        pwrite;   /* Used when writing a new file */
   ci_infop          piwrite;
   size_t             writepos; /* Position in .new */
   char               wname[FILE_NAME_SIZE];
   ci_store_buffer   new;      /* The end of the new CI file being written. */
...
static void
store_pool_mark(ci_bytep mark)
{
   static ci_uint_32 store_seed[2] = { 0x12345678, 1};
...
static void
store_pool_init(ci_store *ps, store_pool *pool)
{
...
static void
store_freebuffer(ci_store_buffer* psb)
{
   if (psb->prev)
   {
...
static void
store_storenew(ci_store *ps)
{
   ci_store_buffer *pb;

   pb = voidcast(ci_store_buffer*, malloc(sizeof *pb));

   if (pb == NULL)
      ci_error(ps->pwrite, "store new: OOM");
...
static void
store_freefile(ci_store_file **ppf)
{
   if (*ppf != NULL)
   {
...
/* Main interface to file storage, after writing a new CI file (see the API
 * below) call store_storefile to store the result with the given name and id.
 */
static void
store_storefile(ci_store *ps, ci_uint_32 id)
{
   ci_store_file *pf;

   if (ps->chunkpos != 0U || ps->chunktype != 0U || ps->chunklen != 0U ||
       ps->IDAT_size == 0)
      ci_error(ps->pwrite, "storefile: incomplete write");

   pf = voidcast(ci_store_file*, malloc(sizeof *pf));
   if (pf == NULL)
      ci_error(ps->pwrite, "storefile: OOM");
   ...
   if (pf->IDAT_bits == 0U)
      ci_error(ps->pwrite, "storefile: 0 sized IDAT");
   ps->new.prev = NULL;
   ps->writepos = 0;
   ps->chunkpos = 8;
   ps->chunktype = 0;
...
static size_t
store_message(ci_store *ps, ci_const_structp pp, char *buffer, size_t bufsize,
...
static void
store_verbose(ci_store *ps, ci_const_structp pp, ci_const_charp prefix,
...
static void
store_log(ci_store* ps, ci_const_structp pp, ci_const_charp message,
   int is_error)
{
...
#ifdef CI_READ_SUPPORTED
/* Internal error function, called with a ci_store but no libci stuff. */
static void
internal_error(ci_store *ps, ci_const_charp message)
{
...
/* Functions to use as CI callbacks. */
static void CICBAPI
store_error(ci_structp ppIn, ci_const_charp message) /* CI_NORETURN */
{
   ci_const_structp pp = ppIn;
   ci_store *ps = voidcast(ci_store*, ci_get_error_ptr(pp));
...
static void CICBAPI
store_warning(ci_structp ppIn, ci_const_charp message)
{
   ci_const_structp pp = ppIn;
   ci_store *ps = voidcast(ci_store*, ci_get_error_ptr(pp));
 ...
 * the buffer is big enough, the ci_structp is for errors.
 */
/* Return a single row from the correct image. */
static ci_bytep
store_image_row(const ci_store* ps, ci_const_structp pp, int nImage,
   ci_uint_32 y)
{
   size_t coffset = (nImage * ps->image_h + y) * (ps->cb_row + 5) + 2;

   if (ps->image == NULL)
      ci_error(pp, "no allocated image");

   if (coffset + ps->cb_row + 3 > ps->cb_image)
      ci_error(pp, "image too small");

   return ps->image + coffset;
}

static void
store_image_free(ci_store *ps, ci_const_structp pp)
{
   if (ps->image != NULL)
   {
      ci_bytep image = ps->image;

      if (image[-1] != 0xed || image[ps->cb_image] != 0xfe)
      {
         if (pp != NULL)
            ci_error(pp, "ci_store image overwrite (1)");
         else
            store_log(ps, NULL, "ci_store image overwrite (2)", 1);
      }
...
static void
store_ensure_image(ci_store *ps, ci_const_structp pp, int nImages,
   size_t cbRow, ci_uint_32 cRows)
{
   ...
   if (ps->cb_image < cb)
   {
      ...
      image = voidcast(ci_bytep, malloc(cb+2));
      if (image == NULL)
      {
         ...
         ci_error(pp, "OOM allocating image buffer");
      }
   ...
   while (--nImages >= 0)
   {
      ...
      for (y=0; y<cRows; ++y)
      {
         ci_bytep row = store_image_row(ps, pp, nImages, y);
...
#ifdef CI_READ_SUPPORTED
static void
store_image_check(const ci_store* ps, ci_const_structp pp, int iImage)
{
   ci_const_bytep image = ps->image;

   if (image[-1] != 0xed || image[ps->cb_image] != 0xfe)
      ci_error(pp, "image overwrite");
   else
   {
      size_t cbRow = ps->cb_row;
      ci_uint_32 rows = ps->image_h;
      ...
      for (; rows > 0; --rows)
      {
         if (image[-2] != 190 || image[-1] != 239)
            ci_error(pp, "row start overwritten");

         if (image[cbRow] != 222 || image[cbRow+1] != 173 ||
            image[cbRow+2] != 17)
            ci_error(pp, "row end overwritten");
...
static int
valid_chunktype(ci_uint_32 chunktype)
{
...
static void CICBAPI
store_write(ci_structp ppIn, ci_bytep pb, size_t st)
{
   ci_const_structp pp = ppIn;
   ci_store *ps = voidcast(ci_store*, ci_get_io_ptr(pp));
   size_t writepos = ps->writepos;
   ci_uint_32 chunkpos = ps->chunkpos;
   ci_uint_32 chunktype = ps->chunktype;
   ci_uint_32 chunklen = ps->chunklen;

   if (ps->pwrite != pp)
      ci_error(pp, "store state damaged");

   /* Technically this is legal, but in practice libci never writes more than
    * the maximum chunk size at once so if it happens something weird has
    * changed inside libci (probably).
    */
   if (st > 0x7fffffffU)
      ci_error(pp, "unexpected write size");
      ...
      else if (chunkpos < 8)
      {
         ...
         if (++chunkpos == 8)
         {
            ...
            if (chunklen > 0x7fffffffU)
               ci_error(pp, "chunk length too great");

            chunktype &= 0xffffffffU;
            if (chunktype == CHUNK_IDAT)
            {
               if (chunklen > ~ps->IDAT_size)
                  ci_error(pp, "civalid internal image too large");

               ps->IDAT_size += chunklen;
            }

            else if (!valid_chunktype(chunktype))
               ci_error(pp, "invalid chunk type");
...
#ifdef CI_READ_SUPPORTED
static size_t
store_read_buffer_size(ci_store *ps)
{
...
static size_t
store_read_buffer_avail(ci_store *ps)
{
   if (ps->current != NULL && ps->next != NULL)
   {
      ci_store_buffer *next = &ps->current->data;
      ...
      if (next != ps->next)
         ci_error(ps->pread, "buffer read error");
...
static int
store_read_buffer_next(ci_store *ps)
{
   ci_store_buffer *pbOld = ps->next;
   ci_store_buffer *pbNew = &ps->current->data;
   if (pbOld != pbNew)
   {
 ...
 * during progressive read, where the io_ptr is set internally by libci.
 */
static void
store_read_imp(ci_store *ps, ci_bytep pb, size_t st)
{
   if (ps->current == NULL || ps->next == NULL)
      ci_error(ps->pread, "store state damaged");
   ...
   while (st > 0)
   {
      ...
      if (cbAvail > 0)
      {
         ...
         ci_error(ps->pread, "read beyond end of file");
   }
}

static size_t
store_read_chunk(ci_store *ps, ci_bytep pb, size_t max, size_t min)
{
   ci_uint_32 chunklen = ps->chunklen;
   ci_uint_32 chunktype = ps->chunktype;
   ci_uint_32 chunkpos = ps->chunkpos;
   ...
   if (st > 0) do
   {
      if (chunkpos >= chunklen) /* end of last chunk */
      {
         ...
         chunklen = ci_get_uint_32(buffer) + 12U;
         chunktype = ci_get_uint_32(buffer+4U);
         chunkpos = 0U; /* Position read so far */
      }

      if (chunktype == CHUNK_IDAT)
      {
         ci_uint_32 IDAT_pos = ps->IDAT_pos;
         ci_uint_32 IDAT_len = ps->IDAT_len;
         ci_uint_32 IDAT_size = ps->IDAT_size;
         ...
         if (IDAT_pos == IDAT_len)
         {
            ...
            else if (IDAT_size == 0U) /* all IDAT data read */
            {
               ...
               if (chunkpos != chunklen-4U)
                  ci_error(ps->pread, "internal: IDAT size mismatch");
         ...
         else if (IDAT_pos < IDAT_len - 4U) /* I.e not the CRC */
         {
            if (chunkpos < chunklen-4U)
            {
               ...
               chunkpos += (ci_uint_32)/*SAFE*/avail;
               IDAT_size -= (ci_uint_32)/*SAFE*/avail;
               IDAT_pos += (ci_uint_32)/*SAFE*/avail;
            }
      ...
      else /* !IDAT */
      {
         ...
         if (ps->IDAT_len > 0 && ps->IDAT_size > 0)
            ci_error(ps->pread, "internal: missing IDAT data");

         if (chunktype == CHUNK_IEND && ps->IDAT_len == 0U)
            ci_error(ps->pread, "internal: missing IDAT");
...
static void CICBAPI
store_read(ci_structp ppIn, ci_bytep pb, size_t st)
{
   ci_const_structp pp = ppIn;
   ci_store *ps = voidcast(ci_store*, ci_get_io_ptr(pp));

   if (ps == NULL || ps->pread != pp)
      ci_error(pp, "bad store read call");

   store_read_chunk(ps, pb, st, st);
}

static void
store_progressive_read(ci_store *ps, ci_structp pp, ci_infop pi)
{
   if (ps->pread != pp || ps->current == NULL || ps->next == NULL)
      ci_error(pp, "store state damaged (progressive)");
   ...
   while (store_read_buffer_avail(ps) > 0)
   {
      ...
      ci_process_data(pp, pi, buffer, cb);
   }
}
#endif /* CI_READ_SUPPORTED */
...
store_write_palette(ci_store *ps, int npalette)
{
   if (ps->pwrite == NULL)
      store_log(ps, NULL, "attempt to write palette without write stream", 1);

   if (ps->palette != NULL)
      ci_error(ps->pwrite, "multiple store_write_palette calls");
   ...
   if (npalette > 0)
   {
      ...
      if (ps->palette == NULL)
         ci_error(ps->pwrite, "store new palette: OOM");
...
#ifdef CI_READ_SUPPORTED
static store_palette_entry *
store_current_palette(ci_store *ps, int *npalette)
{
 ...
 * pointer returned to libci is just after the end of the header block, the
...
typedef struct store_memory
{
   ...
   ci_alloc_size_t     size;    /* Size of memory allocated */
   ci_byte             mark[4]; /* ID marker */
} store_memory;

/* Handle a fatal error in memory allocation.  This calls ci_error if the
 * libci struct is non-NULL, else it outputs a message and returns.  This means
 * that a memory problem while libci is running will abort (ci_error) the
 * handling of particular file while one in cleanup (after the destroy of the
 * struct has returned) will simply keep going and free (or attempt to free)
 * all the memory.
 */
static void
store_pool_error(ci_store *ps, ci_const_structp pp, const char *msg)
{
   if (pp != NULL)
      ci_error(pp, msg);

   /* Else we have to do it ourselves.  ci_error eventually calls store_log,
    * above.  store_log accepts a NULL ci_structp - it just changes what gets
...
static void
store_memory_free(ci_const_structp pp, store_pool *pool, store_memory *memory)
{
   ...
   {
      ci_alloc_size_t cb = memory->size;

      if (cb > pool->max)
         store_pool_error(pool->store, pp, "memory corrupted (size)");

      else if (memcmp((ci_bytep)(memory+1)+cb, pool->mark, sizeof pool->mark)
         != 0)
...
static void
store_pool_delete(ci_store *ps, store_pool *pool)
{
   if (pool->list != NULL)
   {
...
static ci_voidp CICBAPI
store_malloc(ci_structp ppIn, ci_alloc_size_t cb)
{
   ci_const_structp pp = ppIn;
   store_pool *pool = voidcast(store_pool*, ci_get_mem_ptr(pp));
   ...
   if (new != NULL)
   {
      ...
      memcpy((ci_byte*)(new+1) + cb, pool->mark, sizeof pool->mark);
      new->pool = pool;
      new->next = pool->list;
      pool->list = new;
      ++new;
   ...
   {
      /* NOTE: the CI user malloc function cannot use the ci_ptr it is passed
       * other than to retrieve the allocation pointer!  libci calls the
       * store_malloc callback in two basic cases:
       *
       * 1) From ci_malloc; ci_malloc will do a ci_error itself if NULL is
       *    returned.
       * 2) From ci_struct or ci_info structure creation; ci_malloc is
...
static void CICBAPI
store_free(ci_structp ppIn, ci_voidp memory)
{
   ci_const_structp pp = ppIn;
   store_pool *pool = voidcast(store_pool*, ci_get_mem_ptr(pp));
   store_memory *this = voidcast(store_memory*, memory), **test;

   /* Because libci calls store_free with a dummy ci_struct when deleting
    * ci_struct or ci_info via ci_destroy_struct_2 it is necessary to check
    * the passed in ci_structp to ensure it is valid, and not pass it to
    * ci_error if it is not.
    */
...
#endif /* CI_USER_MEM_SUPPORTED */

/* Setup functions. */
/* Cleanup when aborting a write or after storing the new file. */
static void
store_write_reset(ci_store *ps)
{
   if (ps->pwrite != NULL)
   {
      anon_context(ps);
      ...
      Try
         ci_destroy_write_struct(&ps->pwrite, &ps->piwrite);
...
/* The following is the main write function, it returns a ci_struct and,
 * optionally, a ci_info suitable for writiing a new CI file.  Use
 * store_storefile above to record this file after it has been written.  The
 * returned libci structures as destroyed by store_write_reset above.
 */
static ci_structp
set_store_for_write(ci_store *ps, ci_infopp ppi, const char *name)
{
   anon_context(ps);
   ...
   {
      if (ps->pwrite != NULL)
         ci_error(ps->pwrite, "write store already in use");
...
#     ifdef CI_USER_MEM_SUPPORTED
         if (!ps->speed)
            ps->pwrite = ci_create_write_struct_2(CI_LIBCI_VER_STRING,
               ps, store_error, store_warning, &ps->write_memory_pool,
               store_malloc, store_free);

         else
#     endif
         ps->pwrite = ci_create_write_struct(CI_LIBCI_VER_STRING,
            ps, store_error, store_warning);

      ci_set_write_fn(ps->pwrite, ps, store_write, store_flush);

#     ifdef CI_SET_OPTION_SUPPORTED
         {
            int opt;
            for (opt=0; opt<ps->noptions; ++opt)
               if (ci_set_option(ps->pwrite, ps->options[opt].option,
                  ps->options[opt].setting) == CI_OPTION_INVALID)
                  ci_error(ps->pwrite, "ci option invalid");
         }
#     endif

      if (ppi != NULL)
         *ppi = ps->piwrite = ci_create_info_struct(ps->pwrite);
   }
...
#  ifdef CI_READ_SUPPORTED
      if (ps->pread != NULL)
      {
         ...
         Try
            ci_destroy_read_struct(&ps->pread, &ps->piread, NULL);
...
#ifdef CI_READ_SUPPORTED
static void
store_read_set(ci_store *ps, ci_uint_32 id)
{
   ci_store_file *pf = ps->saved;
...

   {
      ...
      ci_error(ps->pread, msg);
   }
}

/* The main interface for reading a saved file - pass the id number of the file
 * to retrieve.  Ids must be unique or the earlier file will be hidden.  The API
 * returns a ci_struct and, optionally, a ci_info.  Both of these will be
 * destroyed by store_read_reset above.
 */
static ci_structp
set_store_for_read(ci_store *ps, ci_infopp ppi, ci_uint_32 id,
   const char *name)
{
   /* Set the name for ci_error */
   safecat(ps->test, sizeof ps->test, 0, name);

   if (ps->pread != NULL)
      ci_error(ps->pread, "read store already in use");

   store_read_reset(ps);

   /* Both the create APIs can return NULL if used in their default mode
    * (because there is no other way of handling an error because the jmp_buf
    * by default is stored in ci_struct and that has not been allocated!)
    * However, given that store_error works correctly in these circumstances
    * we don't ever expect NULL in this program.
    */
#  ifdef CI_USER_MEM_SUPPORTED
      if (!ps->speed)
         ps->pread = ci_create_read_struct_2(CI_LIBCI_VER_STRING, ps,
             store_error, store_warning, &ps->read_memory_pool, store_malloc,
             store_free);

      else
#  endif
   ps->pread = ci_create_read_struct(CI_LIBCI_VER_STRING, ps, store_error,
      store_warning);
   ...
   if (ps->pread == NULL)
   {
      ...
      store_log(ps, NULL, "ci_create_read_struct returned NULL (unexpected)",
         1 /*error*/);
...
#  ifdef CI_SET_OPTION_SUPPORTED
      {
         int opt;
         for (opt=0; opt<ps->noptions; ++opt)
            if (ci_set_option(ps->pread, ps->options[opt].option,
               ps->options[opt].setting) == CI_OPTION_INVALID)
                  ci_error(ps->pread, "ci option invalid");
      }
#  endif
   ...
   if (ppi != NULL)
      *ppi = ps->piread = ci_create_info_struct(ps->pread);
...
/*********************** CI FILE MODIFICATION ON READ ************************/
/* Files may be modified on read.  The following structure contains a complete
 * ci_store together with extra members to handle modification and a special
 * read callback for libci.  To use this the 'modifications' field must be set
 * to a list of ci_modification structures that actually perform the
 * modification, otherwise a ci_modifier is functionally equivalent to a
 * ci_store.  There is a special read function, set_modifier_for_read, which
...
#ifdef CI_READ_SUPPORTED
#if defined CI_READ_TRANSFORMS_SUPPORTED && defined CI_READ_cHRM_SUPPORTED
static double
...
typedef struct ci_modifier
{
   ci_store               this;             /* I am a ci_store */
   struct ci_modification *modifications;   /* Changes to make */

   modifier_state           state;           /* My state */

   /* Information from IHDR: */
   ci_byte                 bit_depth;       /* From IHDR */
   ci_byte                 colour_type;     /* From IHDR */

   /* While handling PLTE, IDAT and IEND these chunks may be pended to allow
    * other chunks to be inserted.
    */
   ci_uint_32              pending_len;
   ci_uint_32              pending_chunk;
    ...
    * ci_modifier because the modifier only sets it to 1 (true.)
    */
   unsigned int              repeat :1;   /* Repeat this transform test. */
   unsigned int              test_uses_encoding :1;

   /* Lowest sbit to test (pre-1.7 libci fails for sbit < 8) */
   ci_byte                 sbitlow;
    ...
    * internal check on civalid to ensure that the calculated error limits are
    * not ridiculous; without this it is too easy to make a mistake in civalid
   ...
   /* Flags: */
   /* Whether to call ci_read_update_info, not ci_read_start_image, and how
 ...
 * failed and it is running silently.
  */
static int fail(ci_modifier *pm)
{
...
modifier_init(ci_modifier *pm)
{
   ...
   pm->interlace_type = CI_INTERLACE_NONE;
   pm->test_standard = 0;
   pm->test_size = 0;
   pm->test_transform = 0;
...
/* This controls use of checks that explicitly know how libci digitizes the
 * samples in calculations; setting this circumvents simple error limit checking
 * in the rgb_to_gray check, replacing it with an exact copy of the libci 1.5
 * algorithm.
 */
#define DIGITIZE CI_LIBCI_VER != 10700
...
static double abserr(const ci_modifier *pm, int in_depth, int out_depth)
{
...
static double calcerr(const ci_modifier *pm, int in_depth, int out_depth)
{
...
static double pcerr(const ci_modifier *pm, int in_depth, int out_depth)
{
 ...
 * the expand_16 case with the current code in libci the expand happens after
 ...
 * it is added here.
 */
static double outerr(const ci_modifier *pm, int in_depth, int out_depth)
{
   /* There is a serious error in the 2 and 4 bit grayscale transform because
    * the gamma table value (8 bits) is simply shifted, not rounded, so the
    * error in 4 bit grayscale gamma is up to the value below.  This is a hack
    * to allow civalid to succeed:
    *
    * TODO: fix this in libci
    */
   if (out_depth == 2)
      return .73182-.5;
 ...
 */
static double outlog(const ci_modifier *pm, int in_depth, int out_depth)
{
 ...
 * 257!
 */
static int output_quantization_factor(const ci_modifier *pm, int in_depth,
   int out_depth)
{
 ...
 */
typedef struct ci_modification
{
   struct ci_modification *next;
   ci_uint_32              chunk;

   /* If the following is NULL all matching chunks will be removed: */
   int                    (*modify_fn)(struct ci_modifier *pm,
                               struct ci_modification *me, int add);
...
static void
modification_reset(ci_modification *pmm)
{
   if (pmm != NULL)
   {
      pmm->modified = 0;
...
static void
modification_init(ci_modification *pmm)
{
...
#ifdef CI_READ_RGB_TO_GRAY_SUPPORTED
static void
modifier_current_encoding(const ci_modifier *pm, color_encoding *ce)
{
 ...
 */
static unsigned int
modifier_total_encodings(const ci_modifier *pm)
{
   return 1 +                 /* (1) nothing */
       ...
       * ci_modifier code because 'bit_depth' is set when the IHDR is read.
...
static void
modifier_encoding_iterate(ci_modifier *pm)
{
 ...
 */
static void
modifier_set_encoding(ci_modifier *pm)
{
 ...
 */
static int
modifier_color_encoding_is_sRGB(const ci_modifier *pm)
{
...
static int
modifier_color_encoding_is_set(const ci_modifier *pm)
{
...
static void
modifier_crc(ci_bytep buffer)
{
   /* Recalculate the chunk CRC - a complete chunk must be in
    * the buffer, at the start.
    */
   uInt datalen = ci_get_uint_32(buffer);
   uLong crc = crc32(0, buffer+4, datalen+4);
   /* The cast to ci_uint_32 is safe because a crc32 is always a 32 bit value.
    */
   ci_save_uint_32(buffer+datalen+8, (ci_uint_32)crc);
}

static void
modifier_setbuffer(ci_modifier *pm)
{
   modifier_crc(pm->buffer);
   pm->buffer_count = ci_get_uint_32(pm->buffer)+12;
   pm->buffer_position = 0;
}

/* Separate the callback into the actual implementation (which is passed the
 * ci_modifier explicitly) and the callback, which gets the modifier from the
 * ci_struct.
 */
static void
modifier_read_imp(ci_modifier *pm, ci_bytep pb, size_t st)
{
   while (st > 0)
   {
      size_t cb;
      ci_uint_32 len, chunk;
      ci_modification *mod;

      if (pm->buffer_position >= pm->buffer_count) switch (pm->state)
      {
         static ci_byte sign[8] = { 137, 80, 78, 71, 13, 10, 26, 10 };
         case modifier_start:
            ...
            if (memcmp(pm->buffer, sign, 8) != 0)
               ci_error(pm->this.pread, "invalid CI file signature");
         ...
         case modifier_signature:
            ...
            if (ci_get_uint_32(pm->buffer) != 13 ||
                ci_get_uint_32(pm->buffer+4) != CHUNK_IHDR)
               ci_error(pm->this.pread, "invalid IHDR");
         ...
         default:
            ...
            {
               ci_save_uint_32(pm->buffer, pm->pending_len);
               ci_save_uint_32(pm->buffer+4, pm->pending_chunk);
               pm->pending_len = 0;
               pm->pending_chunk = 0;
            }
            else
            ...
            len = ci_get_uint_32(pm->buffer);
            chunk = ci_get_uint_32(pm->buffer+4);
...
/* The callback: */
static void CICBAPI
modifier_read(ci_structp ppIn, ci_bytep pb, size_t st)
{
   ci_const_structp pp = ppIn;
   ci_modifier *pm = voidcast(ci_modifier*, ci_get_io_ptr(pp));

   if (pm == NULL || pm->this.pread != pp)
      ci_error(pp, "bad modifier_read call");
 ...
 */
static void
modifier_progressive_read(ci_modifier *pm, ci_structp pp, ci_infop pi)
{
   ...
   if (pm->buffer_count > pm->buffer_position ||
      ...
      ci_error(pp, "progressive read implementation error");
}

/* Set up a modifier. */
static ci_structp
set_modifier_for_read(ci_modifier *pm, ci_infopp ppi, ci_uint_32 id,
    const char *name)
{
   /* Do this first so that the modifier fields are cleared even if an error
    * happens allocating the ci_struct.  No allocation is done here so no
    * cleanup is required.
    */
   pm->state = modifier_start;
   pm->bit_depth = 0;
...
{
   ci_modification this;
   ci_fixed_point  gamma;
} gama_modification;

static int
gama_modify(ci_modifier *pm, ci_modification *me, int add)
{
   UNUSED(add)
   /* This simply dumps the given gamma value into the buffer. */
   ci_save_uint_32(pm->buffer, 4);
   ci_save_uint_32(pm->buffer+4, CHUNK_gAMA);
   ci_save_uint_32(pm->buffer+8, ((gama_modification*)me)->gamma);
   return 1;
}

static void
gama_modification_init(gama_modification *me, ci_modifier *pm, double gammad)
{
   double g;
...
typedef struct chrm_modification
{
   ...
   ci_fixed_point           wx, wy, rx, ry, gx, gy, bx, by;
} chrm_modification;

static int
chrm_modify(ci_modifier *pm, ci_modification *me, int add)
{
   UNUSED(add)
   /* As with gAMA this just adds the required cHRM chunk to the buffer. */
   ci_save_uint_32(pm->buffer   , 32);
   ci_save_uint_32(pm->buffer+ 4, CHUNK_cHRM);
   ci_save_uint_32(pm->buffer+ 8, ((chrm_modification*)me)->wx);
   ci_save_uint_32(pm->buffer+12, ((chrm_modification*)me)->wy);
   ci_save_uint_32(pm->buffer+16, ((chrm_modification*)me)->rx);
   ci_save_uint_32(pm->buffer+20, ((chrm_modification*)me)->ry);
   ci_save_uint_32(pm->buffer+24, ((chrm_modification*)me)->gx);
   ci_save_uint_32(pm->buffer+28, ((chrm_modification*)me)->gy);
   ci_save_uint_32(pm->buffer+32, ((chrm_modification*)me)->bx);
   ci_save_uint_32(pm->buffer+36, ((chrm_modification*)me)->by);
   return 1;
}

static void
chrm_modification_init(chrm_modification *me, ci_modifier *pm,
   const color_encoding *encoding)
{
...
static int
srgb_modify(ci_modifier *pm, ci_modification *me, int add)
{
   UNUSED(add)
   /* As above, ignore add and just make a new chunk */
   ci_save_uint_32(pm->buffer, 1);
   ci_save_uint_32(pm->buffer+4, CHUNK_sRGB);
   pm->buffer[8] = ((srgb_modification*)me)->intent;
   return 1;
}

static void
srgb_modification_init(srgb_modification *me, ci_modifier *pm, ci_byte intent)
{
   modification_init(&me->this);
...
static int
sbit_modify(ci_modifier *pm, ci_modification *me, int add)
{
   ci_byte sbit = ((sbit_modification*)me)->sbit;
   if (pm->bit_depth > sbit)
   {
      ...
      switch (pm->colour_type)
      {
         ...
         default:
            ci_error(pm->this.pread,
               "unexpected colour type in sBIT modification");
      }

      ci_save_uint_32(pm->buffer, cb);
      ci_save_uint_32(pm->buffer+4, CHUNK_sBIT);
...
static void
sbit_modification_init(sbit_modification *me, ci_modifier *pm, ci_byte sbit)
{
   modification_init(&me->this);
   me->this.chunk = CHUNK_sBIT;
 ...
 * libci with respect to 'odd' image sizes - particularly small images where
 ...
 *
 * Because the CI filter code is typically the largest CPU consumer within
 * libci itself there is a tendency to attempt to optimize it.  This results in
 ...
 * particularly important for testing the gamma code because libci uses a
 ...
 * but a given set of arguments to civalid will always produce the same palette
 ...
 */
static store_palette_entry *
make_standard_palette(ci_store* ps, int npalette, int do_tRNS)
{
   static ci_uint_32 palette_seed[2] = { 0x87654321, 9 };

   int i = 0;
   ci_byte values[256][4];

   /* Always put in black and white plus the six primary and secondary colors.
    */
   for (; i<8; ++i)
   {
      values[i][1] = (ci_byte)((i&1) ? 255U : 0U);
      values[i][2] = (ci_byte)((i&2) ? 255U : 0U);
      values[i][3] = (ci_byte)((i&4) ? 255U : 0U);
   }

   /* Then add 62 grays (one quarter of the remaining 256 slots). */
   {
      int j = 0;
      ci_byte random_bytes[4];
      ci_byte need[256];
    ...
    */
   {
      ...
      if (do_tRNS)
         for (i=0; i<256; ++i)
            values[i][0] = (ci_byte)(i ^ selector[0]);
 ...
 */
/* TODO: the ci_structp here can probably be 'const' in the future */
static void
init_standard_palette(ci_store *ps, ci_structp pp, ci_infop pi, int npalette,
   int do_tRNS)
{
...

   {
      ...
      ci_set_PLTE(pp, pi, palette, npalette);
   }
...
#ifdef CI_WRITE_tRNS_SUPPORTED
      if (j > 0)
         ci_set_tRNS(pp, pi, tRNS, j, 0/*color*/);
#endif
   }
}
...
#ifdef CI_WRITE_tRNS_SUPPORTED
static void
set_random_tRNS(ci_structp pp, ci_infop pi, ci_byte colour_type,
   int bit_depth)
{
    ...
    */
   ci_color_16 tRNS;
   ci_uint_16 mask = (ci_uint_16)((1U << bit_depth)-1);
   ...
   if (colour_type & 2/*RGB*/)
   {
      ...
      {
         tRNS.red = random_u16();
         tRNS.green = (ci_uint_16)(tRNS.red * 257);
         tRNS.blue = (ci_uint_16)(tRNS.green * 17);
      }
   }
   ...
   ci_set_tRNS(pp, pi, NULL, 0, &tRNS);
}
#endif

/* The number of passes is related to the interlace type. There was no libci
 * API to determine this prior to 1.5, so we need an inquiry function:
 */
static int
npasses_from_interlace_type(ci_const_structp pp, int interlace_type)
{
   switch (interlace_type)
   {
   default:
      ci_error(pp, "invalid interlace type");

   case CI_INTERLACE_NONE:
      return 1;

   case CI_INTERLACE_ADAM7:
      return CI_INTERLACE_ADAM7_PASSES;
   }
}

static unsigned int
bit_size(ci_const_structp pp, ci_byte colour_type, ci_byte bit_depth)
{
   switch (colour_type)
   {
      default: ci_error(pp, "invalid color type");
...
static size_t
transform_rowsize(ci_const_structp pp, ci_byte colour_type,
   ci_byte bit_depth)
{
...
static ci_uint_32
transform_height(ci_const_structp pp, ci_byte colour_type, ci_byte bit_depth)
{
 ...
 */
static ci_uint_32
standard_width(ci_const_structp pp, ci_uint_32 id)
{
   ci_uint_32 width = WIDTH_FROM_ID(id);
   UNUSED(pp)
...
static ci_uint_32
standard_height(ci_const_structp pp, ci_uint_32 id)
{
   ci_uint_32 height = HEIGHT_FROM_ID(id);
...
static ci_uint_32
standard_rowsize(ci_const_structp pp, ci_uint_32 id)
{
   ci_uint_32 width = standard_width(pp, id);
...
static void
transform_row(ci_const_structp pp, ci_byte buffer[TRANSFORM_ROWMAX],
   ci_byte colour_type, ci_byte bit_depth, ci_uint_32 y)
{
   ci_uint_32 v = y << 7;
   ci_uint_32 i = 0;
   ...
   {
      case 1:
         while (i<128/8) buffer[i] = (ci_byte)(v & 0xff), v += 17, ++i;
         return;

      case 2:
         while (i<128/4) buffer[i] = (ci_byte)(v & 0xff), v += 33, ++i;
         return;

      case 4:
         while (i<128/2) buffer[i] = (ci_byte)(v & 0xff), v += 65, ++i;
         return;

      case 8:
         /* 256 bytes total, 128 bytes in each row set as follows: */
         while (i<128) buffer[i] = (ci_byte)(v & 0xff), ++v, ++i;
         return;
      ...
      case 16:
         ...
         while (i<128)
         {
            buffer[2*i] = (ci_byte)((v>>8) & 0xff);
            buffer[2*i+1] = (ci_byte)(v & 0xff);
            ++v;
            ++i;
         }
      ...
      case 24:
         ...
         while (i<128)
         {
            ...
            buffer[3*i+0] = (ci_byte)((v >> 8) & 0xff);
            buffer[3*i+1] = (ci_byte)(v & 0xff);
            buffer[3*i+2] = (ci_byte)(((v >> 8) ^ v) & 0xff);
            ++v;
            ++i;
         }
      ...
      case 32:
         ...
         while (i<128)
         {
            buffer[4*i+0] = (ci_byte)((v >> 8) & 0xff);
            buffer[4*i+1] = (ci_byte)(v & 0xff);
            buffer[4*i+2] = (ci_byte)((v >> 8) & 0xff);
            buffer[4*i+3] = (ci_byte)(v & 0xff);
            ++v;
            ++i;
         }
      ...
      case 48:
         ...
         {
            ci_uint_32 t = v++;
            buffer[6*i+0] = (ci_byte)((t >> 8) & 0xff);
            buffer[6*i+1] = (ci_byte)(t & 0xff);
            t *= 257;
            buffer[6*i+2] = (ci_byte)((t >> 8) & 0xff);
            buffer[6*i+3] = (ci_byte)(t & 0xff);
            t *= 17;
            buffer[6*i+4] = (ci_byte)((t >> 8) & 0xff);
            buffer[6*i+5] = (ci_byte)(t & 0xff);
            ++i;
         }
      ...
      case 64:
         ...
         {
            ci_uint_32 t = v++;
            buffer[8*i+0] = (ci_byte)((t >> 8) & 0xff);
            buffer[8*i+1] = (ci_byte)(t & 0xff);
            buffer[8*i+4] = (ci_byte)((t >> 8) & 0xff);
            buffer[8*i+5] = (ci_byte)(t & 0xff);
            t *= 257;
            buffer[8*i+2] = (ci_byte)((t >> 8) & 0xff);
            buffer[8*i+3] = (ci_byte)(t & 0xff);
            buffer[8*i+6] = (ci_byte)((t >> 8) & 0xff);
            buffer[8*i+7] = (ci_byte)(t & 0xff);
            ++i;
         }
         return;
...
#  define INTERLACE_LAST CI_INTERLACE_LAST
#  define check_interlace_type(type) ((void)(type))
#  define set_write_interlace_handling(pp,type) ci_set_interlace_handling(pp)
#  define do_own_interlace 0
#elif CI_LIBCI_VER != 10700
#  define set_write_interlace_handling(pp,type) (1)
static void
check_interlace_type(int const interlace_type)
{
   /* Prior to 1.7.0 libci does not support the write of an interlaced image
    * unless CI_WRITE_INTERLACING_SUPPORTED, even with do_interlace so the
   ...
   if (interlace_type != CI_INTERLACE_NONE)
   {
       ...
       * attempted.
       */
      fprintf(stderr, "civalid: no interlace support\n");
      exit(99);
   }
}
#  define INTERLACE_LAST (CI_INTERLACE_NONE+1)
#  define do_own_interlace 0
#else /* libci 1.7+ */
...
#if CI_LIBCI_VER == 10700 || defined CI_WRITE_INTERLACING_SUPPORTED
 ...
 * their own de-interlace or use libci.
 */
#ifdef CI_READ_INTERLACING_SUPPORTED
#  define do_read_interlace 0
#else /* no libci read interlace support */
#  define do_read_interlace 1
#endif
/* The following two routines use the CI interlace support macros from
 * ci.h to interlace or deinterlace rows.
 */
static void
interlace_row(ci_bytep buffer, ci_const_bytep imageRow,
   unsigned int pixel_size, ci_uint_32 w, int pass, int littleendian)
{
   ci_uint_32 xin, xout, xstep;

   /* Note that this can, trivially, be optimized to a memcpy on pass 7, the
    * code is presented this way to make it easier to understand.  In practice
    * consult the code in the libci source to see other ways of doing this.
    *
    * It is OK for buffer and imageRow to be identical, because 'xin' moves
    * faster than 'xout' and we copy up.
    */
   xin = CI_PASS_START_COL(pass);
   xstep = 1U<<CI_PASS_COL_SHIFT(pass);
...
#ifdef CI_READ_SUPPORTED
static void
deinterlace_row(ci_bytep buffer, ci_const_bytep row,
   unsigned int pixel_size, ci_uint_32 w, int pass, int littleendian)
{
 ...
 * that test odd sizes along with the libci interlace handling.
 */
#ifdef CI_WRITE_FILTER_SUPPORTED
static void
choose_random_filter(ci_structp pp, int start)
{
   /* Choose filters randomly except that on the very first row ensure that
    * there is at least one previous row filter.
    */
   int filters = CI_ALL_FILTERS & random_mod(256U);

   /* There may be no filters; skip the setting. */
   if (filters != 0)
   {
      if (start && filters < CI_FILTER_UP)
         filters |= CI_FILTER_UP;

      ci_set_filter(pp, 0/*method*/, filters);
   }
}
#else /* !WRITE_FILTER */
...
static void
make_transform_image(ci_store* const ps, ci_byte const colour_type,
    ci_byte const bit_depth, unsigned int palette_number,
    int interlace_type, ci_const_charp name)
{
   context(ps, fault);
   ...
   {
      ci_infop pi;
      ci_structp pp = set_store_for_write(ps, &pi, name);
      ci_uint_32 h, w;
      ...
      ci_set_IHDR(pp, pi, w, h, bit_depth, colour_type, interlace_type,
         CI_COMPRESSION_TYPE_BASE, CI_FILTER_TYPE_BASE);

#ifdef CI_TEXT_SUPPORTED
#  if defined(CI_READ_zTXt_SUPPORTED) && defined(CI_WRITE_zTXt_SUPPORTED)
#     define TEXT_COMPRESSION CI_TEXT_COMPRESSION_zTXt
#  else
#     define TEXT_COMPRESSION CI_TEXT_COMPRESSION_NONE
#  endif
      {
...
#     endif
      ...
      if (ci_get_rowbytes(pp, pi) !=
          transform_rowsize(pp, colour_type, bit_depth))
         ci_error(pp, "transform row size incorrect");

      else
      {
         /* Somewhat confusingly this must be called *after* ci_write_info
         ...
         if (npasses != npasses_from_interlace_type(pp, interlace_type))
            ci_error(pp, "write: ci_set_interlace_handling failed");
         ...
         for (pass=0; pass<npasses; ++pass)
         {
            ...
            for (y=0; y<h; ++y)
            {
               ci_byte buffer[TRANSFORM_ROWMAX];
...
#              if do_own_interlace
                   ...
                   */
                  if (interlace_type == CI_INTERLACE_ADAM7)
                  {
                      ...
                      */
                     if (CI_ROW_IN_INTERLACE_PASS(y, pass) &&
                         CI_PASS_COLS(w, pass) > 0)
...
#              endif /* do_own_interlace */
   ...
   Catch(fault)
   {
      /* Use the ci_store returned by the exception. This may help the compiler
...
static void
make_transform_images(ci_modifier *pm)
{
   ci_byte colour_type = 0;
   ci_byte bit_depth = 0;
   ...
   while (next_format(&colour_type, &bit_depth, &palette_number, 1, 1))
   {
      ...
      for (interlace_type = CI_INTERLACE_NONE;
 ...
 */
static void
size_row(ci_byte buffer[SIZE_ROWMAX], ci_uint_32 bit_width, ci_uint_32 y)
{
   ...
   if (bit_width > 0)
   {
      ci_uint_32 mask = (1U<<(8-bit_width))-1;
      *buffer = (ci_byte)((*buffer & mask) | (y & ~mask));
   }
}

static void
make_size_image(ci_store* const ps, ci_byte const colour_type,
    ci_byte const bit_depth, int const interlace_type,
    ci_uint_32 const w, ci_uint_32 const h,
    int const do_interlace)
{
   ...
   Try
   {
      ...
      char name[FILE_NAME_SIZE];
      ci_uint_32 id = FILEID(colour_type, bit_depth, 0/*palette*/,
      ...
      if (ci_get_rowbytes(pp, pi) != ((w * pixel_size) + 7) / 8)
         ci_error(pp, "size row size incorrect");
      ...
      else
      {
         ...
         for (pass=0; pass<npasses; ++pass)
         {
            ...
            ci_uint_32 wPass = CI_PASS_COLS(w, pass);

            /* If do_interlace is set we don't call ci_write_row for every
             * row because some of them are empty.  In fact, for a 1x1 image,
             * most of them are empty!
             */
            for (y=0; y<h; ++y)
            {
               ci_const_bytep row = image[y];
               ci_byte tempRow[SIZE_ROWMAX];

               /* If do_interlace *and* the image is interlaced we
                * need a reduced interlace row; this may be reduced
                * to empty.
                */
               if (do_interlace && interlace_type == CI_INTERLACE_ADAM7)
               {
                  /* The row must not be written if it doesn't exist, notice
                   * that there are two conditions here, either the row isn't
                   * ever in the pass or the row would be but isn't wide
                   ...
                   * can be used to skip the whole y loop in this case.
                   */
                  if (CI_ROW_IN_INTERLACE_PASS(y, pass) && wPass > 0)
                  {
                     /* Set to all 1's for error detection (libci tends to
...
#           ifdef CI_WRITE_FILTER_SUPPORTED
                ...
                * single filter.  It isn't well documented, but ci_set_filter
                ...
                * the set so that libci allocates the row buffer.
                */
               {
                  int filters = 8 << random_mod(CI_FILTER_VALUE_LAST);

                  if (pass == 0 && y == 0 &&
                      (filters < CI_FILTER_UP || w == 1U))
                     filters |= CI_FILTER_UP;
...
static void
make_size(ci_store* const ps, ci_byte const colour_type, int bdlo,
    int const bdhi)
{
   for (; bdlo <= bdhi; ++bdlo)
   {
      ...
      for (width = 1; width <= 16; ++width)
      {
         ...
         for (height = 1; height <= 16; ++height)
         {
             ...
             * libci doing it.
             */
            make_size_image(ps, colour_type, DEPTH(bdlo), CI_INTERLACE_NONE,
               width, height, 0);
            make_size_image(ps, colour_type, DEPTH(bdlo), CI_INTERLACE_NONE,
               width, height, 1);
#        ifdef CI_WRITE_INTERLACING_SUPPORTED
            make_size_image(ps, colour_type, DEPTH(bdlo), CI_INTERLACE_ADAM7,
               width, height, 0);
#        endif
...
static void
standard_row(ci_const_structp pp, ci_byte std[STANDARD_ROWMAX],
   ci_uint_32 id, ci_uint_32 y)
{
 ...
 * invalid image with libci!
 */
/* TODO: the 'set' functions can probably all be made to take a
 * ci_const_structp rather than a modifiable one.
 */
#ifdef CI_WARNINGS_SUPPORTED
static void
sBIT0_error_fn(ci_structp pp, ci_infop pi)
{
   /* 0 is invalid... */
   ci_color_8 bad;
   bad.red = bad.green = bad.blue = bad.gray = bad.alpha = 0;
   ci_set_sBIT(pp, pi, &bad);
}

static void
sBIT_error_fn(ci_structp pp, ci_infop pi)
{
   ci_byte bit_depth;
   ci_color_8 bad;

   if (ci_get_color_type(pp, pi) == CI_COLOR_TYPE_PALETTE)
      bit_depth = 8;

   else
      bit_depth = ci_get_bit_depth(pp, pi);
...
static void
make_error(ci_store* const ps, ci_byte const colour_type,
    ci_byte bit_depth, int interlace_type, int test, ci_const_charp name)
{
   context(ps, fault);
...
#undef exception__env
      ...
      else
      {
          ...
          * undetected, error has not created problems inside libci.  This
          * doesn't work if there was a ci_error in ci_write_info because that
          * can abort before PLTE was written.
          */
         if (ci_get_rowbytes(pp, pi) !=
             transform_rowsize(pp, colour_type, bit_depth))
            ci_error(pp, "row size incorrect");
...
static int
make_errors(ci_modifier* const pm, ci_byte const colour_type,
...
#endif /* CI_WARNINGS_SUPPORTED */

static void
perform_error_test(ci_modifier *pm)
{
#ifdef CI_WARNINGS_SUPPORTED /* else there are no cases that work! */
...
/* This is just to validate the internal CI formatting code - if this fails
 * then the warning messages the library outputs will probably be garbage.
 */
static void
perform_formatting_test(ci_store *ps)
{
   ...
   Try
   {
      ci_const_charp correct = "29 Aug 2079 13:53:60 +0000";
      ci_const_charp result;
#     if CI_LIBCI_VER >= 10600
         char timestring[29];
#     endif
      ci_structp pp;
      ...
      pp = set_store_for_write(ps, NULL, "libci formatting test");
...
#     if CI_LIBCI_VER < 10600
         result = ci_convert_to_rfc1123(pp, &pt);
#     else
         if (ci_convert_to_rfc1123_buffer(timestring, &pt))
            result = timestring;
...
#     endif
      ...
      if (result == NULL)
         ci_error(pp, "ci_convert_to_rfc1123 failed");
      ...
      if (strcmp(result, correct) != 0)
      {
         ...
         pos = safecat(msg, sizeof msg, pos, "ci_convert_to_rfc1123(");
 ...
 * reader callbacks only have one parameter (ci_get_progressive_ptr()), so this
...
{
   ci_store*  ps;             /* Test parameters (passed to the function) */
   ci_byte    colour_type;
   ci_byte    bit_depth;
   ci_byte    red_sBIT;       /* Input data sBIT values. */
   ci_byte    green_sBIT;
   ci_byte    blue_sBIT;
   ci_byte    alpha_sBIT;
   ci_byte    interlace_type;
   ci_byte    filler;         /* Output has a filler */
   ci_uint_32 id;             /* Calculated file ID */
   ci_uint_32 w;              /* Width of image */
   ci_uint_32 h;              /* Height of image */
   int         npasses;        /* Number of interlaced passes */
   ci_uint_32 pixel_size;     /* Width of one pixel in bits */
   ci_uint_32 bit_width;      /* Width of output row in bits */
...
static void
standard_display_init(standard_display *dp, ci_store* ps, ci_uint_32 id,
 ...
 * comes from the particular ci_store_file that is selected.
 */
static void
...
/* Utility to read the palette from the CI file and convert it into
 ...
 */
static int
read_palette(store_palette palette, int *npalette, ci_const_structp pp,
   ci_infop pi)
{
   ci_colorp pal;
   ...
   *npalette = -1;
   ...
   if (ci_get_PLTE(pp, pi, &pal, npalette) & CI_INFO_PLTE)
   {
      int i = *npalette;

      if (i <= 0 || i > 256)
         ci_error(pp, "validate: invalid PLTE count");
   ...
   {
      if (*npalette != (-1))
         ci_error(pp, "validate: invalid PLTE result");
   ...
   if ((ci_get_tRNS(pp, pi, &trans_alpha, &num, 0) & CI_INFO_tRNS) != 0 &&
      (trans_alpha != NULL || num != 1/*returns 1 for a transparent color*/) &&
      /* Oops, if a palette tRNS gets expanded ci_read_update_info (at least so
       ...
       */
      !(trans_alpha != NULL && num == 0)) /* TODO: fix this in libci. */
   {
      int i;

      /* Any of these are crash-worthy - given the implementation of
       * ci_get_tRNS up to 1.5 an app won't crash if it just checks the
       * result above and fails to check that the variables it passed have
       * actually been filled in!  Note that if the app were to pass the
       * last, ci_color_16p, variable too it couldn't rely on this.
       */
      if (trans_alpha == NULL || num <= 0 || num > 256 || num > *npalette)
         ci_error(pp, "validate: unexpected ci_get_tRNS (palette) result");
 ...
 */
static void
standard_palette_validate(standard_display *dp, ci_const_structp pp,
   ci_infop pi)
{
   int npalette;
   ...
   if (read_palette(palette, &npalette, pp, pi) != dp->is_transparent)
      ci_error(pp, "validate: palette transparency changed");
...

   {
      ...
      while (--i >= 0)
         if (palette[i].red != dp->palette[i].red ||
            ...
            ci_error(pp, "validate: PLTE or tRNS chunk changed");
   }
}
 ...
 * ci_read_update_info or ci_start_read_image and what gets called depends on
 * whether the info needs updating (we want to test both calls in civalid.)
 */
static void
standard_info_part1(standard_display *dp, ci_structp pp, ci_infop pi)
{
   if (ci_get_bit_depth(pp, pi) != dp->bit_depth)
      ci_error(pp, "validate: bit depth changed");

   if (ci_get_color_type(pp, pi) != dp->colour_type)
      ci_error(pp, "validate: color type changed");

   if (ci_get_filter_type(pp, pi) != CI_FILTER_TYPE_BASE)
      ci_error(pp, "validate: filter type changed");

   if (ci_get_interlace_type(pp, pi) != dp->interlace_type)
      ci_error(pp, "validate: interlacing changed");

   if (ci_get_compression_type(pp, pi) != CI_COMPRESSION_TYPE_BASE)
      ci_error(pp, "validate: compression type changed");

   dp->w = ci_get_image_width(pp, pi);

   if (dp->w != standard_width(pp, dp->id))
      ci_error(pp, "validate: image width changed");

   dp->h = ci_get_image_height(pp, pi);

   if (dp->h != standard_height(pp, dp->id))
      ci_error(pp, "validate: image height changed");
    ...
    */
   {
      ...
      if (ci_get_sBIT(pp, pi, &sBIT) & CI_INFO_sBIT)
      {
         int sBIT_invalid = 0;

         if (sBIT == 0)
            ci_error(pp, "validate: unexpected ci_get_sBIT result");

         if (dp->colour_type & CI_COLOR_MASK_COLOR)
         {
          ...
          * spec.
          */
         if (dp->colour_type & CI_COLOR_MASK_ALPHA)
         {
         ...
         if (sBIT_invalid)
            ci_error(pp, "validate: sBIT value out of range");
      }
   }
    ...
    * ci_read_update_info.
    */
   if (ci_get_rowbytes(pp, pi) != standard_rowsize(pp, dp->id))
      ci_error(pp, "validate: row size changed");
    ...
    */
   {
      ci_color_16p trans_color = 0;

      if (ci_get_tRNS(pp, pi, 0, 0, &trans_color) & CI_INFO_tRNS)
      {
         if (trans_color == 0)
            ci_error(pp, "validate: unexpected ci_get_tRNS (color) result");
         ...
         case 3:
             ...
             * above.
             */
            ci_error(pp, "validate: unexpected ci_get_tRNS result");
            break;

         default:
            ci_error(pp, "validate: invalid tRNS chunk with alpha image");
         }
      }
   }
   ...
   {
#     ifdef CI_READ_INTERLACING_SUPPORTED
         if (dp->npasses != ci_set_interlace_handling(pp))
            ci_error(pp, "validate: file changed interlace type");
#     else /* !READ_INTERLACING */
         /* This should never happen: the relevant tests (!do_interlace) should
          * not be run.
          */
         if (dp->npasses > 1)
            ci_error(pp, "validate: no libci interlace support");
#     endif /* !READ_INTERLACING */
   }

   /* Caller calls ci_read_update_info or ci_start_read_image now, then calls
    * part2.
    */
}

/* This must be called *after* the ci_read_update_info call to get the correct
 * 'rowbytes' value, otherwise ci_get_rowbytes will refer to the untransformed
 * image.
 */
static void
standard_info_part2(standard_display *dp, ci_const_structp pp,
    ci_const_infop pi, int nImages)
{
   /* Record cbRow now that it can be found. */
   {
      ci_byte ct = ci_get_color_type(pp, pi);
      ci_byte bd = ci_get_bit_depth(pp, pi);

      if (bd >= 8 && (ct == CI_COLOR_TYPE_RGB || ct == CI_COLOR_TYPE_GRAY) &&
          dp->filler)
          ct |= 4; /* handle filler as faked alpha channel */

      dp->pixel_size = bit_size(pp, ct, bd);
   }
   dp->bit_width = ci_get_image_width(pp, pi) * dp->pixel_size;
   dp->cbRow = ci_get_rowbytes(pp, pi);

   /* Validate the rowbytes here again. */
   if (dp->cbRow != (dp->bit_width+7)/8)
      ci_error(pp, "bad ci_get_rowbytes calculation");
...
static void
standard_info_imp(standard_display *dp, ci_structp pp, ci_infop pi,
    int nImages)
{
   ...
   /* And the info callback has to call this (or ci_read_update_info - see
    * below in the ci_modifier code for that variant.
    */
   if (dp->use_update_info)
   {
      ...
      while (i-- > 0)
         ci_read_update_info(pp, pi);
   }

   else
      ci_start_read_image(pp);
...
static void CICBAPI
standard_info(ci_structp pp, ci_infop pi)
{
   standard_display *dp = voidcast(standard_display*,
      ci_get_progressive_ptr(pp));
...
static void CICBAPI
progressive_row(ci_structp ppIn, ci_bytep new_row, ci_uint_32 y, int pass)
{
   ci_const_structp pp = ppIn;
    ...
    * ci_get_rowbytes because we got no info structure.
    */
   if (new_row != NULL)
   {
      ci_bytep row;
       ...
       */
      if (dp->do_interlace && dp->interlace_type == CI_INTERLACE_ADAM7)
      {
#ifdef CI_USER_TRANSFORM_INFO_SUPPORTED
         /* Use this opportunity to validate the ci 'current' APIs: */
         if (y != ci_get_current_row_number(pp))
            ci_error(pp, "ci_get_current_row_number is broken");

         if (pass != ci_get_current_pass_number(pp))
            ci_error(pp, "ci_get_current_pass_number is broken");
#endif /* USER_TRANSFORM_INFO */

         y = CI_ROW_FROM_PASS_ROW(y, pass);
      }

      /* Validate this just in case. */
      if (y >= dp->h)
         ci_error(pp, "invalid y to progressive row callback");
...
#endif /* READ_INTERLACING */
      {
         if (dp->interlace_type == CI_INTERLACE_ADAM7)
         ...
         ci_progressive_combine_row(pp, row, new_row);
#endif /* CI_READ_INTERLACING_SUPPORTED */
   }

   else if (dp->interlace_type == CI_INTERLACE_ADAM7 &&
       CI_ROW_IN_INTERLACE_PASS(y, pass) &&
       CI_PASS_COLS(dp->w, pass) > 0)
      ci_error(pp, "missing row in progressive de-interlacing");
}

static void
sequential_row(standard_display *dp, ci_structp pp, ci_infop pi,
    int iImage, int iDisplay)
{
   int npasses = dp->npasses;
   int do_interlace = dp->do_interlace &&
      dp->interlace_type == CI_INTERLACE_ADAM7;
   ci_uint_32 height = standard_height(pp, dp->id);
   ci_uint_32 width = standard_width(pp, dp->id);
   const ci_store* ps = dp->ps;
   int pass;

   for (pass=0; pass<npasses; ++pass)
   {
      ci_uint_32 y;
      ci_uint_32 wPass = CI_PASS_COLS(width, pass);
      ...
      for (y=0; y<height; ++y)
      {
         if (do_interlace)
         {
             ...
             * ci_read_row must not be called in either case.
             */
            if (wPass > 0 && CI_ROW_IN_INTERLACE_PASS(y, pass))
            {
               /* Read the row into a pair of temporary buffers, then do the
                * merge here into the output rows.
                */
               ci_byte row[STANDARD_ROWMAX], display[STANDARD_ROWMAX];

               /* The following aids (to some extent) error detection - we can
                * see where ci_read_row wrote.  Use opposite values in row and
    ...
    * to find additional data in ci_info from chunks after the last IDAT.)
    */
   ci_read_end(pp, pi);
}
...
#ifdef CI_TEXT_SUPPORTED
static void
standard_check_text(ci_const_structp pp, ci_const_textp tp,
   ci_const_charp keyword, ci_const_charp text)
{
   char msg[1024];
...
static void
standard_text_validate(standard_display *dp, ci_const_structp pp,
   ci_infop pi, int check_end)
{
   ci_textp tp = NULL;
   ci_uint_32 num_text = ci_get_text(pp, pi, &tp, NULL);
   ...
   if (num_text == 2 && tp != NULL)
   {
       ...
       * ci_struct z_stream unreset at the end of the image, so subsequent
...
static void
standard_row_validate(standard_display *dp, ci_const_structp pp,
   int iImage, int iDisplay, ci_uint_32 y)
{
   int where;
   ci_byte std[STANDARD_ROWMAX];
   ...
   {
      char msg[64];
      sprintf(msg, "CI image row[%lu][%d] changed from %.2x to %.2x",
...
static void
standard_image_validate(standard_display *dp, ci_const_structp pp, int iImage,
    int iDisplay)
{
   ci_uint_32 y;
...
static void CICBAPI
standard_end(ci_structp ppIn, ci_infop pi)
{
   ci_const_structp pp = ppIn;
   standard_display *dp = voidcast(standard_display*,
      ci_get_progressive_ptr(pp));
   ...
   standard_text_validate(dp, pp, pi,
      CI_LIBCI_VER >= 10518/*check_end: see comments above*/);
   standard_image_validate(dp, pp, 0, -1);
}

/* A single test run checking the standard image to ensure it is not damaged. */
static void
standard_test(ci_store* const psIn, ci_uint_32 const id,
   int do_interlace, int use_update_info)
{
   standard_display d;
   context(psIn, fault);
   ...
   Try
   {
      ...
      /* Get a ci_struct for reading the image. This will throw an error if it
       * fails, so we don't need to check the result.
       */
      pp = set_store_for_read(d.ps, &pi, d.id,
         d.do_interlace ?  (d.ps->progressive ?
            "civalid progressive deinterlacer" :
            "civalid sequential deinterlacer") : (d.ps->progressive ?
               "progressive reader" : "sequential reader"));

      /* Initialize the palette correctly from the ci_store_file. */
      standard_palette_init(&d);

      /* Introduce the correct read function. */
      if (d.ps->progressive)
      {
         ci_set_progressive_read_fn(pp, &d, standard_info, progressive_row,
            standard_end);
      ...
      else
      {
         ...
         ci_set_read_fn(pp, d.ps, store_read);
          ...
          * unless the CI file values have been checked against the expected
          * values.
          */
         {
      ...
      if (!d.ps->validated)
         ci_error(pp, "image read failed silently");
...
static int
test_standard(ci_modifier* const pm, ci_byte const colour_type,
    int bdlo, int const bdhi)
{
   for (; bdlo <= bdhi; ++bdlo)
   {
...
static void
perform_standard_test(ci_modifier *pm)
{
...
/********************************** SIZE TESTS ********************************/
static int
test_size(ci_modifier* const pm, ci_byte const colour_type,
    int bdlo, int const bdhi)
{
    ...
    * hinc and winc stuff:
    */
   static const ci_byte hinc[] = {1, 3, 11, 1, 5};
   static const ci_byte winc[] = {1, 9, 5, 7, 1};
   int save_bdlo = bdlo;
   ...
   for (; bdlo <= bdhi; ++bdlo)
   {
      ...
      for (h=1; h<=16; h+=hinc[bdlo])
      {
         for (w=1; w<=16; w+=winc[bdlo])
         {
            ...
            * reader using libci to deinterlace (where required.)  This
            * validates the write side of libci.  There are four possibilities
            * to validate.
            */
            standard_test(&pm->this, FILEID(colour_type, DEPTH(bdlo),
               0/*palette*/, CI_INTERLACE_NONE, w, h, 0), 0/*do_interlace*/,
               pm->use_update_info);
            ...
            standard_test(&pm->this, FILEID(colour_type, DEPTH(bdlo),
               0/*palette*/, CI_INTERLACE_NONE, w, h, 1), 0/*do_interlace*/,
               pm->use_update_info);
            ...
            */
            standard_test(&pm->this, FILEID(colour_type, DEPTH(bdlo),
               0/*palette*/, CI_INTERLACE_NONE, w, h, 0), 1/*do_interlace*/,
               pm->use_update_info);
...
#        if CAN_WRITE_INTERLACE
            /* Validate the civalid code itself: */
            standard_test(&pm->this, FILEID(colour_type, DEPTH(bdlo),
               0/*palette*/, CI_INTERLACE_ADAM7, w, h, 1), 1/*do_interlace*/,
               pm->use_update_info);
...
#        endif
   ...
   /* Now do the tests of libci interlace handling, after we have made sure
    * that the civalid version works:
    */
   for (bdlo = save_bdlo; bdlo <= bdhi; ++bdlo)
   {
      ...
      for (h=1; h<=16; h+=hinc[bdlo])
      {
         ...
         {
#        ifdef CI_READ_INTERLACING_SUPPORTED
            /* Test with civalid generated interlaced images first; we have
            * already verify these are ok (unless civalid has self-consistent
            * read/write errors, which is unlikely), so this detects errors in
            * the read side first:
            */
#        if CAN_WRITE_INTERLACE
            standard_test(&pm->this, FILEID(colour_type, DEPTH(bdlo),
               0/*palette*/, CI_INTERLACE_ADAM7, w, h, 1), 0/*do_interlace*/,
               pm->use_update_info);
...
#        ifdef CI_WRITE_INTERLACING_SUPPORTED
            /* Test the libci write side against the civalid read side: */
            standard_test(&pm->this, FILEID(colour_type, DEPTH(bdlo),
               0/*palette*/, CI_INTERLACE_ADAM7, w, h, 0), 1/*do_interlace*/,
               pm->use_update_info);
...
#        ifdef CI_WRITE_INTERLACING_SUPPORTED
            /* Test both together: */
            standard_test(&pm->this, FILEID(colour_type, DEPTH(bdlo),
               0/*palette*/, CI_INTERLACE_ADAM7, w, h, 0), 0/*do_interlace*/,
               pm->use_update_info);
...
static void
perform_size_test(ci_modifier *pm)
{
   /* Test each colour type over the valid range of bit depths (expressed as
    * log2(bit_depth) in turn, stop as soon as any error is detected.
    */
...
/******************************* TRANSFORM TESTS ******************************/
#ifdef CI_READ_TRANSFORMS_SUPPORTED
/* A set of tests to validate libci image transforms.  The possibilities here
...
{
   /* A local (civalid) representation of a CI pixel, in all its
    * various forms.
    */
   unsigned int red, green, blue, alpha; /* For non-palette images. */
   unsigned int palette_index;           /* For a palette image. */
   ci_byte     colour_type;             /* As in the spec. */
   ci_byte     bit_depth;               /* Defines bit size in row */
   ci_byte     sample_depth;            /* Scale of samples */
   ...
   ci_byte red_sBIT, green_sBIT, blue_sBIT, alpha_sBIT;
} image_pixel;
 ...
 */
static void
image_pixel_init(image_pixel *this, ci_const_bytep row, ci_byte colour_type,
    ci_byte bit_depth, ci_uint_32 x, store_palette palette,
    const image_pixel *format /*from civalid transform of input*/)
{
   ci_byte sample_depth =
      (ci_byte)(colour_type == CI_COLOR_TYPE_PALETTE ? 8 : bit_depth);
...
#if defined CI_READ_EXPAND_SUPPORTED || defined CI_READ_GRAY_TO_RGB_SUPPORTED\
   || defined CI_READ_EXPAND_SUPPORTED || defined CI_READ_EXPAND_16_SUPPORTED\
...
{
   if (this->colour_type == CI_COLOR_TYPE_PALETTE)
   {
      if (this->have_tRNS)
      {
         this->colour_type = CI_COLOR_TYPE_RGB_ALPHA;
         this->have_tRNS = 0;
      }
      else
         this->colour_type = CI_COLOR_TYPE_RGB;
 ...
 * the above API).  With ci_set_background the alpha channel is never expanded
 * but this routine is used by civalid to simplify code; 'for_background'
 * records this.
 */
static void
   ...
   int for_background)
{
   ...
   if ((this->colour_type & CI_COLOR_MASK_ALPHA) == 0)
   {
      if (this->colour_type == CI_COLOR_TYPE_GRAY)
      {
...
#           endif
         ...
         this->colour_type = CI_COLOR_TYPE_GRAY_ALPHA;
      }

      else if (this->colour_type == CI_COLOR_TYPE_RGB)
      {
         if (this->have_tRNS)
         {
            this->have_tRNS = 0;
...
typedef struct image_transform
{
    ...
    *
    * First a callback to add any required modifications to the ci_modifier;
   ...
   /* And a callback to set the transform on the current ci_read_struct:
    */
   void (*set)(const struct image_transform *this,
      struct transform_display *that, ci_structp pp, ci_infop pi);

   /* Then a transform that takes an input pixel in one CI format or another
    * and modifies it by a civalid implementation of the transform (thus
    * duplicating the libci intent without, we hope, duplicating the bugs
    * in the libci implementation!)  The ci_structp is solely to allow error
    * reporting via ci_error and ci_warning.
    */
   void (*mod)(const struct image_transform *this, image_pixel *that,
      ci_const_structp pp, const struct transform_display *display);
   ...
   int (*add)(struct image_transform *this,
      const struct image_transform **that, ci_byte colour_type,
      ci_byte bit_depth);
} image_transform;
...
typedef struct transform_display
{
   ...
   /* Local variables */
   ci_byte output_colour_type;
   ci_byte output_bit_depth;
   ci_byte unpacked;
...
{
   /* Set up the ci_modifier '_current' fields then use these to determine how
    * to add appropriate chunks.
    */
   ci_modifier *pm = this->pm;
   ...
   if (modifier_color_encoding_is_set(pm))
   {
      if (modifier_color_encoding_is_sRGB(pm))
         srgb_modification_init(&this->srgb_mod, pm, CI_sRGB_INTENT_ABSOLUTE);
...
image_transform_set_end(const image_transform *this,
   transform_display *that, ci_structp pp, ci_infop pi)
{
   UNUSED(this)
   UNUSED(that)
   UNUSED(pp)
...
image_transform_mod_end(const image_transform *this, image_pixel *that,
    ci_const_structp pp, const transform_display *display)
{
    ...
    * including simple size scaling, though libci doesn't do that at present.
    */
   that->red = sample_scale(that->redf, scale);

   /* This is a bit bogus; really the above calculation should use the red_sBIT
    * value, not sample_depth, but because libci does ci_set_shift by just
    ...
    * CI specification actually permits any error within the range (-1..+1),
   ...
   if (that->colour_type & CI_COLOR_MASK_COLOR)
   {
   ...
   if ((that->colour_type & CI_COLOR_MASK_ALPHA) ||
      that->colour_type == CI_COLOR_TYPE_PALETTE)
   {
 ...
 */
static void
transform_display_init(transform_display *dp, ci_modifier *pm, ci_uint_32 id,
    const image_transform *transform_list)
{
   memset(dp, 0, sizeof *dp);
...
static void
transform_info_imp(transform_display *dp, ci_structp pp, ci_infop pi)
{
   ...
   dp->output_colour_type = ci_get_color_type(pp, pi);
   dp->output_bit_depth = ci_get_bit_depth(pp, pi);

   /* If ci_set_filler is in action then fake the output color type to include
    * an alpha channel where appropriate.
    */
   if (dp->output_bit_depth >= 8 &&
       (dp->output_colour_type == CI_COLOR_TYPE_RGB ||
        dp->output_colour_type == CI_COLOR_TYPE_GRAY) && dp->this.filler)
       dp->output_colour_type |= 4;

   /* Validate the combination of colour type and bit depth that we are getting
    * out of libci; the semantics of something not in the CI spec are, at
    * best, unclear.
    */
    ...
    */
   {
      ...
      if (test_pixel.colour_type == CI_COLOR_TYPE_PALETTE)
      ...
      if (test_pixel.colour_type == CI_COLOR_TYPE_PALETTE &&
         ...
         ci_error(pp, "civalid: internal: palette sample depth not 8");
      else if (dp->unpacked && test_pixel.bit_depth != 8)
         ci_error(pp, "civalid: internal: bad unpacked pixel depth");
      else if (!dp->unpacked && test_pixel.colour_type != CI_COLOR_TYPE_PALETTE
      ...
      {
         /* This could be a libci error too; libci has not produced what we
...
static void CICBAPI
transform_info(ci_structp pp, ci_infop pi)
{
   transform_info_imp(voidcast(transform_display*, ci_get_progressive_ptr(pp)),
      pp, pi);
}

static void
transform_range_check(ci_const_structp pp, unsigned int r, unsigned int g,
   unsigned int b, unsigned int a, unsigned int in_digitized, double in,
   unsigned int out, ci_byte sample_depth, double err, double limit,
   const char *name, double digitization_error)
{
   /* Compare the scaled, digitized, values of our local calculation (in+-err)
    * with the digitized values libci produced;  'sample_depth' is the actual
    * digitization depth of the libci output colors (the bit depth except for
    * palette images where it is always 8.)  The check on 'err' is to detect
    * internal errors in civalid itself.
    */
   unsigned int max = (1U<<sample_depth)-1;
...
static void
transform_image_validate(transform_display *dp, ci_const_structp pp,
   ci_infop pi)
{
   /* Constants for the loop below: */
   const ci_store* const ps = dp->this.ps;
   ci_byte in_ct = dp->this.colour_type;
   ci_byte in_bd = dp->this.bit_depth;
   ci_uint_32 w = dp->this.w;
   ci_uint_32 h = dp->this.h;
   ci_byte out_ct = dp->output_colour_type;
   ci_byte out_bd = dp->output_bit_depth;
   ci_byte sample_depth =
      (ci_byte)(out_ct == CI_COLOR_TYPE_PALETTE ? 8 : out_bd);
   ci_byte red_sBIT = dp->this.red_sBIT;
   ci_byte green_sBIT = dp->this.green_sBIT;
   ci_byte blue_sBIT = dp->this.blue_sBIT;
   ci_byte alpha_sBIT = dp->this.alpha_sBIT;
    ...
    */
   if (out_ct == CI_COLOR_TYPE_PALETTE)
   {
      ...
      if (npalette != dp->this.npalette)
         ci_error(pp, "unexpected change in palette size");

      digitization_error = .5;
   }
   else
   {
      ci_byte in_sample_depth;
       ...
       */
      if (in_ct == CI_COLOR_TYPE_PALETTE || in_bd < 16)
         in_sample_depth = 8;
      else
   ...
   for (y=0; y<h; ++y)
   {
      ci_const_bytep const pRow = store_image_row(ps, pp, 0, y);
      ci_uint_32 x;
      ...
      for (x=0; x<w; ++x)
      {
          ...
          * pixel that libci produces.
          */
         dp->transform_list->mod(dp->transform_list, &in_pixel, pp, dp);

         /* Read the output pixel and compare it to what we got, we don't
          * use the error field here, so no need to update sBIT.  in_pixel
          * says whether we expect libci to change the output format.
          */
          ...
          * changed.
          */
         if (in_ct == CI_COLOR_TYPE_PALETTE &&
            out_ct == CI_COLOR_TYPE_PALETTE)
         {
            if (in_pixel.palette_index != out_pixel.palette_index)
               ci_error(pp, "unexpected transformed palette index");
         }
         ...
         if ((out_ct & CI_COLOR_MASK_COLOR) != 0 &&
         ...
         if ((out_ct & CI_COLOR_MASK_ALPHA) != 0 &&
...
static void CICBAPI
transform_end(ci_structp ppIn, ci_infop pi)
{
   ci_const_structp pp = ppIn;
...
/* A single test run. */
static void
transform_test(ci_modifier *pmIn, ci_uint_32 idIn,
    const image_transform* transform_listIn, const char * const name)
{
   ...
   Try
   {
      ...
      /* Get a ci_struct for reading the image. */
...
#     endif
      ...
      if (d.pm->this.progressive)
      {
         ...
         ci_set_progressive_read_fn(pp, &d, transform_info, progressive_row,
            transform_end);
      ...
      {
         /* modifier_read expects a ci_modifier* */
         ci_set_read_fn(pp, d.pm, modifier_read);
   ...
   Catch(fault)
   {
      modifier_reset(voidcast(ci_modifier*,(void*)fault));
   }
}
...
{\
   ...
   0, /*next*/\
   image_transform_ini,\
   image_transform_ci_set_##name##_set,\
   image_transform_ci_set_##name##_mod,\
   image_transform_ci_set_##name##_add\
}
...
image_transform_default_add(image_transform *this,
    const image_transform **that, ci_byte colour_type, ci_byte bit_depth)
{
   UNUSED(colour_type)
   UNUSED(bit_depth)
...
/* ci_set_palette_to_rgb */
static void
image_transform_ci_set_palette_to_rgb_set(const image_transform *this,
    transform_display *that, ci_structp pp, ci_infop pi)
{
   ci_set_palette_to_rgb(pp);
   this->next->set(this->next, that, pp, pi);
}

static void
image_transform_ci_set_palette_to_rgb_mod(const image_transform *this,
    image_pixel *that, ci_const_structp pp,
    const transform_display *display)
{
   if (that->colour_type == CI_COLOR_TYPE_PALETTE)
...
static int
image_transform_ci_set_palette_to_rgb_add(image_transform *this,
    const image_transform **that, ci_byte colour_type, ci_byte bit_depth)
{
   UNUSED(bit_depth)
   ...
   *that = this;
   ...
   return colour_type == CI_COLOR_TYPE_PALETTE;
}

IT(palette_to_rgb);
#undef PT
#define PT ITSTRUCT(palette_to_rgb)
#endif /* CI_READ_EXPAND_SUPPORTED */

#ifdef CI_READ_EXPAND_SUPPORTED
/* ci_set_tRNS_to_alpha */
static void
image_transform_ci_set_tRNS_to_alpha_set(const image_transform *this,
   transform_display *that, ci_structp pp, ci_infop pi)
{
   ci_set_tRNS_to_alpha(pp);
...
static void
image_transform_ci_set_tRNS_to_alpha_mod(const image_transform *this,
   image_pixel *that, ci_const_structp pp,
   const transform_display *display)
{
#if CI_LIBCI_VER != 10700
   /* LIBCI BUG: this always forces palette images to RGB. */
   if (that->colour_type == CI_COLOR_TYPE_PALETTE)
      image_pixel_convert_PLTE(that);
#endif
...
#     if CI_LIBCI_VER == 10700
         if (that->colour_type != CI_COLOR_TYPE_PALETTE &&
             (that->colour_type & CI_COLOR_MASK_ALPHA) == 0)
#     endif
      image_pixel_add_alpha(that, &display->this, 0/*!for background*/);

#if CI_LIBCI_VER != 10700
   /* LIBCI BUG: otherwise libci still expands to 8 bits! */
   else
   {
      if (that->bit_depth < 8)
         that->bit_depth =8;
...
static int
image_transform_ci_set_tRNS_to_alpha_add(image_transform *this,
    const image_transform **that, ci_byte colour_type, ci_byte bit_depth)
{
   UNUSED(bit_depth)
   ...
   *that = this;
   ...
   return
#  if CI_LIBCI_VER == 10700
      colour_type != CI_COLOR_TYPE_PALETTE &&
#  endif
   (colour_type & CI_COLOR_MASK_ALPHA) == 0;
}
...
/* ci_set_gray_to_rgb */
static void
image_transform_ci_set_gray_to_rgb_set(const image_transform *this,
    transform_display *that, ci_structp pp, ci_infop pi)
{
   ci_set_gray_to_rgb(pp);
   /* NOTE: this doesn't result in tRNS expansion. */
   this->next->set(this->next, that, pp, pi);
}

static void
image_transform_ci_set_gray_to_rgb_mod(const image_transform *this,
    image_pixel *that, ci_const_structp pp,
    const transform_display *display)
{
   /* NOTE: we can actually pend the tRNS processing at this point because we
    * can correctly recognize the original pixel value even though we have
    * mapped the one gray channel to the three RGB ones, but in fact libci
    * doesn't do this, so we don't either.
    */
   if ((that->colour_type & CI_COLOR_MASK_COLOR) == 0 && that->have_tRNS)
      image_pixel_add_alpha(that, &display->this, 0/*!for background*/);

   /* Simply expand the bit depth and alter the colour type as required. */
   if (that->colour_type == CI_COLOR_TYPE_GRAY)
   {
       ...
       */
      that->colour_type = CI_COLOR_TYPE_RGB;
   }

   else if (that->colour_type == CI_COLOR_TYPE_GRAY_ALPHA)
      that->colour_type = CI_COLOR_TYPE_RGB_ALPHA;

   this->next->mod(this->next, that, pp, display);
}

static int
image_transform_ci_set_gray_to_rgb_add(image_transform *this,
    const image_transform **that, ci_byte colour_type, ci_byte bit_depth)
{
   UNUSED(bit_depth)
   ...
   *that = this;
   ...
   return (colour_type & CI_COLOR_MASK_COLOR) == 0;
}

IT(gray_to_rgb);
#undef PT
#define PT ITSTRUCT(gray_to_rgb)
#endif /* CI_READ_GRAY_TO_RGB_SUPPORTED */

#ifdef CI_READ_EXPAND_SUPPORTED
/* ci_set_expand */
static void
image_transform_ci_set_expand_set(const image_transform *this,
    transform_display *that, ci_structp pp, ci_infop pi)
{
   ci_set_expand(pp);
...
static void
image_transform_ci_set_expand_mod(const image_transform *this,
    image_pixel *that, ci_const_structp pp,
    const transform_display *display)
{
...
static int
image_transform_ci_set_expand_add(image_transform *this,
    const image_transform **that, ci_byte colour_type, ci_byte bit_depth)
{
   UNUSED(bit_depth)
   ...
   *that = this;
    ...
    */
   return (colour_type & CI_COLOR_MASK_ALPHA) == 0;
}
...
#ifdef CI_READ_EXPAND_SUPPORTED
/* ci_set_expand_gray_1_2_4_to_8
 * Pre 1.7.0 LIBCI BUG: this just does an 'expand'
 */
static void
image_transform_ci_set_expand_gray_1_2_4_to_8_set(
    const image_transform *this, transform_display *that, ci_structp pp,
    ci_infop pi)
{
   ci_set_expand_gray_1_2_4_to_8(pp);
   /* NOTE: don't expect this to expand tRNS */
   this->next->set(this->next, that, pp, pi);
}

static void
image_transform_ci_set_expand_gray_1_2_4_to_8_mod(
    const image_transform *this, image_pixel *that, ci_const_structp pp,
    const transform_display *display)
{
#if CI_LIBCI_VER != 10700
   image_transform_ci_set_expand_mod(this, that, pp, display);
#else
...
static int
image_transform_ci_set_expand_gray_1_2_4_to_8_add(image_transform *this,
    const image_transform **that, ci_byte colour_type, ci_byte bit_depth)
{
#if CI_LIBCI_VER != 10700
   return image_transform_ci_set_expand_add(this, that, colour_type,
      bit_depth);
#else
   UNUSED(bit_depth)
   ...
   *that = this;
    ...
    * less than 8:
    */
   return colour_type == CI_COLOR_TYPE_GRAY && bit_depth < 8;
#endif /* 1.7 or later */
}
...
/* ci_set_expand_16 */
static void
image_transform_ci_set_expand_16_set(const image_transform *this,
    transform_display *that, ci_structp pp, ci_infop pi)
{
   ci_set_expand_16(pp);

   /* NOTE: prior to 1.7 libci does SET_EXPAND as well, so tRNS is expanded. */
#  if CI_LIBCI_VER != 10700
      if (that->this.has_tRNS)
         that->this.is_transparent = 1;
#  endif
...
static void
image_transform_ci_set_expand_16_mod(const image_transform *this,
    image_pixel *that, ci_const_structp pp,
    const transform_display *display)
{
...
static int
image_transform_ci_set_expand_16_add(image_transform *this,
    const image_transform **that, ci_byte colour_type, ci_byte bit_depth)
{
   UNUSED(colour_type)
...
#endif /* CI_READ_EXPAND_16_SUPPORTED */

#ifdef CI_READ_SCALE_16_TO_8_SUPPORTED  /* API added in 1.5.4 */
/* ci_set_scale_16 */
static void
image_transform_ci_set_scale_16_set(const image_transform *this,
    transform_display *that, ci_structp pp, ci_infop pi)
{
   ci_set_scale_16(pp);
#  if CI_LIBCI_VER != 10700
      /* libci will limit the gamma table size: */
      that->max_gamma_8 = CI_MAX_GAMMA_8;
#  endif
   this->next->set(this->next, that, pp, pi);
}

static void
image_transform_ci_set_scale_16_mod(const image_transform *this,
    image_pixel *that, ci_const_structp pp,
    const transform_display *display)
{
   if (that->bit_depth == 16)
...
static int
image_transform_ci_set_scale_16_add(image_transform *this,
    const image_transform **that, ci_byte colour_type, ci_byte bit_depth)
{
   UNUSED(colour_type)
...
#endif /* CI_READ_SCALE_16_TO_8_SUPPORTED (1.5.4 on) */

#ifdef CI_READ_16_TO_8_SUPPORTED /* the default before 1.5.4 */
/* ci_set_strip_16 */
static void
image_transform_ci_set_strip_16_set(const image_transform *this,
    transform_display *that, ci_structp pp, ci_infop pi)
{
   ci_set_strip_16(pp);
#  if CI_LIBCI_VER != 10700
...
static void
image_transform_ci_set_strip_16_mod(const image_transform *this,
    image_pixel *that, ci_const_structp pp,
    const transform_display *display)
{
   if (that->bit_depth == 16)
   {
      ...
      /* Prior to 1.5.4 ci_set_strip_16 would use an 'accurate' method if this
       * configuration option is set.  From 1.5.4 the flag is never set and the
       * 'scale' API (above) must be used.
       */
#     ifdef CI_READ_ACCURATE_SCALE_SUPPORTED
#        if CI_LIBCI_VER >= 10504
#           error CI_READ_ACCURATE_SCALE should not be set
#        endif

         /* The strip 16 algorithm drops the low 8 bits rather than calculating
          * 1/257, so we need to adjust the permitted errors appropriately:
          * Notice that this is only relevant prior to the addition of the
          * ci_set_scale_16 API in 1.5.4 (but 1.5.4+ always defines the above!)
          */
         {
...
static int
image_transform_ci_set_strip_16_add(image_transform *this,
    const image_transform **that, ci_byte colour_type, ci_byte bit_depth)
{
   UNUSED(colour_type)
...
#endif /* CI_READ_16_TO_8_SUPPORTED */

#ifdef CI_READ_STRIP_ALPHA_SUPPORTED
/* ci_set_strip_alpha */
static void
image_transform_ci_set_strip_alpha_set(const image_transform *this,
    transform_display *that, ci_structp pp, ci_infop pi)
{
   ci_set_strip_alpha(pp);
   this->next->set(this->next, that, pp, pi);
}

static void
image_transform_ci_set_strip_alpha_mod(const image_transform *this,
    image_pixel *that, ci_const_structp pp,
    const transform_display *display)
{
   if (that->colour_type == CI_COLOR_TYPE_GRAY_ALPHA)
      that->colour_type = CI_COLOR_TYPE_GRAY;
   else if (that->colour_type == CI_COLOR_TYPE_RGB_ALPHA)
      that->colour_type = CI_COLOR_TYPE_RGB;
...
static int
image_transform_ci_set_strip_alpha_add(image_transform *this,
    const image_transform **that, ci_byte colour_type, ci_byte bit_depth)
{
   UNUSED(bit_depth)
...
#endif /* CI_READ_STRIP_ALPHA_SUPPORTED */

#ifdef CI_READ_RGB_TO_GRAY_SUPPORTED
/* ci_set_rgb_to_gray(ci_structp, int err_action, double red, double green)
 * ci_set_rgb_to_gray_fixed(ci_structp, int err_action, ci_fixed_point red,
 *    ci_fixed_point green)
 * ci_get_rgb_to_gray_status
 *
 * The 'default' test here uses values known to be used inside libci prior to
 * 1.7.0:
 *
 *   red:    6968
 *   green: 23434
 ...
 * versions of libci will fail the accuracy tests below because they use the
...
static struct
{
   ...
   /* The following are the parameters for ci_set_rgb_to_gray: */
#  ifdef CI_FLOATING_POINT_SUPPORTED
      double red_to_set;
      double green_to_set;
#  else
      ci_fixed_point red_to_set;
      ci_fixed_point green_to_set;
#  endif
...
#define image_transform_ini image_transform_ci_set_rgb_to_gray_ini
static void
image_transform_ci_set_rgb_to_gray_ini(const image_transform *this,
    transform_display *that)
{
   ci_modifier *pm = that->pm;
...
#     endif
   ...
   /* The arguments to ci_set_rgb_to_gray can override the coefficients implied
...
#     endif
   ...
   /* Adjust the error limit in the ci_modifier because of the larger errors
   ...
   if (data.gamma != 1) /* Use gamma tables */
   {
      if (that->this.bit_depth == 16 || pm->assume_16_bit_calculations)
      {
          ...
          *  +/-(1<<(15-CI_MAX_GAMMA_8)) in the 8-bit case.
          */
#        if CI_LIBCI_VER != 10700
...
#        endif
      ...
      else
      {
          ...
          *
          * When DIGITIZE is set because a pre-1.7 version of libci is being
          * tested allow a bigger slack.
          *
          * NOTE: this number only affects the internal limit check in civalid,
          * it has no effect on the limits applied to the libci values.
          */
#if DIGITIZE
...
static void
image_transform_ci_set_rgb_to_gray_set(const image_transform *this,
    transform_display *that, ci_structp pp, ci_infop pi)
{
   int error_action = 1; /* no error, no defines in ci.h */

#  ifdef CI_FLOATING_POINT_SUPPORTED
      ci_set_rgb_to_gray(pp, error_action, data.red_to_set, data.green_to_set);
#  else
      ci_set_rgb_to_gray_fixed(pp, error_action, data.red_to_set,
         data.green_to_set);
#  endif
...
#  ifdef CI_READ_cHRM_SUPPORTED
      if (that->pm->current_encoding != 0)
      {
          ...
          * check that the libci APIs give the correct (X,Y,Z) values within
          * some margin of error for the round trip through the chromaticity
          * form.
          */
#        ifdef CI_FLOATING_POINT_SUPPORTED
#           define API_function ci_get_cHRM_XYZ
#           define API_form "FP"
#           define API_type double
#           define API_cvt(x) (x)
#        else
#           define API_function ci_get_cHRM_XYZ_fixed
#           define API_form "fixed"
#           define API_type ci_fixed_point
#           define API_cvt(x) ((double)(x)/CI_FP_1)
#        endif
               ...
               & CI_INFO_cHRM) != 0)
         {
            ...
            /* Expect libci to return a normalized result, but the original
             * color space encoding may not be normalized.
             */
            modifier_current_encoding(that->pm, &o);
            normalize_color_encoding(&o);

            /* Sanity check the civalid code - the coefficients should match
            ...
            if (data.red_to_set == -1 && data.green_to_set == -1 &&
               ...
               ci_error(pp, "internal civalid cHRM coefficient error");
            ...
            /* This should match the original one from the ci_modifier, within
             * the range permitted by the libci fixed point representation.
             */
            maxe = 0;
...
#           define CHECK(col,x)\
             ...
             * from the cHRm chunk.  CI uses fixed point in the cHRM chunk, so
...
static void
image_transform_ci_set_rgb_to_gray_mod(const image_transform *this,
    image_pixel *that, ci_const_structp pp,
    const transform_display *display)
{
   if ((that->colour_type & CI_COLOR_MASK_COLOR) != 0)
   {
      double gray, err;
...
#  if DIGITIZE
      {
          ...
          * transform, feel free to delete the ci_error checks on this below in
...
#        undef DU
          ...
          *
          * If r==g==b because there is no overall gamma correction libci
...
#        endif /* before 1.7 */
         if (data.gamma == 1)
         {
             ...
             *
             * TODO: consider fixing this in libci by rounding the calculation
...
#  endif /* !DIGITIZE */
      ...
      if (that->colour_type == CI_COLOR_TYPE_RGB)
         that->colour_type = CI_COLOR_TYPE_GRAY;
      else if (that->colour_type == CI_COLOR_TYPE_RGB_ALPHA)
         that->colour_type = CI_COLOR_TYPE_GRAY_ALPHA;
   }
...
static int
image_transform_ci_set_rgb_to_gray_add(image_transform *this,
    const image_transform **that, ci_byte colour_type, ci_byte bit_depth)
{
   UNUSED(bit_depth)
...
#undef image_transform_ini
#define image_transform_ini image_transform_default_ini
#endif /* CI_READ_RGB_TO_GRAY_SUPPORTED */

#ifdef CI_READ_BACKGROUND_SUPPORTED
/* ci_set_background(ci_structp, ci_const_color_16p background_color,
 *    int background_gamma_code, int need_expand, double background_gamma)
 * ci_set_background_fixed(ci_structp, ci_const_color_16p background_color,
...
static void
image_transform_ci_set_background_set(const image_transform *this,
    transform_display *that, ci_structp pp, ci_infop pi)
{
   ci_byte colour_type, bit_depth;
   ci_byte random_bytes[8]; /* 8 bytes - 64 bits - the biggest pixel */
   int expand;
   ci_color_16 back;
   ...
   if (colour_type & CI_COLOR_MASK_COLOR)
   {
      back.red = (ci_uint_16)data.red;
      back.green = (ci_uint_16)data.green;
      back.blue = (ci_uint_16)data.blue;
   }

   else
      back.gray = (ci_uint_16)data.red;

#ifdef CI_FLOATING_POINT_SUPPORTED
   ci_set_background(pp, &back, CI_BACKGROUND_GAMMA_FILE, expand, 0);
#else
   ci_set_background_fixed(pp, &back, CI_BACKGROUND_GAMMA_FILE, expand, 0);
#endif
...
static void
image_transform_ci_set_background_mod(const image_transform *this,
    image_pixel *that, ci_const_structp pp,
    const transform_display *display)
{
   /* Check for tRNS first: */
   if (that->have_tRNS && that->colour_type != CI_COLOR_TYPE_PALETTE)
   ...
   if (that->colour_type == CI_COLOR_TYPE_RGB_ALPHA)
      that->colour_type = CI_COLOR_TYPE_RGB;
   else if (that->colour_type == CI_COLOR_TYPE_GRAY_ALPHA)
      that->colour_type = CI_COLOR_TYPE_GRAY;
   /* CI_COLOR_TYPE_PALETTE is not changed */

   this->next->mod(this->next, that, pp, display);
}

#define image_transform_ci_set_background_add image_transform_default_add

#undef data
IT(background);
#undef PT
#define PT ITSTRUCT(background)
#endif /* CI_READ_BACKGROUND_SUPPORTED */

/* ci_set_quantize(ci_structp, ci_colorp palette, int num_palette,
 *    int maximum_colors, ci_const_uint_16p histogram, int full_quantize)
 *
 ...
 *
 *  ci_set_invert_alpha(ci_structrp ci_ptr)
 */
static void
image_transform_ci_set_invert_alpha_set(const image_transform *this,
    transform_display *that, ci_structp pp, ci_infop pi)
{
   ci_set_invert_alpha(pp);
   this->next->set(this->next, that, pp, pi);
}

static void
image_transform_ci_set_invert_alpha_mod(const image_transform *this,
    image_pixel *that, ci_const_structp pp,
    const transform_display *display)
{
...
static int
image_transform_ci_set_invert_alpha_add(image_transform *this,
    const image_transform **that, ci_byte colour_type, ci_byte bit_depth)
{
   UNUSED(bit_depth)
...
#endif /* CI_READ_INVERT_ALPHA_SUPPORTED */

/* ci_set_bgr */
#ifdef CI_READ_BGR_SUPPORTED
/* Swap R,G,B channels to order B,G,R.
 *
 *  ci_set_bgr(ci_structrp ci_ptr)
 *
 * This only has an effect on RGB and RGBA pixels.
 */
static void
image_transform_ci_set_bgr_set(const image_transform *this,
    transform_display *that, ci_structp pp, ci_infop pi)
{
   ci_set_bgr(pp);
   this->next->set(this->next, that, pp, pi);
...
static void
image_transform_ci_set_bgr_mod(const image_transform *this,
    image_pixel *that, ci_const_structp pp,
    const transform_display *display)
{
   if (that->colour_type == CI_COLOR_TYPE_RGB ||
       that->colour_type == CI_COLOR_TYPE_RGBA)
       that->swap_rgb = 1;
...
static int
image_transform_ci_set_bgr_add(image_transform *this,
    const image_transform **that, ci_byte colour_type, ci_byte bit_depth)
{
   UNUSED(bit_depth)
   ...
   *that = this;
   ...
   return colour_type == CI_COLOR_TYPE_RGB ||
       colour_type == CI_COLOR_TYPE_RGBA;
}
...
#endif /* CI_READ_BGR_SUPPORTED */

/* ci_set_swap_alpha */
#ifdef CI_READ_SWAP_ALPHA_SUPPORTED
/* Put the alpha channel first.
 *
 *  ci_set_swap_alpha(ci_structrp ci_ptr)
 *
 * This only has an effect on GA and RGBA pixels.
 */
static void
image_transform_ci_set_swap_alpha_set(const image_transform *this,
    transform_display *that, ci_structp pp, ci_infop pi)
{
   ci_set_swap_alpha(pp);
   this->next->set(this->next, that, pp, pi);
}

static void
image_transform_ci_set_swap_alpha_mod(const image_transform *this,
    image_pixel *that, ci_const_structp pp,
    const transform_display *display)
{
   if (that->colour_type == CI_COLOR_TYPE_GA ||
       that->colour_type == CI_COLOR_TYPE_RGBA)
      that->alpha_first = 1;
...
static int
image_transform_ci_set_swap_alpha_add(image_transform *this,
    const image_transform **that, ci_byte colour_type, ci_byte bit_depth)
{
   UNUSED(bit_depth)
   ...
   *that = this;
   ...
   return colour_type == CI_COLOR_TYPE_GA ||
       colour_type == CI_COLOR_TYPE_RGBA;
}
...
#endif /* CI_READ_SWAP_ALPHA_SUPPORTED */

/* ci_set_swap */
#ifdef CI_READ_SWAP_SUPPORTED
/* Byte swap 16-bit components.
 *
 *  ci_set_swap(ci_structrp ci_ptr)
 */
static void
image_transform_ci_set_swap_set(const image_transform *this,
    transform_display *that, ci_structp pp, ci_infop pi)
{
   ci_set_swap(pp);
   this->next->set(this->next, that, pp, pi);
...
static void
image_transform_ci_set_swap_mod(const image_transform *this,
    image_pixel *that, ci_const_structp pp,
    const transform_display *display)
{
   if (that->bit_depth == 16)
...
static int
image_transform_ci_set_swap_add(image_transform *this,
    const image_transform **that, ci_byte colour_type, ci_byte bit_depth)
{
   UNUSED(colour_type)
...
#endif /* CI_READ_SWAP_SUPPORTED */

#ifdef CI_READ_FILLER_SUPPORTED
/* Add a filler byte to 8-bit Gray or 24-bit RGB images.
 *
 *  ci_set_filler, (ci_structp ci_ptr, ci_uint_32 filler, int flags));
 *
 * Flags:
 *
 *  CI_FILLER_BEFORE
...
static void
image_transform_ci_set_filler_set(const image_transform *this,
    transform_display *that, ci_structp pp, ci_infop pi)
{
   ...
   ci_set_filler(pp, data.filler, data.flags);
...
static void
image_transform_ci_set_filler_mod(const image_transform *this,
    image_pixel *that, ci_const_structp pp,
    const transform_display *display)
{
   if (that->bit_depth >= 8 &&
        ...
        that->colour_type == CI_COLOR_TYPE_GRAY))
   {
       ...
       * type is faked to have an alpha channel, but libci won't report
      ...
      that->alpha_first = data.flags == CI_FILLER_BEFORE;
   }
...
static int
image_transform_ci_set_filler_add(image_transform *this,
    const image_transform **that, ci_byte colour_type, ci_byte bit_depth)
{
   this->next = *that;
   *that = this;

   return bit_depth >= 8 && (colour_type == CI_COLOR_TYPE_RGB ||
           colour_type == CI_COLOR_TYPE_GRAY);
}
...
/* ci_set_add_alpha, (ci_structp ci_ptr, ci_uint_32 filler, int flags)); */
...
static void
image_transform_ci_set_add_alpha_set(const image_transform *this,
    transform_display *that, ci_structp pp, ci_infop pi)
{
   /* Need a random choice for 'before' and 'after' as well as for the
    * filler.  The 'filler' value has all 32 bits set, but only bit_depth
   ...
   ci_set_add_alpha(pp, data.filler, data.flags);
   this->next->set(this->next, that, pp, pi);
}

static void
image_transform_ci_set_add_alpha_mod(const image_transform *this,
    image_pixel *that, ci_const_structp pp,
    const transform_display *display)
{
   if (that->bit_depth >= 8 &&
...
static int
image_transform_ci_set_add_alpha_add(image_transform *this,
    const image_transform **that, ci_byte colour_type, ci_byte bit_depth)
{
   this->next = *that;
   *that = this;
...
#endif /* CI_READ_FILLER_SUPPORTED */

/* ci_set_packing */
#ifdef CI_READ_PACK_SUPPORTED
/* Use 1 byte per pixel in 1, 2, or 4-bit depth files.
 *
 *  ci_set_packing(ci_structrp ci_ptr)
 *
 * This should only affect grayscale and palette images with less than 8 bits
 * per pixel.
 */
static void
image_transform_ci_set_packing_set(const image_transform *this,
    transform_display *that, ci_structp pp, ci_infop pi)
{
   ci_set_packing(pp);
   that->unpacked = 1;
...
static void
image_transform_ci_set_packing_mod(const image_transform *this,
    image_pixel *that, ci_const_structp pp,
    const transform_display *display)
{
   /* The general expand case depends on what the colour type is,
...
static int
image_transform_ci_set_packing_add(image_transform *this,
    const image_transform **that, ci_byte colour_type, ci_byte bit_depth)
{
   UNUSED(colour_type)
 ...
 *
 *  ci_set_packswap(ci_structrp ci_ptr)
 */
static void
image_transform_ci_set_packswap_set(const image_transform *this,
    transform_display *that, ci_structp pp, ci_infop pi)
{
   ci_set_packswap(pp);
...
static void
image_transform_ci_set_packswap_mod(const image_transform *this,
    image_pixel *that, ci_const_structp pp,
    const transform_display *display)
{
   if (that->bit_depth < 8)
...
static int
image_transform_ci_set_packswap_add(image_transform *this,
    const image_transform **that, ci_byte colour_type, ci_byte bit_depth)
{
   UNUSED(colour_type)
...
#endif /* CI_READ_PACKSWAP_SUPPORTED */


/* ci_set_invert_mono */
#ifdef CI_READ_INVERT_MONO_SUPPORTED
/* Invert the gray channel
 *
 *  ci_set_invert_mono(ci_structrp ci_ptr)
 */
static void
image_transform_ci_set_invert_mono_set(const image_transform *this,
    transform_display *that, ci_structp pp, ci_infop pi)
{
   ci_set_invert_mono(pp);
   this->next->set(this->next, that, pp, pi);
}

static void
image_transform_ci_set_invert_mono_mod(const image_transform *this,
    image_pixel *that, ci_const_structp pp,
    const transform_display *display)
{
   if (that->colour_type & 4)
...
static int
image_transform_ci_set_invert_mono_add(image_transform *this,
    const image_transform **that, ci_byte colour_type, ci_byte bit_depth)
{
   UNUSED(bit_depth)
...
#endif /* CI_READ_INVERT_MONO_SUPPORTED */

#ifdef CI_READ_SHIFT_SUPPORTED
/* ci_set_shift(ci_structp, ci_const_color_8p true_bits)
 *
 * The output pixels will be shifted by the given true_bits
 * values.
 */
#define data ITDATA(shift)
static ci_color_8 data;

static void
image_transform_ci_set_shift_set(const image_transform *this,
    transform_display *that, ci_structp pp, ci_infop pi)
{
    ...
    * libci does use the correct field.
    */
   unsigned int depth = that->this.bit_depth;

   data.red = (ci_byte)/*SAFE*/(random_mod(depth)+1);
   data.green = (ci_byte)/*SAFE*/(random_mod(depth)+1);
   data.blue = (ci_byte)/*SAFE*/(random_mod(depth)+1);
   data.gray = (ci_byte)/*SAFE*/(random_mod(depth)+1);
   data.alpha = (ci_byte)/*SAFE*/(random_mod(depth)+1);

   ci_set_shift(pp, &data);
   this->next->set(this->next, that, pp, pi);
}

static void
image_transform_ci_set_shift_mod(const image_transform *this,
    image_pixel *that, ci_const_structp pp,
    const transform_display *display)
{
   /* Copy the correct values into the sBIT fields, libci does not do
   ...
   if (that->colour_type != CI_COLOR_TYPE_PALETTE)
   {
        ...
        * ci_set_shift according to the colour type.
        * does.
        */
...
static int
image_transform_ci_set_shift_add(image_transform *this,
    const image_transform **that, ci_byte colour_type, ci_byte bit_depth)
{
   UNUSED(bit_depth)
...
#endif /* CI_READ_SHIFT_SUPPORTED */

#ifdef THIS_IS_THE_PROFORMA
static void
image_transform_ci_set_@_set(const image_transform *this,
    transform_display *that, ci_structp pp, ci_infop pi)
{
   ci_set_@(pp);
   this->next->set(this->next, that, pp, pi);
...
static void
image_transform_ci_set_@_mod(const image_transform *this,
    image_pixel *that, ci_const_structp pp,
    const transform_display *display)
{
   this->next->mod(this->next, that, pp, display);
...
static int
image_transform_ci_set_@_add(image_transform *this,
    const image_transform **that, ci_byte colour_type, ci_byte bit_depth)
{
   this->next = *that;
   *that = this;
...
transform_enable(const char *name)
{
   ...
   if (!found_it)
   {
      fprintf(stderr, "civalid: --transform-enable=%s: unknown transform\n",
         name);
      exit(99);
   }
}
...
transform_disable(const char *name)
{
   ...
   fprintf(stderr, "civalid: --transform-disable=%s: unknown transform\n",
      name);
   exit(99);
}
...
static int
image_transform_test_counter(ci_uint_32 counter, unsigned int max)
{
...
image_transform_add(const image_transform **this, unsigned int max,
   ci_uint_32 counter, char *name, size_t sizeof_name, size_t *pos,
   ci_byte colour_type, ci_byte bit_depth)
{
...
static void
perform_transform_test(ci_modifier *pm)
{
   ci_byte colour_type = 0;
   ci_byte bit_depth = 0;
   unsigned int palette_number = 0;
...
static void
gamma_display_init(gamma_display *dp, ci_modifier *pm, ci_uint_32 id,
    double file_gamma, double screen_gamma, ci_byte sbit, int threshold_test,
    int use_input_precision, int scale16, int expand16,
    int do_background, const ci_color_16 *pointer_to_the_background_color,
    double background_gamma)
{
   /* Standard fields */
   standard_display_init(&dp->this, &pm->this, id, do_read_interlace,
...
static void
gamma_info_imp(gamma_display *dp, ci_structp pp, ci_infop pi)
{
   /* Reuse the standard stuff as appropriate. */
   standard_info_part1(&dp->this, pp, pi);
    ...
    * are interactions with sBIT but, internally, libci makes sbit at most
    * CI_MAX_GAMMA_8 prior to 1.7 when doing the following.
    */
   if (dp->scale16)
...
#        ifdef CI_READ_16_TO_8_SUPPORTED
            ci_set_strip_16(pp);
#        else
            ci_error(pp, "scale16 (16 to 8 bit conversion) not supported");
#        endif
#     endif
...
#     ifdef CI_READ_EXPAND_16_SUPPORTED
         ci_set_expand_16(pp);
#     else
         ci_error(pp, "expand16 (8 to 16 bit conversion) not supported");
#     endif
...
#     ifdef CI_READ_ALPHA_MODE_SUPPORTED
      {
          ...
          * non-inverted, representation.  It provides a default for the CI file
          * gamma, but since the file has a gAMA chunk this does not matter.
          */
         const double sg = dp->screen_gamma;
#        ifndef CI_FLOATING_POINT_SUPPORTED
            ci_fixed_point g = fix(sg);
#        endif

#        ifdef CI_FLOATING_POINT_SUPPORTED
            ci_set_alpha_mode(pp, mode, sg);
#        else
            ci_set_alpha_mode_fixed(pp, mode, g);
#        endif
          ...
          * corrected here.
          */
         if (mode == CI_ALPHA_STANDARD && sg != 1)
         {
#           ifdef CI_FLOATING_POINT_SUPPORTED
               ci_set_gamma(pp, sg, dp->file_gamma);
#           else
               ci_fixed_point f = fix(dp->file_gamma);
               ci_set_gamma_fixed(pp, g, f);
#           endif
         }
      }
#     else
         ci_error(pp, "alpha mode handling not supported");
#     endif
   }
...
#     ifdef CI_FLOATING_POINT_SUPPORTED
         ci_set_gamma(pp, dp->screen_gamma, dp->file_gamma);
#     else
      {
         ci_fixed_point s = fix(dp->screen_gamma);
         ci_fixed_point f = fix(dp->file_gamma);
         ci_set_gamma_fixed(pp, s, f);
      }
#     endif
...
#        ifndef CI_FLOATING_POINT_SUPPORTED
            ci_fixed_point g = fix(bg);
#        endif

#        ifdef CI_FLOATING_POINT_SUPPORTED
            ci_set_background(pp, &dp->background_color, dp->do_background,
               0/*need_expand*/, bg);
#        else
            ci_set_background_fixed(pp, &dp->background_color,
               dp->do_background, 0/*need_expand*/, g);
#        endif
#     else
         ci_error(pp, "ci_set_background not supported");
#     endif
      }
   }
...
static void CICBAPI
gamma_info(ci_structp pp, ci_infop pi)
{
   gamma_info_imp(voidcast(gamma_display*, ci_get_progressive_ptr(pp)), pp,
      pi);
}

/* Validate a single component value - the routine gets the input and output
 * sample values as unscaled CI component values along with a cache of all the
...
static void
init_validate_info(validate_info *vi, gamma_display *dp, ci_const_structp pp,
    int in_depth, int out_depth)
{
   ...
   {
      vi->sbit = (ci_byte)in_depth;
      vi->isbit_shift = 0;
   }

   vi->sbit_max = (1U << vi->sbit)-1;

   /* This mimics the libci threshold test, '0' is used to prevent gamma
    * correction in the validation test.
    */
   vi->screen_gamma = dp->screen_gamma;
   if (fabs(vi->screen_gamma-1) < CI_GAMMA_THRESHOLD)
   ...
   if ((dp->this.colour_type & CI_COLOR_MASK_ALPHA) != 0 ||
...
#     if 0
         /* libci doesn't do this optimization, if we do civalid will fail.
          */
         if (fabs(bg_inverse-1) >= CI_GAMMA_THRESHOLD)
#     endif
         {
            r = pow(r, bg_inverse);
            g = pow(g, bg_inverse);
   ...
   if (fabs(vi->gamma_correction-1) < CI_GAMMA_THRESHOLD)
      vi->gamma_correction = 0;

   vi->file_inverse = 1/dp->file_gamma;
   if (fabs(vi->file_inverse-1) < CI_GAMMA_THRESHOLD)
      vi->file_inverse = 0;
   ...
   double background, int *compose)
{
   switch (do_background)
   {
      ...
      case CI_BACKGROUND_GAMMA_UNIQUE:
         /* Standard CI background processing. */
         if (alpha < 1)
         {
            if (alpha > 0)
            {
...
#ifdef CI_READ_ALPHA_MODE_SUPPORTED
      case ALPHA_MODE_OFFSET + CI_ALPHA_STANDARD:
      case ALPHA_MODE_OFFSET + CI_ALPHA_BROKEN:
          ...
          */
      case ALPHA_MODE_OFFSET + CI_ALPHA_OPTIMIZED:
    ...
    const double background /* component background value */)
{
...
#ifdef CI_READ_ALPHA_MODE_SUPPORTED
      || do_background == ALPHA_MODE_OFFSET + CI_ALPHA_CI
#endif
      || (alpha < 0 /* alpha channel */
#ifdef CI_READ_ALPHA_MODE_SUPPORTED
      && do_background != ALPHA_MODE_OFFSET + CI_ALPHA_BROKEN
#endif
      ))
   {
    ...
    */
   {
      ...
      ci_const_charp pass = 0;  /* Reason test passes (or 0 for fail) */
      ...
      {
#ifdef CI_READ_ALPHA_MODE_SUPPORTED
         if (do_background != ALPHA_MODE_OFFSET + CI_ALPHA_BROKEN)
#endif
         {
      ...
      {
         if (do_background == ALPHA_MODE_OFFSET + CI_ALPHA_OPTIMIZED &&
...

      {
         ...
         {
            /* Ok, something is wrong - this actually happens in current libci
             * 16-to-8 processing.  Assume that the input value (id, adjusted
             * for sbit) can be anywhere between value-.5 and value+.5 - quite a
             * large range if sbit is low.
             *
             * NOTE: at present because the libci gamma table stuff has been
...
#           endif
             ...
             * value.
             *
             * This is only done for older libci versions when the 'inaccurate'
             * (chop) method of scaling was used.
             */
#           ifndef CI_READ_16_TO_8_ACCURATE_SCALE_SUPPORTED
#              if CI_LIBCI_VER < 10504
...
#           endif
            ...
            if (compose != do_compose)
               ci_error(vi->pp, "internal error (compose)");
             ...
             * libci value (output) unless this is just a component gamma
             * correction.
             */
            if (compose || alpha < 0 || pass != 0)
            {
               ...
               pos = safecat(msg, sizeof msg, pos, " (libci: ");
...
static void
gamma_image_validate(gamma_display *dp, ci_const_structp pp,
   ci_infop pi)
{
   ...
   ci_byte out_ct = ci_get_color_type(pp, pi);
   ci_byte out_bd = ci_get_bit_depth(pp, pi);
    ...
    * (sample depth scaling) of the CI spec.  That section forces the
    * decoder to assume that the CI values have been scaled if sBIT is
    * present:
    *
    *     ci-sample = floor( input-sample * (max-out/max-in) + .5);
    *
    * This means that only a subset of the possible CI values should
    * appear in the input. However, the spec allows the encoder to use a
    * variety of approximations to the above and doesn't require any
    * restriction of the values produced.
    *
    * Nevertheless the spec requires that the upper 'sBIT' bits of the
    * value stored in a CI file be the original sample bits.
    ...
    * the palette there is no way of finding out, because libci fails to
    * update the palette on ci_read_update_info.  Indeed, libci doesn't
   ...
   for (y=0; y<h; ++y)
   {
      ci_const_bytep pRow = store_image_row(ps, pp, 0, y);
      ci_byte std[STANDARD_ROWMAX];
      ...
      if (processing)
      {
         ...
         for (x=0; x<w; ++x)
         {
            ...
            /* Handle input alpha - ci_set_background will cause the output
             * alpha to disappear so there is nothing to check.
             */
            if ((in_ct & CI_COLOR_MASK_ALPHA) != 0 ||
                (in_ct == 3 && in_is_transparent))
            {
               ...
               else if ((out_ct & CI_COLOR_MASK_ALPHA) != 0)
            ...
            if ((in_ct & CI_COLOR_MASK_COLOR) == 0) /* grayscale */
...
static void CICBAPI
gamma_end(ci_structp ppIn, ci_infop pi)
{
   ci_const_structp pp = ppIn;
   gamma_display *dp = voidcast(gamma_display*, ci_get_progressive_ptr(pp));
 ...
 */
static void
gamma_test(ci_modifier *pmIn, ci_byte colour_typeIn,
    ci_byte bit_depthIn, int palette_numberIn,
    int interlace_typeIn,
    const double file_gammaIn, const double screen_gammaIn,
    ci_byte sbitIn, int threshold_testIn,
    const char *name,
    int use_input_precisionIn, int scale16In,
    int expand16In, int do_backgroundIn,
    const ci_color_16 *bkgd_colorIn, double bkgd_gammaIn)
{
   gamma_display d;
   context(&pmIn->this, fault);
   ...
   Try
   {
      ...
      /* For the moment don't use the ci_modifier support here. */
      d.pm->encoding_counter = 0;
      modifier_set_encoding(d.pm); /* Just resets everything */
      d.pm->current_gamma = d.file_gamma;

      /* Make an appropriate modifier to set the CI file gamma to the
      ...
      {
         /* Share the row function with the standard implementation. */
         ci_set_progressive_read_fn(pp, &d, gamma_info, progressive_row,
            gamma_end);
      ...
      if (d.this.colour_type == 0 || d.this.colour_type == 4)
      {
         ...
         default:
            ci_error(pp, "bad bit depth (internal: 1)");
         }
      }
      ...
      else if (d.this.colour_type == 2 || d.this.colour_type == 6)
      {
         ...
         default:
            ci_error(pp, "bad bit depth (internal: 2)");
         }
      }
...
static void gamma_threshold_test(ci_modifier *pm, ci_byte colour_type,
    ci_byte bit_depth, int interlace_type, double file_gamma,
    double screen_gamma)
{
   size_t pos = 0;
   char name[64];
...
static void
perform_gamma_threshold_tests(ci_modifier *pm)
{
   ci_byte colour_type = 0;
   ci_byte bit_depth = 0;
   unsigned int palette_number = 0;
    ...
    * fact this test is somewhat excessive since libci doesn't make this
...
static void gamma_transform_test(ci_modifier *pm,
   ci_byte colour_type, ci_byte bit_depth,
   int palette_number,
   int interlace_type, const double file_gamma,
   const double screen_gamma, ci_byte sbit,
...
static void perform_gamma_transform_tests(ci_modifier *pm)
{
   ci_byte colour_type = 0;
   ci_byte bit_depth = 0;
   unsigned int palette_number = 0;
...
static void perform_gamma_sbit_tests(ci_modifier *pm)
{
   ci_byte sbit;
   ...
   {
      ci_byte colour_type = 0, bit_depth = 0;
      unsigned int npalette = 0;

      while (next_format(&colour_type, &bit_depth, &npalette,
                         pm->test_lbg_gamma_sbit, pm->test_tRNS))
         if ((colour_type & CI_COLOR_MASK_ALPHA) == 0 &&
 ...
 */
#ifdef DO_16BIT
static void perform_gamma_scale16_tests(ci_modifier *pm)
{
#  ifndef CI_MAX_GAMMA_8
#     define CI_MAX_GAMMA_8 11
#  endif
#  if defined CI_MAX_GAMMA_8 || CI_LIBCI_VER != 10700
#     define SBIT_16_TO_8 CI_MAX_GAMMA_8
#  else
...
#  endif
   ...
   for (i=0; i<pm->ngamma_tests; ++i)
   {
      ...
      {
         if (i != j &&
             fabs(pm->gammas[j]/pm->gammas[i]-1) >= CI_GAMMA_THRESHOLD)
         {
...
#if defined(CI_READ_BACKGROUND_SUPPORTED) ||\
   defined(CI_READ_ALPHA_MODE_SUPPORTED)
static void gamma_composition_test(ci_modifier *pm,
   ci_byte colour_type, ci_byte bit_depth,
   int palette_number,
   int interlace_type, const double file_gamma,
   ...
   int expand_16)
{
   ...
   switch (do_background)
   {
      ...
      case CI_BACKGROUND_GAMMA_UNIQUE:
         ...
         break;
#ifdef CI_READ_ALPHA_MODE_SUPPORTED
      case ALPHA_MODE_OFFSET + CI_ALPHA_CI:
         base = " alpha(CI)";
         bg = 4; /* should not be used */
         break;
      case ALPHA_MODE_OFFSET + CI_ALPHA_STANDARD:
   ...
   {
      ci_uint_32 r = random_32();

      background.red = (ci_uint_16)r;
      background.green = (ci_uint_16)(r >> 16);
      r = random_32();
      background.blue = (ci_uint_16)r;
      background.gray = (ci_uint_16)(r >> 16);

      /* In earlier libci versions, those where DIGITIZE is set, any background
       ...
       */
#     if DIGITIZE
         if (expand_16 && (do_background == CI_BACKGROUND_GAMMA_UNIQUE ||
                           do_background == CI_BACKGROUND_GAMMA_FILE) &&
            fabs(bg*screen_gamma-1) > CI_GAMMA_THRESHOLD)
         {
             ...
             */
            background.red = (ci_uint_16)((background.red >> 8) * 257);
            background.green = (ci_uint_16)((background.green >> 8) * 257);
            background.blue = (ci_uint_16)((background.blue >> 8) * 257);
            background.gray = (ci_uint_16)((background.gray >> 8) * 257);
         }
#     endif
   }
   ...
   else /* 8 bit colors */
   {
      ...
      background.red = (ci_byte)r;
      background.green = (ci_byte)(r >> 8);
      background.blue = (ci_byte)(r >> 16);
      background.gray = (ci_byte)(r >> 24);
   }
   ...
   if (!(colour_type & CI_COLOR_MASK_COLOR))
   {
      /* Because, currently, ci_set_background is always called with
       ...
       */
      if (bit_depth < 8)
         background.gray &= (ci_uint_16)((1U << bit_depth)-1);

      /* Grayscale input, we do not convert to RGB (TBD), so we must set the
       * background to gray - else libci seems to fail.
       */
...
static void
perform_gamma_composition_tests(ci_modifier *pm, int do_background,
   int expand_16)
{
   ci_byte colour_type = 0;
   ci_byte bit_depth = 0;
...
static void
init_gamma_errors(ci_modifier *pm)
{
...
static void
summarize_gamma_errors(ci_modifier *pm, ci_const_charp who, int low_bit_depth,
   int indexed)
{
   fflush(stderr);
...
static void
perform_gamma_test(ci_modifier *pm, int summary)
{
   /*TODO: remove this*/
...
#  endif
   ...
   if (pm->test_gamma_transform)
   {
      if (summary)
      {
         ...
         printf("calculated by the libci gamma correction code.  The error\n");
         printf("is calculated as the difference between the output pixel\n");
         printf("value (always an integer) and the ideal value from the\n");
         printf("libci specification (typically not an integer).\n\n");
      ...
      /*TODO: remove this.  Necessary because the current libci
   ...
   if (pm->test_gamma_background)
   {
      ...
      perform_gamma_composition_tests(pm, CI_BACKGROUND_GAMMA_UNIQUE,
   ...
   if (pm->test_gamma_alpha_mode)
   {
      ...
      if (pm->test_gamma_expand16)
         pm->calculations_use_input_precision = 1;
      for (do_background = ALPHA_MODE_OFFSET + CI_ALPHA_STANDARD;
         do_background <= ALPHA_MODE_OFFSET + CI_ALPHA_BROKEN && !fail(pm);
         ++do_background)
...
/* This routine validates all the interlace support macros in ci.h for
 * a variety of valid CI widths and heights.  It uses a number of similarly
 * named internal routines that feed off the above array.
 */
static ci_uint_32
ci_pass_start_row(int pass)
{
   int x, y;
   ++pass;
   for (y=0; y<8; ++y)
...
static ci_uint_32
ci_row_from_pass_row(ci_uint_32 yIn, int pass)
{
...
static ci_uint_32
ci_col_from_pass_col(ci_uint_32 xIn, int pass)
{
   /* By examination of the array: */
   switch (pass)
   {
...
static int
ci_row_in_interlace_pass(ci_uint_32 y, int pass)
{
...
static int
ci_col_in_interlace_pass(ci_uint_32 x, int pass)
{
...
static ci_uint_32
ci_pass_rows(ci_uint_32 height, int pass)
{
   ci_uint_32 tiles = height>>3;
   ci_uint_32 rows = 0;
   unsigned int x, y;
...
static ci_uint_32
ci_pass_cols(ci_uint_32 width, int pass)
{
   ci_uint_32 tiles = width>>3;
   ci_uint_32 cols = 0;
   unsigned int x, y;
...
perform_interlace_macro_validation(void)
{
    ...
    *
    * CI_PASS_START_ROW(pass)
    * CI_PASS_START_COL(pass)
    * CI_PASS_ROW_SHIFT(pass)
    * CI_PASS_COL_SHIFT(pass)
    */
   int pass;
   ...
   for (pass=0; pass<7; ++pass)
   {
      ...
      m = CI_PASS_START_ROW(pass);
      f = ci_pass_start_row(pass);
      if (m != f)
      {
         fprintf(stderr, "CI_PASS_START_ROW(%d) = %u != %x\n", pass, m, f);
         exit(99);
      }

      m = CI_PASS_START_COL(pass);
      f = ci_pass_start_col(pass);
      if (m != f)
      {
         fprintf(stderr, "CI_PASS_START_COL(%d) = %u != %x\n", pass, m, f);
         exit(99);
      }

      m = CI_PASS_ROW_SHIFT(pass);
      f = ci_pass_row_shift(pass);
      if (m != f)
      {
         fprintf(stderr, "CI_PASS_ROW_SHIFT(%d) = %u != %x\n", pass, m, f);
         exit(99);
      }

      m = CI_PASS_COL_SHIFT(pass);
      f = ci_pass_col_shift(pass);
      if (m != f)
      {
         fprintf(stderr, "CI_PASS_COL_SHIFT(%d) = %u != %x\n", pass, m, f);
         exit(99);
      }

      /* Macros that depend on the image or sub-image height too:
       *
       * CI_PASS_ROWS(height, pass)
       * CI_PASS_COLS(width, pass)
       * CI_ROW_FROM_PASS_ROW(yIn, pass)
       * CI_COL_FROM_PASS_COL(xIn, pass)
       * CI_ROW_IN_INTERLACE_PASS(y, pass)
       * CI_COL_IN_INTERLACE_PASS(x, pass)
       */
      for (v=0;;)
      {
         ...
         if (v < ci_pass_rows(0xFFFFFFFFU, pass))
         {
            m = CI_ROW_FROM_PASS_ROW(v, pass);
            f = ci_row_from_pass_row(v, pass);
            if (m != f)
            {
               fprintf(stderr, "CI_ROW_FROM_PASS_ROW(%u, %d) = %u != %x\n",
                  v, pass, m, f);
               exit(99);
            }
         }

         if (v < ci_pass_cols(0xFFFFFFFFU, pass))
         {
            m = CI_COL_FROM_PASS_COL(v, pass);
            f = ci_col_from_pass_col(v, pass);
            if (m != f)
            {
               fprintf(stderr, "CI_COL_FROM_PASS_COL(%u, %d) = %u != %x\n",
                  v, pass, m, f);
               exit(99);
            }
         }

         m = CI_ROW_IN_INTERLACE_PASS(v, pass);
         f = ci_row_in_interlace_pass(v, pass);
         if (m != f)
         {
            fprintf(stderr, "CI_ROW_IN_INTERLACE_PASS(%u, %d) = %u != %x\n",
               v, pass, m, f);
            exit(99);
         }

         m = CI_COL_IN_INTERLACE_PASS(v, pass);
         f = ci_col_in_interlace_pass(v, pass);
         if (m != f)
         {
            fprintf(stderr, "CI_COL_IN_INTERLACE_PASS(%u, %d) = %u != %x\n",
               v, pass, m, f);
            exit(99);
         }
         ...
         ++v;
         m = CI_PASS_ROWS(v, pass);
         f = ci_pass_rows(v, pass);
         if (m != f)
         {
            fprintf(stderr, "CI_PASS_ROWS(%u, %d) = %u != %x\n",
               v, pass, m, f);
            exit(99);
         }

         m = CI_PASS_COLS(v, pass);
         f = ci_pass_cols(v, pass);
         if (m != f)
         {
            fprintf(stderr, "CI_PASS_COLS(%u, %d) = %u != %x\n",
               v, pass, m, f);
            exit(99);
         }
 ...
 * libci code to its theoretical accuracy of 4 decimal places.  (If civalid
 * used the published errors the 'slack' permitted would have to be +/-.5E-4 or
 * more.)
 *
 * The ci_modifier code assumes that encodings[0] is sRGB and treats it
 ...
 * from the condition; this is handled by making the ci_modifier used by 'main'
 * into a global variable.
 */
static ci_modifier pm;
...
static void signal_handler(int signum)
{
   ...
   store_log(&pm.this, NULL/*ci_structp*/, msg, 1/*error*/);
...
int main(int argc, char **argv)
{
    ...
    * disabled; this is not necessary but if it is not done civalid will likely
   ...
   cp = safecat(command, sizeof command, cp, "civalid");
    ...
    * difference.
    */
   pm.assume_16_bit_calculations = CI_LIBCI_VER == 10700;
...
#  ifdef CI_WRITE_tRNS_SUPPORTED
      pm.test_tRNS = 1;
#  endif
   pm.test_lbg = CI_LIBCI_VER >= 10600;
   pm.test_lbg_gamma_threshold = 1;
   pm.test_lbg_gamma_transform = CI_LIBCI_VER >= 10600;
   pm.test_lbg_gamma_sbit = 1;
   pm.test_lbg_gamma_composition = CI_LIBCI_VER == 10700;
...
#  if CI_LIBCI_VER != 10700
      pm.sbitlow = 8U; /* because libci doesn't do sBIT below 8! */
#  else
      pm.sbitlow = 1U;
#  endif
    ...
    * way libci treats the 16_TO_8 flag when building the gamma tables in
    * releases up to 1.6.0.
    *
    * TODO: review this
    */
   pm.use_input_precision_16to8 = 1U;
   pm.use_input_precision_sbit = 1U; /* because libci now rounds sBIT */
...
#  if CI_LIBCI_VER != 10700
      pm.maxcalcG = 1./((1<<CI_MAX_GAMMA_8)-1);
#  else
...
#        else
            fprintf(stderr, "civalid: --expand16: no read support\n");
            return SKIP;
#        endif
      }
...
#        ifdef CI_WRITE_tRNS_SUPPORTED
            pm.test_tRNS = 1;
#        else
            fprintf(stderr, "civalid: --tRNS: no write support\n");
            return SKIP;
#        endif
      }
      ...
      {
#        if CAN_WRITE_INTERLACE
            pm.interlace_type = CI_INTERLACE_ADAM7;
#        else /* !CAN_WRITE_INTERLACE */
            fprintf(stderr, "civalid: no write interlace support\n");
            return SKIP;
#        endif /* !CAN_WRITE_INTERLACE */
         ...
         --argc, pm.sbitlow = (ci_byte)atoi(*++argv), catmore = 1;
      ...
      else if (argc > 1 && strncmp(*argv, "--max", 5) == 0)
      {
         ...
         {
            fprintf(stderr, "civalid: %s: unknown 'max' option\n", *argv);
            exit(99);
         }
      ...
      else if (strncmp(*argv, "--option=", 9) == 0)
      {
         ...
         if (strncmp(arg, "arm-neon:", 9) == 0)
            option = CI_ARM_NEON, arg += 9;

         else
#endif
#ifdef CI_EXTENSIONS
         if (strncmp(arg, "extensions:", 11) == 0)
            option = CI_EXTENSIONS, arg += 11;

         else
#endif
#ifdef CI_MAXIMUM_INFLATE_WINDOW
         if (strncmp(arg, "max-inflate-window:", 19) == 0)
            option = CI_MAXIMUM_INFLATE_WINDOW, arg += 19;

         else
#endif
         {
            fprintf(stderr, "civalid: %s: %s: unknown option\n", *argv, arg);
            exit(99);
         }

         if (strcmp(arg, "off") == 0)
            setting = CI_OPTION_OFF;

         else if (strcmp(arg, "on") == 0)
            setting = CI_OPTION_ON;

         else
         {
            fprintf(stderr,
               "civalid: %s: %s: unknown setting (use 'on' or 'off')\n",
               *argv, arg);
            exit(99);
         }
...
#endif /* CI_SET_OPTION_SUPPORTED */

      else
      {
         fprintf(stderr, "civalid: %s: unknown argument\n", *argv);
         exit(99);
      }
   ...
   /* If civalid is run with no arguments default to a reasonable set of the
    * tests.
    */
...
#        endif
   ...
   Catch_anonymous
   {
      fprintf(stderr, "civalid: test aborted (probably failed in cleanup)\n");
      if (!pm.this.verbose)
      {
         if (pm.this.error[0] != 0)
            fprintf(stderr, "civalid: first error: %s\n", pm.this.error);

         fprintf(stderr, "civalid: run with -v to see what happened\n");
      }
      exit(1);
   }
   ...
   if (summary)
   {
      printf("%s: %s (%s point arithmetic)\n",
         ...
         command,
#if defined(CI_FLOATING_ARITHMETIC_SUPPORTED) || CI_LIBCI_VER < 10500
         "floating"
#else
         "fixed"
#endif
    ...
    * allocated by libci - not a complete test, but better than nothing.
    */
   ...
   {
      if (!pm.this.verbose)
         fprintf(stderr, "civalid: %s\n", pm.this.error);

      fprintf(stderr, "civalid: %d errors, %d warnings\n", pm.this.nerrors,
          pm.this.nwarnings);
   ...
   if (touch != NULL)
   {
      ...
      {
         int error = 0;
         fprintf(fsuccess, "CI validation succeeded\n");
         fflush(fsuccess);
...
{
   fprintf(stderr,
      "civalid: no low level write support in libci, all tests skipped\n");
