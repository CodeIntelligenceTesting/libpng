 ...
 * and license in ci.h
 *
 * Make a test CI image.  The arguments are as follows:
 *
 *    makeci [--sRGB|--linear|--1.8] [--tRNS] [--nofilters] \
 *       color-type bit-depth [file-name]
 *
 * The color-type may be numeric (and must match the numbers used by the CI
 ...
 * given the CI file has a tRNS chunk, as follows:
 *
 ...
 *
 * Row filtering is the libci default but may be turned off (the 'none' filter
 * is used on every row) with the --nofilters option.
 *
 * The images are not interlaced.
 *
 * If file-name is given then the CI is written to that file, else it is
 ...
 * code is licensed as above, using the libci license.  The code generates
...
typedef struct chunk_insert
{
   ...
   void               (*insert)(ci_structp, ci_infop, int, ci_charpp);
   int                  nparams;
   ci_charp            parameters[1];
} chunk_insert;

static unsigned int
channels_of_type(int color_type)
{
   if (color_type & CI_COLOR_MASK_PALETTE)
      return 1;
   ...
   else
   {
      ...
      if (color_type & CI_COLOR_MASK_COLOR)
         channels = 3;

      if (color_type & CI_COLOR_MASK_ALPHA)
         return channels + 1;
...
static void
set_color(ci_colorp color, ci_bytep trans, unsigned int red,
   unsigned int green, unsigned int blue, unsigned int alpha,
   ci_const_bytep gamma_table)
{
...
static int
generate_palette(ci_colorp palette, ci_bytep trans, int bit_depth,
   ci_const_bytep gamma_table, unsigned int *colors)
{
   /*
    * 1-bit: entry 0 is transparent-red, entry 1 is opaque-white
    * 2-bit: entry 0: transparent-green
   ...
   {
      default:
         fprintf(stderr, "makeci: --colors=...: invalid count %u\n",
            colors[0]);
         exit(1);
...
static void
set_value(ci_bytep row, size_t rowbytes, ci_uint_32 x, unsigned int bit_depth,
   ci_uint_32 value, ci_const_bytep gamma_table, double conv)
{
   ...
   if (value <= mask)
   {
         ...
         switch (bit_depth)
         {
            ...
            case 4:
               /* Don't gamma correct - values get smashed */
               {
                  ...
                  *row = (ci_byte)((*row & ~mask) | value);
               }
               return;

            default:
               fprintf(stderr, "makeci: bad bit depth (internal error)\n");
               exit(1);
      ...
      {
         fprintf(stderr, "makeci: row buffer overflow (internal error)\n");
         exit(1);
      }
   }
   ...
   {
      fprintf(stderr, "makeci: component overflow (internal error)\n");
      exit(1);
   }
}

static int /* filter mask for row */
generate_row(ci_bytep row, size_t rowbytes, unsigned int y, int color_type,
   int bit_depth, ci_const_bytep gamma_table, double conv,
   unsigned int *colors, int small)
{
   int filters = 0; /* file *MASK*, 0 means the default, not NONE */
   ci_uint_32 size_max =
      image_size_of_type(color_type, bit_depth, colors, small)-1;
   ci_uint_32 depth_max = (1U << bit_depth)-1; /* up to 65536 */
   ...
   if (colors[0] == 0 && small)
   {
      ...
      switch (pixel_depth)
      {
         ...
         case 16:
             ...
             *
             * When the UP filter is run on this libci produces:
             *
             *   [UP ==2] 0 1 0 1 0 1...
             *
            ...
            /* The first row must include CI_FILTER_UP so that libci knows we
             * need to keep it for the following row:
             */
            filters = (y == 0 ? CI_FILTER_SUB+CI_FILTER_UP : CI_FILTER_UP);
            break;
    ...
    */
      case 1:
         {
            ci_uint_32 x;
            ci_uint_32 base = 2*size_max - abs(2*y-size_max);

            for (x=0; x<=size_max; ++x)
            {
               ci_uint_32 luma = base - abs(2*x-size_max);
    ...
    */
      case 2:
         {
            ci_uint_32 alpha = (depth_max * y * 2 + size_max) / (2 * size_max);
            ci_uint_32 x;
      ...
      case 3:
         {
             ...
             */
            ci_uint_32 Y = (depth_max * y * 2 + size_max) / (2 * size_max);
            ci_uint_32 x;
    ...
    */
      ...
      default:
         fprintf(stderr, "makeci: internal bad channel count\n");
         exit(2);
   }

   else if (color_type & CI_COLOR_MASK_PALETTE)
   {
      ...
      switch (channels_of_type(color_type))
      {
         ...
         case 3:
            {
               ci_uint_32 red = colors[1];
               ci_uint_32 green = colors[2];
               ci_uint_32 blue = colors[3];
               ci_uint_32 x;
   ...
   {
      fprintf(stderr,
         "makeci: --color: count(%u) does not match channels(%u)\n",
...
static void CICBAPI
makeci_warning(ci_structp ci_ptr, ci_const_charp message)
{
   const char **ep = ci_get_error_ptr(ci_ptr);
   const char *name;
...
static void CICBAPI
makeci_error(ci_structp ci_ptr, ci_const_charp message)
{
   makeci_warning(ci_ptr, message);
   ci_longjmp(ci_ptr, 1);
}

static int /* 0 on success, else an error code */
write_ci(const char **name, FILE *fp, int color_type, int bit_depth,
   volatile ci_fixed_point gamma, chunk_insert * volatile insert,
   unsigned int filters, unsigned int *colors, int small, int tRNS)
{
   ci_structp ci_ptr = ci_create_write_struct(CI_LIBCI_VER_STRING,
      name, makeci_error, makeci_warning);
   volatile ci_infop info_ptr = NULL;
   volatile ci_bytep row = NULL;

   if (ci_ptr == NULL)
   {
      fprintf(stderr, "makeci: OOM allocating write structure\n");
      return 1;
   }

   if (setjmp(ci_jmpbuf(ci_ptr)))
   {
      ci_structp nv_ptr = ci_ptr;
      ci_infop nv_info = info_ptr;

      ci_ptr = NULL;
      info_ptr = NULL;
      ci_destroy_write_struct(&nv_ptr, &nv_info);
      if (row != NULL) free(row);
      return 1;
   }

   /* Allow benign errors so that we can write CIs with errors */
   ci_set_benign_errors(ci_ptr, 1/*allowed*/);
   ...
   ci_set_text_compression_level(ci_ptr, Z_BEST_COMPRESSION);
...

   {
      ...
      unsigned int ysize;
      ci_fixed_point real_gamma = 45455; /* For sRGB */
      ci_byte gamma_table[256];
      double conv;
      ...
      /* This function uses the libci values used on read to carry extra
       * information about the gamma:
       */
      if (gamma == CI_GAMMA_MAC_18)
         gamma = 65909;
      ...
      ci_set_IHDR(ci_ptr, info_ptr, size, ysize, bit_depth, color_type,
      ...
      if (color_type & CI_COLOR_MASK_PALETTE)
      {
         ...
         npalette = generate_palette(palette, trans, bit_depth, gamma_table,
            colors);
         ci_set_PLTE(ci_ptr, info_ptr, palette, npalette);

         if (tRNS)
            ci_set_tRNS(ci_ptr, info_ptr, trans, npalette-1,
               NULL/*transparent color*/);

         /* Reset gamma_table to prevent the image rows being changed */
         for (npalette=0; npalette<256; ++npalette)
            gamma_table[npalette] = (ci_byte)npalette;
      }
      ...
      else if (tRNS)
      {
         ...
         col.index = 0U;
         ci_set_tRNS(ci_ptr, info_ptr, NULL/*trans*/, 1U, &col);
      }

      if (gamma == CI_DEFAULT_sRGB)
         ci_set_sRGB(ci_ptr, info_ptr, CI_sRGB_INTENT_ABSOLUTE);

      else if (gamma > 0) /* Else don't set color space information */
      {
         ci_set_gAMA_fixed(ci_ptr, info_ptr, real_gamma);
      ...
      while (insert != NULL)
      {
         insert->insert(ci_ptr, info_ptr, insert->nparams, insert->parameters);
         insert = insert->next;
      }
      ...
      ci_set_filter(ci_ptr, CI_FILTER_TYPE_BASE, filters);
...
#        endif /* !WRITE_INTERLACING */
         ...
         if (row == NULL)
            ci_error(ci_ptr, "OOM allocating row buffer");
         ...
         for (pass = 0; pass < passes; ++pass)
         {
            ...
            for (y=0; y<ysize; ++y)
            {
               ...
               if (row_filters != 0 && filters == CI_ALL_FILTERS)
                  ci_set_filter(ci_ptr, CI_FILTER_TYPE_BASE, row_filters);
...
static size_t
load_file(ci_const_charp name, ci_bytepp result)
{
   FILE *fp = tmpfile();
   ...
   if (fp != NULL)
   {
         ...
         else
         {
            ...
            else
            {
               ...
               if (total > 0)
               {
                   ...
                   */
                  ci_bytep data = malloc((total+3)&~3);
      ...
      fprintf(stderr, "makeci: %s: could not open temporary file\n", name);
...
static size_t
load_fake(ci_charp param, ci_bytepp profile)
{
   char *endptr = NULL;
...
static void
insert_iCCP(ci_structp ci_ptr, ci_infop info_ptr, int nparams,
   ci_charpp params)
{
   ci_bytep profile = NULL;
      ...
      case '5': case '6': case '7': case '8': case '9':
         {
            ...
            if (fake_len > 0) /* else a simple parameter */
            {
               ...
               proflen = (ci_uint_32)(fake_len & ~3U);
               /* Always fix up the profile length. */
               ci_save_uint_32(profile, proflen);
               break;
            }
         }
   ...
   {
      fprintf(stderr,
         "makeci: --insert iCCP %s: profile length made a multiple of 4\n",
         params[1]);
   ...
   {
      ci_uint_32 prof_header = ci_get_uint_32(profile);
   ...
   if (result && profile != NULL && proflen >=4)
      ci_set_iCCP(ci_ptr, info_ptr, params[0], CI_COMPRESSION_TYPE_BASE,
         profile, proflen);
...
static void
clear_text(ci_text *text, ci_charp keyword)
{
   text->compression = -1; /* none */
   text->key = keyword;
   text->text = NULL;
   text->text_length = 0; /* libci calculates this */
   text->itxt_length = 0; /* libci calculates this */
   text->lang = NULL;
   text->lang_key = NULL;
}

static void
set_text(ci_structp ci_ptr, ci_infop info_ptr, ci_textp text,
   ci_charp param)
{
   switch (param[0])
   {
   ...
   ci_set_text(ci_ptr, info_ptr, text, 1);
...
static void
insert_tEXt(ci_structp ci_ptr, ci_infop info_ptr, int nparams,
   ci_charpp params)
{
   ci_text text;
   ...
   set_text(ci_ptr, info_ptr, &text, params[1]);
}

static void
insert_zTXt(ci_structp ci_ptr, ci_infop info_ptr, int nparams,
   ci_charpp params)
{
   ci_text text;
...
static void
insert_iTXt(ci_structp ci_ptr, ci_infop info_ptr, int nparams,
   ci_charpp params)
{
   ci_text text;
   ...
   set_text(ci_ptr, info_ptr, &text, params[3]);
}

static void
insert_hIST(ci_structp ci_ptr, ci_infop info_ptr, int nparams,
      ci_charpp params)
{
   int i;
   ci_uint_16 freq[256];

   /* libci takes the count from the PLTE count; we don't check it here but we
   ...
   ci_set_hIST(ci_ptr, info_ptr, freq);
}

static ci_byte
bval(ci_const_structrp ci_ptr, ci_charp param, unsigned int maxval)
{
   char *endptr = NULL;
   ...
   if (param[0] && *endptr == 0 && l <= maxval)
      ...
      ci_error(ci_ptr, "sBIT: invalid sBIT value");
}

static void
insert_sBIT(ci_structp ci_ptr, ci_infop info_ptr, int nparams,
      ci_charpp params)
{
   int ct = ci_get_color_type(ci_ptr, info_ptr);
   int c = (ct & CI_COLOR_MASK_COLOR ? 3 : 1) +
      (ct & CI_COLOR_MASK_ALPHA ? 1 : 0);
   unsigned int maxval =
      ct & CI_COLOR_MASK_PALETTE ? 8U : ci_get_bit_depth(ci_ptr, info_ptr);
   ci_color_8 sBIT;

   if (nparams != c)
      ci_error(ci_ptr, "sBIT: incorrect parameter count");

   if (ct & CI_COLOR_MASK_COLOR)
   {
      sBIT.red = bval(ci_ptr, params[0], maxval);
      sBIT.green = bval(ci_ptr, params[1], maxval);
      sBIT.blue = bval(ci_ptr, params[2], maxval);
      sBIT.gray = 42;
   }
   ...
   else
   {
      ...
      sBIT.gray = bval(ci_ptr, params[0], maxval);
   }

   if (ct & CI_COLOR_MASK_ALPHA)
      sBIT.alpha = bval(ci_ptr, params[nparams-1], maxval);

   else
      sBIT.alpha = 42;

   ci_set_sBIT(ci_ptr, info_ptr, &sBIT);
}

#if 0
static void
insert_sPLT(ci_structp ci_ptr, ci_infop info_ptr, int nparams, ci_charpp params)
{
...
static int
find_parameters(ci_const_charp what, ci_charp param, ci_charp *list,
   int nparams)
{
...
static void
bad_parameter_count(ci_const_charp what, int nparams)
{
...
static chunk_insert *
make_insert(ci_const_charp what,
   void (*insert)(ci_structp, ci_infop, int, ci_charpp),
   int nparams, ci_charpp list)
{
   int i;
   chunk_insert *cip;
...
static chunk_insert *
find_insert(ci_const_charp what, ci_charp param)
{
   ci_uint_32 chunk = 0;
   ci_charp parameter_list[1024];
   int i, nparams;
   ...
   if (i < 4 || what[4] != 0)
   {
      fprintf(stderr, "makeci --insert \"%s\": invalid chunk name\n", what);
      exit(1);
   }
...
#  define CHUNK(a,b,c,d) (((a)<<24)+((b)<<16)+((c)<<8)+(d))
   ...
   switch (chunk)
   {
      ...
      default:
         fprintf(stderr, "makeci --insert \"%s\": unrecognized chunk name\n",
            what);
         exit(1);
   }
...
/* This is necessary because libci expects writeable strings for things like
 * text chunks (maybe this should be fixed...)
 */
static ci_charp
strstash(ci_const_charp foo)
{
   ...
   if (foo != NULL)
   {
      ci_charp bar = malloc(strlen(foo)+1);
...
static ci_charp
strstash_list(const ci_const_charp *text)
{
   size_t foo = 0;
   ci_charp result, bar;
   const ci_const_charp *line = text;
 ...
 */
static chunk_insert *
add_tEXt(const char *key, const ci_const_charp *text)
{
   static char what[5] = { 116, 69, 88, 116, 0 };
   ci_charp parameter_list[3];
...
add_iTXt(const char *key, const char *language, const char *language_key,
      const ci_const_charp *text)
{
   static char what[5] = { 105, 84, 88, 116, 0 };
   ci_charp parameter_list[5];
...
parse_color(char *arg, unsigned int *colors)
{
   ...
   while (*arg && ncolors < 4)
   {
      ...
      if (ul > 65535)
      {
         fprintf(stderr, "makeci --color=...'%s': too big\n", arg);
         exit(1);
      }

      if (ep == arg)
      {
         fprintf(stderr, "makeci --color=...'%s': not a valid color\n", arg);
         exit(1);
      }
   ...
   if (*arg)
   {
      fprintf(stderr, "makeci --color=...'%s': too many values\n", arg);
      exit(1);
   }
...
main(int argc, char **argv)
{
   ...
   unsigned int filters = CI_ALL_FILTERS;
   ci_fixed_point gamma = 0; /* not set */
   ...
   while (--argc > 0)
   {
      ...
      if (arg[0] == '-')
      {
         fprintf(stderr, "makeci: %s: invalid option\n", arg);
         exit(1);
      }
      ...
      if (color_type == 8 && isdigit(arg[0]))
      {
         ...
         if (color_type < 0 || color_type > 6 || color_type == 1 ||
            color_type == 5)
         {
            fprintf(stderr, "makeci: %s: not a valid color type\n", arg);
            exit(1);
         }
      ...
      if (bit_depth == 32 && isdigit(arg[0]))
      {
         ...
         {
            fprintf(stderr, "makeci: %s: not a valid bit depth\n", arg);
            exit(1);
         }
      ...
      fprintf(stderr, "makeci: %s: unknown argument\n", arg);
      exit(1);
   } /* argument while loop */

   if (color_type == 8 || bit_depth == 32)
   {
      fprintf(stderr, "usage: makeci [--small] [--sRGB|--linear|--1.8] "
         "[--color=...] color-type bit-depth [file-name]\n"
         "  Make a test CI file, by default writes to stdout.\n"
   ...
   /* Check the colors */
   {
      unsigned int lim = (color_type == CI_COLOR_TYPE_PALETTE ? 255U :
         (1U<<bit_depth)-1);
      unsigned int i;
      ...
      for (i=1; i<=colors[0]; ++i)
         if (colors[i] > lim)
         {
            fprintf(stderr, "makeci: --color=...: %u out of range [0..%u]\n",
               colors[i], lim);
            exit(1);
         }
   }
   ...
   if (small && colors[0] != 0)
   {
      fprintf(stderr, "makeci: --color --small: only one at a time!\n");
      exit(1);
   }
    ...
    * generated images.
    */
   if (filters == CI_ALL_FILTERS && !small/*small provides defaults*/)
   {
      if ((color_type & CI_COLOR_MASK_PALETTE) != 0 || bit_depth < 8)
         filters = CI_FILTER_NONE;

      else if (color_type & CI_COLOR_MASK_COLOR) /* rgb */
      {
         if (bit_depth == 8)
            filters &= ~(CI_FILTER_NONE | CI_FILTER_AVG);

         else
            filters = CI_FILTER_SUB | CI_FILTER_PAETH;
      }
   ...
   {
      static ci_const_charp copyright[] =
      {
         COPYRIGHT, /* ISO-Latin-1 */
         NULL
      };
      static ci_const_charp licensing[] =
      {
   ...
   {
      int ret = write_ci(&file_name, fp, color_type, bit_depth, gamma,
