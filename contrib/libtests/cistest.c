 ...
 * and license in ci.h
 *
 * Test for the CI 'simplified' APIs.
 */
...
#ifdef CI_SIMPLIFIED_READ_SUPPORTED /* Else nothing can be done */
#include "../tools/sRGB.h"

/* KNOWN ISSUES
 *
 * These defines switch on alternate algorithms for format conversions to match
 * the current libci implementation; they are set to allow cistest to pass
 * even though libci is producing answers that are not as correct as they
 * should be.
 */
#define ALLOW_UNUSED_GPC 0
    ...
    * use while testing the correct function to use to take into account libci
...
/* During parallel runs of cistest each temporary file needs a unique name,
...
static ci_uint_32 color_seed[2];
...
{
   if (alpha <= component)
      return 255; /* Arbitrary, but consistent with the libci code */
...
print_opts(ci_uint_32 opts)
{
   ...
   if (opts & NO_RESEED)
      printf(" --noreseed");
#if CI_LIBCI_VER < 10700 /* else on by default */
   if (opts & GBG_ERROR)
...
formatof(const char *arg)
{
   ...
   fprintf(stderr, "cistest: format name '%s' invalid\n", arg);
   return FORMAT_COUNT;
}
...
typedef struct
{
   ci_uint_32 bits[FORMAT_SET_COUNT];
}
format_list;
...
static int format_set(format_list *pf, ci_uint_32 format)
{
   if (format < FORMAT_COUNT)
      return pf->bits[format >> 5] |= ((ci_uint_32)1) << (format & 31);

   return 0;
}

#if 0 /* currently unused */
static int format_unset(format_list *pf, ci_uint_32 format)
{
   if (format < FORMAT_COUNT)
      return pf->bits[format >> 5] &= ~((ci_uint_32)1) << (format & 31);
...
static int format_isset(format_list *pf, ci_uint_32 format)
{
   return format < FORMAT_COUNT &&
      (pf->bits[format >> 5] & (((ci_uint_32)1) << (format & 31))) != 0;
}
...
#        ifdef CI_FORMAT_BGR_SUPPORTED
            ...
            if ((f & CI_FORMAT_FLAG_COLOR) == 0 &&
               (f & CI_FORMAT_FLAG_BGR) != 0)
#        else
...
#        ifdef CI_FORMAT_AFIRST_SUPPORTED
            if ((f & CI_FORMAT_FLAG_ALPHA) == 0 &&
               (f & CI_FORMAT_FLAG_AFIRST) != 0)
#        else
...
/* THE Image STRUCTURE */
/* The super-class of a ci_image, contains the decoded image plus the input
 ...
 */
static void initimage(Image *image, ci_uint_32 opts, const char *file_name,
   int stride_extra)
{
   freeimage(image);
...
{
   size_t size = CI_IMAGE_BUFFER_SIZE(image->image, image->stride);

   if (size+32 > image->bufsize)
   {
      freebuffer(image);
      image->buffer = voidcast(ci_bytep, malloc(size+32));
...
static int
check16(ci_const_bytep bp, int b)
{
   int i = 16;
...
/* ERROR HANDLING */
/* Log a terminal error, also frees the libci part of the image if necessary.
 */
static int
...
/* Make sure the ci_image has been freed - validates that libci is doing what
...
checkopaque(Image *image)
{
    ...
    */
            ...
            (strcmp(image->image.message,
               "libci does not support gamma+background+rgb_to_gray") == 0 ?
...
#define BASE_FORMATS\
   (CI_FORMAT_FLAG_ALPHA|CI_FORMAT_FLAG_COLOR|CI_FORMAT_FLAG_LINEAR)
 ...
 */
static void
gp_g8(Pixel *p, ci_const_voidp pb)
{
   ci_const_bytep pp = voidcast(ci_const_bytep, pb);
...
static void
gp_ga8(Pixel *p, ci_const_voidp pb)
{
   ci_const_bytep pp = voidcast(ci_const_bytep, pb);
...
#ifdef CI_FORMAT_AFIRST_SUPPORTED
static void
gp_ag8(Pixel *p, ci_const_voidp pb)
{
   ci_const_bytep pp = voidcast(ci_const_bytep, pb);
...
static void
gp_rgb8(Pixel *p, ci_const_voidp pb)
{
   ci_const_bytep pp = voidcast(ci_const_bytep, pb);
...
#ifdef CI_FORMAT_BGR_SUPPORTED
static void
gp_bgr8(Pixel *p, ci_const_voidp pb)
{
   ci_const_bytep pp = voidcast(ci_const_bytep, pb);
...
static void
gp_rgba8(Pixel *p, ci_const_voidp pb)
{
   ci_const_bytep pp = voidcast(ci_const_bytep, pb);
...
#ifdef CI_FORMAT_BGR_SUPPORTED
static void
gp_bgra8(Pixel *p, ci_const_voidp pb)
{
   ci_const_bytep pp = voidcast(ci_const_bytep, pb);
...
#ifdef CI_FORMAT_AFIRST_SUPPORTED
static void
gp_argb8(Pixel *p, ci_const_voidp pb)
{
   ci_const_bytep pp = voidcast(ci_const_bytep, pb);
...
#if defined(CI_FORMAT_AFIRST_SUPPORTED) && defined(CI_FORMAT_BGR_SUPPORTED)
static void
gp_abgr8(Pixel *p, ci_const_voidp pb)
{
   ci_const_bytep pp = voidcast(ci_const_bytep, pb);
...
static void
gp_g16(Pixel *p, ci_const_voidp pb)
{
   ci_const_uint_16p pp = voidcast(ci_const_uint_16p, pb);
...
static void
gp_ga16(Pixel *p, ci_const_voidp pb)
{
   ci_const_uint_16p pp = voidcast(ci_const_uint_16p, pb);
...
#ifdef CI_FORMAT_AFIRST_SUPPORTED
static void
gp_ag16(Pixel *p, ci_const_voidp pb)
{
   ci_const_uint_16p pp = voidcast(ci_const_uint_16p, pb);
...
static void
gp_rgb16(Pixel *p, ci_const_voidp pb)
{
   ci_const_uint_16p pp = voidcast(ci_const_uint_16p, pb);
...
#ifdef CI_FORMAT_BGR_SUPPORTED
static void
gp_bgr16(Pixel *p, ci_const_voidp pb)
{
   ci_const_uint_16p pp = voidcast(ci_const_uint_16p, pb);
...
static void
gp_rgba16(Pixel *p, ci_const_voidp pb)
{
   ci_const_uint_16p pp = voidcast(ci_const_uint_16p, pb);
...
#ifdef CI_FORMAT_BGR_SUPPORTED
static void
gp_bgra16(Pixel *p, ci_const_voidp pb)
{
   ci_const_uint_16p pp = voidcast(ci_const_uint_16p, pb);
...
#ifdef CI_FORMAT_AFIRST_SUPPORTED
static void
gp_argb16(Pixel *p, ci_const_voidp pb)
{
   ci_const_uint_16p pp = voidcast(ci_const_uint_16p, pb);
...
#if defined(CI_FORMAT_AFIRST_SUPPORTED) && defined(CI_FORMAT_BGR_SUPPORTED)
static void
gp_abgr16(Pixel *p, ci_const_voidp pb)
{
   ci_const_uint_16p pp = voidcast(ci_const_uint_16p, pb);
...
static void (*
get_pixel(ci_uint_32 format))(Pixel *p, ci_const_voidp pb)
{
    ...
    */
   if (format & CI_FORMAT_FLAG_LINEAR)
   {
      if (format & CI_FORMAT_FLAG_COLOR)
      {
#        ifdef CI_FORMAT_BGR_SUPPORTED
            if (format & CI_FORMAT_FLAG_BGR)
            {
               if (format & CI_FORMAT_FLAG_ALPHA)
               {
#                 ifdef CI_FORMAT_AFIRST_SUPPORTED
                     if (format & CI_FORMAT_FLAG_AFIRST)
                        return gp_abgr16;
 ...
 */
#if CI_FORMAT_FLAG_ALPHA == 1 && CI_FORMAT_FLAG_COLOR == 2 &&\
   CI_FORMAT_FLAG_LINEAR == 4
 ...
 * the different algorithms used in libci for colormap and direct conversions
 ...
 */
#if CI_FORMAT_FLAG_COLORMAP == 8 /* extra check also required */
#  include "cistest-errors.h" /* machine generated */
...
typedef struct
{
   ...
   void (*in_gp)(Pixel*, ci_const_voidp);
   void (*out_gp)(Pixel*, ci_const_voidp);
...
transform_from_formats(Transform *result, Image *in_image,
   const Image *out_image, ci_const_colorp background, int via_linear)
{
   ci_uint_32 in_format, out_format;
   ci_uint_32 in_base, out_base;
   ...
   if (in_format & CI_FORMAT_FLAG_LINEAR)
      result->in_opaque = 65535;
   else
      result->in_opaque = 255;

   result->output_8bit = (out_format & CI_FORMAT_FLAG_LINEAR) == 0;
   ...
   in_format &= BASE_FORMATS | CI_FORMAT_FLAG_COLORMAP;
   in_base = in_format & BASE_FORMATS;
   out_format &= BASE_FORMATS | CI_FORMAT_FLAG_COLORMAP;
   out_base = out_format & BASE_FORMATS;

   if (via_linear)
   {
      /* Check for an error in this program: */
      if (out_format & (CI_FORMAT_FLAG_LINEAR|CI_FORMAT_FLAG_COLORMAP))
      {
      ...
      result->transform = gpc_fn[in_base][out_base | CI_FORMAT_FLAG_LINEAR];
      result->from_linear = gpc_fn[out_base | CI_FORMAT_FLAG_LINEAR][out_base];
      result->error_ptr = gpc_error_via_linear[in_format][out_format];
   }

   else if (~in_format & out_format & CI_FORMAT_FLAG_COLORMAP)
   {
   ...
   else
   {
       ...
       * transform function just gets a pixel value, however because libci
       ...
       * functions to deal with errors in the libci implementation.
       */
      if (in_format & out_format & CI_FORMAT_FLAG_COLORMAP)
   ...
   /* Follow the libci simplified API rules to work out what to pass to the gpc
   ...
   if (in_format & ~out_format & CI_FORMAT_FLAG_ALPHA)
   {
      ...
      if (out_format & CI_FORMAT_FLAG_LINEAR || via_linear)
      {
         if (out_format & CI_FORMAT_FLAG_COLORMAP)
         {
      ...
      else /* sRGB output */
      {
         if (background != NULL)
         {
            if (out_format & CI_FORMAT_FLAG_COLOR)
            {
               ...
               /* TODO: sometimes libci uses the power law conversion here, how
                * to handle this?
                */
            ...
            else /* grayscale: libci only looks at 'g' */
            {
               result->background_color.ir =
                  result->background_color.ig =
         ...
         else if ((out_format & CI_FORMAT_FLAG_COLORMAP) == 0)
         {
            result->background_color.ir =
               result->background_color.ig =
          ...
          * provided; if cistest crashes then that is a bug in this program
          * (though libci should ci_error as well.)
          */
         else
...
static int error_to_sRGB_grayscale = 17; * libci error by calculation +
                                            2 by experiment *
 ...
 * then back to 8-bit sRGB when it is done by libci.  There are two problems:
 *
 * 1) libci currently uses a 2.2 power law with no linear segment, this results
 ...
 *
 * 2) libci currently uses an intermediate 8-bit linear value in gamma
 ...
 */
#if 0
static int error_in_libci_gamma;
...
init_error_via_linear(void)
{
   ...
   for (alpha=1; alpha<=255; ++alpha)
   {
       ...
       * because libci uses a simple power law for sRGB data at present.
       *
       * Add .1 for arithmetic errors inside libci.
       */
    ...
    * ok in this case.
    */
   error_in_libci_gamma = 14;
}
#endif

static void
print_pixel(char string[64], const Pixel *pixel, ci_uint_32 format)
{
   switch (format & (CI_FORMAT_FLAG_ALPHA|CI_FORMAT_FLAG_COLOR))
   {
      case 0:
         sprintf(string, "%s(%d)", format_names[format], pixel->g);
         break;

      case CI_FORMAT_FLAG_ALPHA:
         sprintf(string, "%s(%d,%d)", format_names[format], pixel->g,
            pixel->a);
         break;

      case CI_FORMAT_FLAG_COLOR:
         sprintf(string, "%s(%d,%d,%d)", format_names[format],
            pixel->r, pixel->g, pixel->b);
         break;

      case CI_FORMAT_FLAG_COLOR|CI_FORMAT_FLAG_ALPHA:
...
static int
logpixel(const Transform *transform, ci_uint_32 x, ci_uint_32 y,
   const Pixel *in, const Pixel *calc, const Pixel *out, const char *reason)
{
   ci_uint_32 in_format = transform->in_image->image.format;
   ci_uint_32 out_format = transform->out_image->image.format;

   ci_uint_32 back_format = out_format & ~CI_FORMAT_FLAG_ALPHA;
   ...
   if (transform->from_linear != NULL)
   {
       ...
       * linear.
       */
      back_format |= CI_FORMAT_FLAG_LINEAR;
   }
...
static int
cmppixel(Transform *transform, ci_const_voidp in, ci_const_voidp out,
   ci_uint_32 x, ci_uint_32 y/*or palette index*/)
{
   int maxerr;
   ci_const_charp errmsg;
   ...
   errmsg = NULL;
   {
      ...
      if (err_a > transform->error[3])
      {
         ...
         if (transform->accumulate)
            transform->error[3] = (ci_uint_16)err_a;
   ...
   if (errmsg == NULL) /* else just signal an alpha error */
   {
      ...
      if (transform->accumulate)
      {
         transform->error[limit] = (ci_uint_16)maxerr;
...
static ci_byte
component_loc(ci_byte loc[4], ci_uint_32 format)
{
   ...
   ci_const_colorp background)
{
   ...
   ci_const_bytep rowa = a->buffer+16;
   ci_const_bytep rowb = b->buffer+16;
   ci_uint_32 width = a->image.width;
   ci_uint_32 height = a->image.height;
   ci_uint_32 formata = a->image.format;
   ci_uint_32 formatb = b->image.format;
   unsigned int a_sample = CI_IMAGE_SAMPLE_SIZE(formata);
   unsigned int b_sample = CI_IMAGE_SAMPLE_SIZE(formatb);
   ...
   if (!(formata & CI_FORMAT_FLAG_COLORMAP) &&
      (formata & CI_FORMAT_FLAG_LINEAR))
      stridea *= 2;

   if (!(formatb & CI_FORMAT_FLAG_COLORMAP) &&
      (formatb & CI_FORMAT_FLAG_LINEAR))
      strideb *= 2;
    ...
    * then there is a problem in libci.
    */
   if (formata & formatb & CI_FORMAT_FLAG_COLORMAP)
   {
      /* Only check colormap entries that actually exist; */
      ci_const_bytep ppa, ppb;
      int match;
      ci_byte in_use[256], amax = 0, bmax = 0;
      ...
      if (match)
      {
          ...
          */
         ci_const_bytep a_cmap = (ci_const_bytep)a->colormap;
         ci_const_bytep b_cmap = (ci_const_bytep)b->colormap;
         ...
         for (y=0; y<256; ++y, a_cmap += a_sample, b_cmap += b_sample)
            if (in_use[y])
         {
            /* The colormap entries should be valid, but because libci doesn't
...
#        endif
         ...
         if (amax >= a->image.colormap_entries)
         {
            ...
            sprintf(pindex, "%d[%u]", amax,
               (ci_byte)/*SAFE*/a->image.colormap_entries);
         ...
         else if (bmax >= b->image.colormap_entries)
         {
            ...
            sprintf(pindex, "%d[%u]", bmax,
               (ci_byte)/*SAFE*/b->image.colormap_entries);
    ...
    * equivalence !a && !b == !(a || b))
    */
   if (!(((formata ^ formatb) & CI_FORMAT_FLAG_LINEAR) |
      (formata & (formatb ^ CI_FORMAT_FLAG_COLOR) & CI_FORMAT_FLAG_COLOR)))
   {
      /* Was an alpha channel changed? */
      ci_uint_32 alpha_changed = (formata ^ formatb) & CI_FORMAT_FLAG_ALPHA;
   ...
   for (y=0; y<height; ++y, rowa += stridea, rowb += strideb)
   {
      ...
      {
         ci_const_bytep psa, psb;

         if (formata & CI_FORMAT_FLAG_COLORMAP)
            psa = (ci_const_bytep)a->colormap + a_sample * *ppa++;
         else
            psa = ppa, ppa += a_sample;

         if (formatb & CI_FORMAT_FLAG_COLORMAP)
            psb = (ci_const_bytep)b->colormap + b_sample * *ppb++;
         else
         ...
         if (bchannels)
         {
             ...
             * the 8-bit case.
             */
            if (formatb & CI_FORMAT_FLAG_LINEAR) /* 16-bit checks */
            {
               ci_const_uint_16p pua = aligncastconst(ci_const_uint_16p, psa);
               ci_const_uint_16p pub = aligncastconst(ci_const_uint_16p, psb);
 ...
 */
static int
read_file(Image *image, ci_uint_32 format, ci_const_colorp background)
{
   memset(&image->image, 0, sizeof image->image);
   image->image.version = CI_IMAGE_VERSION;

   if (image->input_memory != NULL)
   {
      if (!ci_image_begin_read_from_memory(&image->image, image->input_memory,
...
#  ifdef CI_STDIO_SUPPORTED
      ...
      {
         if (!ci_image_begin_read_from_stdio(&image->image, image->input_file))
            return logerror(image, "stdio init: ", image->file_name, "");
      }

      else
      {
         if (!ci_image_begin_read_from_file(&image->image, image->file_name))
...
#  endif
   ...
   if (image->opts & sRGB_16BIT)
      image->image.flags |= CI_IMAGE_FLAG_16BIT_sRGB;
    ...
    */
   {
      ...
      if ((format & FORMAT_NO_CHANGE) != 0)
      {
         if ((format & CI_FORMAT_FLAG_COLORMAP) != 0 &&
            (image_format & CI_FORMAT_FLAG_COLORMAP) != 0)
      ...
      image->stride = CI_IMAGE_ROW_STRIDE(image->image) + image->stride_extra;
      allocbuffer(image);

      result = ci_image_finish_read(&image->image, background,
         image->buffer+16, (ci_int_32)image->stride, image->colormap);
...
#endif
                  {
                     ci_bytep b = voidcast(ci_bytep, malloc((size_t)cb));
...
write_one_file(Image *output, Image *image, int convert_to_8bit)
{
   if (image->opts & FAST_WRITE)
      image->image.flags |= CI_IMAGE_FLAG_FAST;
   ...
   if (image->opts & USE_STDIO)
   {
      ...
      if (f != NULL)
      {
         if (ci_image_write_to_stdio(&image->image, f, convert_to_8bit,
            image->buffer+16, (ci_int_32)image->stride, image->colormap))
         {
            if (fflush(f) == 0)
            {
   ...
   else if (image->opts & USE_FILE)
   {
      ...
      sprintf(name, "%s%u.ci", tmpf, ++counter);

      if (ci_image_write_to_file(&image->image, name, convert_to_8bit,
         image->buffer+16, (ci_int_32)image->stride, image->colormap))
      {
   ...
   else /* use memory */
   {
      ...
      if (ci_image_write_get_memory_size(image->image, size, convert_to_8bit,
               image->buffer+16, (ci_int_32)image->stride, image->colormap))
      {
         /* This is non-fatal but ignoring it was causing serious problems in
          * the macro to be ignored:
          */
         if (size > CI_IMAGE_CI_SIZE_MAX(image->image))
            return logerror(image, "memory", ": CI_IMAGE_SIZE_MAX wrong", "");
         ...
         if (output->input_memory != NULL)
         {
            ...
            if (ci_image_write_to_memory(&image->image, output->input_memory,
                  &output->input_memory_size, convert_to_8bit, image->buffer+16,
                  (ci_int_32)image->stride, image->colormap))
            {
   ...
   {
      ci_uint_32 original_format = image->image.format;

      if (convert_to_8bit)
         original_format &= ~CI_FORMAT_FLAG_LINEAR;
...
static int
testimage(Image *image, ci_uint_32 opts, format_list *pf)
{
   int result;
   Image copy;
...

   {
      ...
      {
         ci_uint_32 format = counter >> 1;

         ci_color background_color;
         ci_colorp background = NULL;
         ...
         if ((counter & 1) == 0)
         {
            if ((format & CI_FORMAT_FLAG_ALPHA) == 0 &&
               (image->image.format & CI_FORMAT_FLAG_ALPHA) != 0)
            {
                ...
                */
               if ((format & CI_FORMAT_FLAG_COLORMAP) != 0 &&
                  (format & CI_FORMAT_FLAG_LINEAR) == 0)
                  ++counter;
            }
...
#        ifdef CI_SIMPLIFIED_WRITE_SUPPORTED
            ...
            if ((format & CI_FORMAT_FLAG_LINEAR) != 0 &&
               (format & CI_FORMAT_FLAG_COLORMAP) == 0)
            {
                ...
                * this would round-trip correctly, but libci 1.5.7 is not the
...
#        endif /* CI_SIMPLIFIED_WRITE_SUPPORTED */
      }
...
static int
test_one_file(const char *file_name, format_list *formats, ci_uint_32 opts,
...
#     ifndef CI_SIMPLIFIED_WRITE_SUPPORTED
         printf(" (no write)");
#     endif
...
{
   ci_uint_32 opts = FAST_WRITE | STRICT;
   format_list formats;
...
#        endif
      ...
      {
         ci_uint_32 format = formatof(arg+1);
   ...
   if (opts & ACCUMULATE)
   {
      ...
      printf("/* contrib/libtests/cistest-errors.h\n");
      printf(" *\n");
      printf(" * BUILT USING:" CI_HEADER_VERSION_STRING);
      printf(" *\n");
      printf(" * This code is released under the libci license.\n");
      printf(" * For conditions of distribution and use, see the disclaimer\n");
      printf(" * and license in ci.h\n");
      printf(" *\n");
      printf(" * THIS IS A MACHINE GENERATED FILE: do not edit it directly!\n");
      printf(" * Instead run:\n");
      printf(" *\n");
      printf(" *    cistest --accumulate\n");
      printf(" *\n");
      printf(" * on as many CI files as possible; at least CISuite and\n");
      printf(" * contrib/libtests/testcis.\n");
      printf(" */\n");

      printf("static ci_uint_16 gpc_error[16/*in*/][16/*out*/][4/*a*/] =\n");
      printf("{\n");
      for (in=0; in<16; ++in)
      {
         unsigned int out;
      ...
      printf("static ci_uint_16 gpc_error_via_linear[16][4/*out*/][4] =\n");
      printf("{\n");
      for (in=0; in<16; ++in)
      {
         unsigned int out;
      ...
      printf("static ci_uint_16 gpc_error_to_colormap[8/*i*/][8/*o*/][4] =\n");
      printf("{\n");
      for (in=0; in<8; ++in)
      {
         unsigned int out;
   ...
   if (retval == 0 && touch != NULL)
   {
      ...
      {
         int error = 0;
         fprintf(fsuccess, "CI simple API tests succeeded\n");
         fflush(fsuccess);
...
#else /* !CI_SIMPLIFIED_READ_SUPPORTED */
int main(void)
{
   fprintf(stderr, "cistest: no read support in libci, test skipped\n");
   /* So the test is skipped: */
   return SKIP;
}
#endif /* CI_SIMPLIFIED_READ_SUPPORTED */
