 ...
 * overhead.
 */
#if defined (CLOCK_PROCESS_CPUTIME_ID) && defined(CI_STDIO_SUPPORTED) &&\
    defined(CI_EASY_ACCESS_SUPPORTED) &&\
    (CI_LIBCI_VER >= 10700 ? defined(CI_READ_CI_SUPPORTED) :\
     defined (CI_SEQUENTIAL_READ_SUPPORTED) &&\
     defined(CI_INFO_IMAGE_SUPPORTED))
...
static CI_CALLBACK(void, read_and_copy,
      (ci_structp ci_ptr, ci_bytep buffer, size_t cb))
{
   io_data *io = (io_data*)ci_get_io_ptr(ci_ptr);

   if (fread(buffer, cb, 1, io->input) != 1)
      ci_error(ci_ptr, strerror(errno));

   if (fwrite(buffer, cb, 1, io->output) != 1)
   {
      perror("temporary file");
      fprintf(stderr, "temporary file CI write failed\n");
      exit(1);
   }
}

static void read_by_row(ci_structp ci_ptr, ci_infop info_ptr,
      FILE *write_ptr, FILE *read_ptr)
{
   ...
   if (write_ptr != NULL)
   {
      ...
      ci_set_read_fn(ci_ptr, &io_copy, read_and_copy);
   }
...

   {
      ...
      row = voidcast(ci_bytep,malloc(rowbytes));
      display = voidcast(ci_bytep,malloc(rowbytes));
...
static CI_CALLBACK(void, no_warnings, (ci_structp ci_ptr,
         ci_const_charp warning))
{
   (void)ci_ptr;
   (void)warning;
}

static int read_ci(FILE *fp, ci_int_32 transforms, FILE *write_file)
{
   ci_structp ci_ptr = ci_create_read_struct(CI_LIBCI_VER_STRING,0,0,
         no_warnings);
...
#  endif
   ...
   if (transforms < 0)
      read_by_row(ci_ptr, info_ptr, write_file, fp);

   else
      ci_read_ci(ci_ptr, info_ptr, transforms, NULL/*params*/);
...
static int mytime(struct timespec *t)
{
   ...
   fprintf(stderr, "timeci: could not get the time\n");
   return 0;
}

static int perform_one_test(FILE *fp, int nfiles, ci_int_32 transforms)
{
   int i;
   ...
   if (mytime(&before))
   {
      for (i=0; i<nfiles; ++i)
      {
         if (read_ci(fp, transforms, NULL/*write*/))
         {
            if (ferror(fp))
            {
               perror("temporary file");
               fprintf(stderr, "file %d: error reading CI data\n", i);
               return 0;
            }
         }
         ...
         {
            perror("temporary file");
            fprintf(stderr, "file %d: error from libci\n", i);
            return 0;
         }
      }
   }
   ...
   if (mytime(&after))
   {
      ...
      if (ns < 0)
      {
         ...
         if (ns < 0)
         {
            fprintf(stderr, "timeci: bad clock from kernel\n");
            return 0;
         }
      }
      ...
      if (ferror(stdout))
      {
         fprintf(stderr, "timeci: error writing output\n");
         return 0;
      }
...
static int add_one_file(FILE *fp, char *name)
{
   ...
   if (ip != NULL)
   {
      /* Read the file using libci; this detects errors and also deals with
      ...
      if (read_ci(ip, -1/*by row*/, fp/*output*/))
      {
         if (ferror(ip))
         {
            perror(name);
...
usage(FILE *fp)
{
   ...
   fprintf(stderr,
"Usage:\n"
" timeci --assemble <assembly> {files}\n"
"  Read the files into <assembly>, output the count.  Options are ignored.\n"
" timeci --dissemble <assembly> <count> [options]\n"
"  Time <count> files from <assembly>, additional files may not be given.\n"
" Otherwise:\n"
"  Read the files into a temporary file and time the decode\n"
"Transforms:\n"
"  --by-image: read by image with ci_read_ci\n"
"  --<transform>: implies by-image, use CI_TRANSFORM_<transform>\n"
"  Otherwise: read by row using ci_read_row (to a single row buffer)\n"
   /* ISO C90 string length max 509 */);fprintf(stderr,
"{files}:\n"
"  CI files to copy into the assembly and time.  Invalid files are skipped\n"
...
int main(int argc, char **argv)
{
   ...
   if (argc > 2 && strcmp(argv[1], "--assemble") == 0)
   {
      ...
      {
         perror(assembly);
         fprintf(stderr, "timeci --assemble %s: could not open for write\n",
               assembly);
         usage(NULL);
      }
   ...
   else if (argc > 3 && strcmp(argv[1], "--dissemble") == 0)
   {
      ...
      {
         perror(argv[2]);
         fprintf(stderr, "timeci --dissemble %s: could not open for read\n",
               argv[2]);
         usage(NULL);
      }
      ...
      {
         fprintf(stderr,
               "timeci --dissemble <file> <count>: %s is not a count\n",
               argv[3]);
         exit(99);
      }
#ifdef __COVERITY__
   ...
   else /* Else use a temporary file */
   {
      ...
      {
         perror("tmpfile");
         fprintf(stderr, "timeci: could not open the temporary file\n");
   ...
   while (argc > 1 && argv[1][0] == '-' && argv[1][1] == '-')
   {
       ...
       */
      if (transforms == -1)
         transforms = CI_TRANSFORM_IDENTITY;
...
#        define OPT(name) else if (strcmp(opt, #name) == 0)\
      ...
      {
         fprintf(stderr, "timeci %s: unrecognized transform\n", opt);
         usage(fp);
      }
   }
   ...
   else if (nfiles == 0) /* Read from stdin without --dissemble */
   {
      ...
      while (fgets(filename, FILENAME_MAX+1, stdin))
      {
         ...
         {
            fprintf(stderr, "timeci: file name too long: ...%s\n",
               filename+len-32);
            err = 1;
            break;
         }
      ...
      if (ferror(stdin))
      {
         fprintf(stderr, "timeci: stdin: read error\n");
         err = 1;
      }
   }
