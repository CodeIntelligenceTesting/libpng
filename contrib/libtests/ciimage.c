 ...
 * and license in ci.h
 *
 * Test the ci_read_ci and ci_write_ci interfaces.  Given a CI file load it
 * using ci_read_ci and then write with ci_write_ci.  Test all possible
 * transforms.
 */
 ...
 * 4) 'read' - the transform works in ci_read_ci.
 * 5) 'write' - the transform works in ci_write_ci.
 * 6) CI_INFO_chunk; a mask of the chunks that must be present for the
 *    transform to fire.  All must be present - the requirement is that
 *    ci_get_valid() & mask == mask, so if mask is 0 there is no requirement.
 *
...
static struct transform_info
{
   const char *name;
   int         transform;
   ci_uint_32 valid_chunks;
#     define CHUNK_NONE 0
#     define CHUNK_sBIT CI_INFO_sBIT
#     define CHUNK_tRNS CI_INFO_tRNS
   ci_byte    color_mask_required;
   ci_byte    color_mask_absent;
#     define COLOR_MASK_X   0
#     define COLOR_MASK_P   CI_COLOR_MASK_PALETTE
#     define COLOR_MASK_C   CI_COLOR_MASK_COLOR
#     define COLOR_MASK_A   CI_COLOR_MASK_ALPHA
...
#     define TRANSFORM_RW 3
   ci_byte    tested; /* the transform was tested somewhere */
} transform_info[] =
{
   /* List ALL the CI_TRANSFORM_ macros here.  Check for support using the READ
...
#  define T(name,chunk,cm_required,cm_absent,bd,when)\
   {  #name, CI_TRANSFORM_ ## name, CHUNK_ ## chunk,\
...
#  define TRANSFORM_RW_PACKSWAP TRANSFORM_R
   ...
   T(EXPAND,              NONE, P,   X,  ALL,  R),
      /* expands PLTE CI files to RGB (no tRNS) or RGBA (tRNS) *
       * Note that the 'EXPAND' transform does lots of different things: */
   T(EXPAND,              NONE, X,   C,  ALL,  R),
      /* expands grayscale CI files to RGB, or RGBA */
...
{
   /* problem, antidote */
   { CI_TRANSFORM_SHIFT | CI_TRANSFORM_INVERT_ALPHA, 0/*antidote*/ }
};
...
struct buffer_list
{
   ...
   ci_byte            buffer[1024]; /* the actual buffer */
};
...
get_buffer(ci_structp pp)
   /* Used from libci callbacks to get the current buffer */
{
   return (struct buffer*)ci_get_io_ptr(pp);
}
...
struct display
{
   ...
   ci_structp    original_pp;       /* used on the original read */
   ci_infop      original_ip;       /* set by the original read */

   size_t         original_rowbytes; /* of the original rows: */
   ci_bytepp     original_rows;     /* from the original read */
...
{
   if (dp->read_pp != NULL)
      ci_destroy_read_struct(&dp->read_pp, &dp->read_ip, NULL);
}
...
{
      if (dp->write_pp != NULL)
         ci_destroy_write_struct(&dp->write_pp, NULL);
}
#endif
...
#  endif
   ...
   ci_destroy_read_struct(&dp->original_pp, &dp->original_ip, NULL);
   ...
   /* The display pointer is always stored in the ci_struct error pointer */
{
   ...
   if (dp == NULL)
   {
      fprintf(stderr, "ciimage: internal error (no display)\n");
...
static void
buffer_read(struct display *dp, struct buffer *bp, ci_bytep data,
   size_t size)
{
...
static void
read_ci(struct display *dp, struct buffer *bp, const char *operation,
   int transforms)
{
   ci_structp pp;
   ci_infop   ip;
   ...
   dp->read_pp = pp = ci_create_read_struct(CI_LIBCI_VER_STRING, dp,
    ...
    */
   dp->read_ip = ip = ci_create_info_struct(pp);
   if (ip == NULL)
      display_log(dp, LIBCI_ERROR, "failed to create info struct");

#  ifdef CI_SET_USER_LIMITS_SUPPORTED
      /* Remove the user limits, if any */
      ci_set_user_limits(pp, 0x7fffffff, 0x7fffffff);
#  endif

   /* Set the IO handling */
   buffer_start_read(bp);
   ci_set_read_fn(pp, bp, read_function);

   ci_read_ci(pp, ip, transforms, NULL/*params*/);

#if 0 /* crazy debugging */
   {
      ci_bytep pr = ci_get_rows(pp, ip)[0];
      size_t rb = ci_get_rowbytes(pp, ip);
      size_t cb;
      char c = ' ';

      fprintf(stderr, "%.4x %2d (%3lu bytes):", transforms, ci_get_bit_depth(pp,ip), (unsigned long)rb);
    ...
    */
{
   ...
   read_ci(dp, &dp->original_file, "original read", 0/*no transform*/);
   ...
   dp->original_rowbytes = ci_get_rowbytes(pp, ip);
   if (dp->original_rowbytes == 0)
      display_log(dp, LIBCI_BUG, "ci_get_rowbytes returned 0");

   dp->chunks = ci_get_valid(pp, ip, 0xffffffff);
   if ((dp->chunks & CI_INFO_IDAT) == 0) /* set by ci_read_ci */
      display_log(dp, LIBCI_BUG, "ci_read_ci did not set IDAT flag");

   dp->original_rows = ci_get_rows(pp, ip);
   if (dp->original_rows == NULL)
      display_log(dp, LIBCI_BUG, "ci_read_ci did not create row buffers");

   if (!ci_get_IHDR(pp, ip,
      &dp->width, &dp->height, &dp->bit_depth, &dp->color_type,
      &dp->interlace_method, &dp->compression_method, &dp->filter_method))
      display_log(dp, LIBCI_BUG, "ci_get_IHDR failed");
...
{
   /* Compare the ci_info from read_ip with original_info */
   size_t rowbytes;
   ...
   if (!ci_get_IHDR(dp->read_pp, dp->read_ip, &width, &height, &bit_depth,
...
#  ifdef CI_WRITE_INTERLACING_SUPPORTED
      /* If write interlace has been disabled, the CI file is still
       * written correctly, but as a regular (not-interlaced) CI.
       */
      C(interlace_method);
#  endif
   ...
   /* All the chunks from the original CI should be preserved in the output CI
    * because the CI format has not been changed.
    */
   {
      unsigned long chunks =
         ci_get_valid(dp->read_pp, dp->read_ip, 0xffffffff);

      if (chunks != dp->chunks)
         display_log(dp, APP_FAIL, "CI chunks changed from 0x%lx to 0x%lx",
            (unsigned long)dp->chunks, chunks);
   }

   /* rowbytes should be the same */
   rowbytes = ci_get_rowbytes(dp->read_pp, dp->read_ip);
   ...
   if (rowbytes != dp->original_rowbytes)
      display_log(dp, APP_ERROR, "CI rowbytes changed from %lu to %lu",
    ...
    */
   {
      ci_bytepp rows = ci_get_rows(dp->read_pp, dp->read_ip);
      ...
      if (rows == NULL)
         display_log(dp, LIBCI_BUG, "ci_get_rows returned NULL");

      if ((applied_transforms & CI_TRANSFORM_SHIFT) == 0 ||
         (dp->active_transforms & CI_TRANSFORM_SHIFT) == 0 ||
         color_type == CI_COLOR_TYPE_PALETTE)
      {
         ...
         {
            ci_bytep row = rows[y];
            ci_bytep orig = dp->original_rows[y];
...
#     ifdef CI_sBIT_SUPPORTED
      {
         ...
         if (ci_get_sBIT(dp->read_pp, dp->read_ip, &sBIT) != CI_INFO_sBIT)
         ...
         switch (color_type)
         {
            ...
            default:
               display_log(dp, LIBCI_ERROR, "invalid colour type %d",
                  color_type);
               /*NOTREACHED*/
...

         {
            ...
            for (b=0; 8*b<bpp; ++b)
            {
               /* libci should catch this; if not there is a security issue
                * because an app (like this one) may overflow an array. In fact
                * libci doesn't catch this at present.
                */
               if (sig_bits[b] == 0 || sig_bits[b] > bit_depth/*!palette*/)
                  display_log(dp, LIBCI_BUG,
                     "invalid sBIT[%u]  value %d returned for CI bit depth %d",
         ...
         {
            /* sanity check; this is a grayscale CI; something is wrong in the
             * code above.
             */
         ...
         switch (bit_depth)
         {
            ...
            case 16: /* Two bytes per component, big-endian */
               for (b = (bpp >> 4); b > 0; --b)
               {
                  ...
                  sig_bits[2*b+1] = (ci_byte)sig;
                  sig_bits[2*b+0] = (ci_byte)(sig >> 8); /* big-endian */
               }
               break;

            case 8: /* One byte per component */
               for (b=0; b*8 < bpp; ++b)
                  sig_bits[b] = (ci_byte)(0xff00 >> sig_bits[b]);
               break;
            ...
            default:
               display_log(dp, LIBCI_BUG, "invalid bit depth %d", bit_depth);
               break;
         }
...
#ifdef CI_WRITE_CI_SUPPORTED
static void
buffer_write(struct display *dp, struct buffer *buffer, ci_bytep data,
   size_t size)
   /* Generic write function used both from the write callback provided to
    * libci and from the generic read code.
    */
{
...
static void
write_ci(struct display *dp, ci_infop ip, int transforms)
{
   ...
   if (dp->write_pp == NULL)
      display_log(dp, APP_ERROR, "failed to create write ci_struct");

   ci_set_write_fn(dp->write_pp, &dp->written_file, write_function,
      NULL/*flush*/);
...
#  endif
   ...
   /* Certain transforms require the ci_info to be zapped to allow the
    * transform to work correctly.
    */
   if (transforms & (CI_TRANSFORM_PACKING|
                     CI_TRANSFORM_STRIP_FILLER|
                     CI_TRANSFORM_STRIP_FILLER_BEFORE))
   {
      int ct = dp->color_type;

      if (transforms & (CI_TRANSFORM_STRIP_FILLER|
                        CI_TRANSFORM_STRIP_FILLER_BEFORE))
         ct &= ~CI_COLOR_MASK_ALPHA;

      ci_set_IHDR(dp->write_pp, ip, dp->width, dp->height, dp->bit_depth, ct,
         dp->interlace_method, dp->compression_method, dp->filter_method);
   }

   ci_write_ci(dp->write_pp, ip, transforms, NULL/*params*/);
...
test_one_file(struct display *dp, const char *filename)
{
   ...
   {
      read_ci(dp, &dp->original_file, "ignored transforms",
...
#ifdef CI_WRITE_CI_SUPPORTED
   /* Second test: write the original CI data out to a new file (to test the
    * write side) then read the result back in and make sure that it hasn't
    * changed.
    */
   dp->operation = "write";
   write_ci(dp, dp->original_ip, 0/*transforms*/);
   read_ci(dp, &dp->written_file, NULL, 0/*transforms*/);
   if (!compare_read(dp, 0/*transforms applied*/))
      return;
#endif
    ...
    */
   {
      ...
      for (;;)
      {
         read_ci(dp, &dp->original_file, "active transforms", current);
          ...
          * require two sets of read ci_struct/ci_info.
          *
         ...
         {
            /* All transforms reversible: write the CI with the transformations
...
main(int argc, char **argv)
{
   ...
   for (option_end=1; option_end<argc; ++option_end)
   {
      ...
      {
         fprintf(stderr, "ciimage: %s: unknown option\n", name);
         return 99;
      }
      ...
      for (i=option_end; i<argc; ++i)
      {
          ...
          */
         {
            ...
            if (d.options & LOG)
            {
               ...
               printf("%s: ciimage ", pass ? "PASS" : "FAIL");
...
main(void)
{
   fprintf(stderr, "ciimage: no support for ci_read/write_image\n");
   return SKIP;
}
#endif
