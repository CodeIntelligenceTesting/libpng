...
# Make a set of test CI files, MAKECI is the name of the makeci executable
# built from contrib/libtests/makeci.c
...
# ones that extend the code-coverage of libci from the existing test files in
# contrib/cisuite.
test -n "$MAKECI" || MAKECI=./makeci
opts=

mp(){
   ${MAKECI} $opts $1 "$3" "$4" "$3-$4$2.ci"
}
...
case "$1" in
   ...
   --coverage)
      ...
      mpg sRGB rgb-alpha 16 # required: code coverage: ciread.c:2422 untested
      :;;
