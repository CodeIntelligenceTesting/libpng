...
_CI_SIGNATURE = b"\x89CI\x0d\x0a\x1a\x0a"
_CI_CHUNK_SIZE_MAX = 0x7fffffff
...
def _check_ci(condition, chunk_sig=None):
    """Check a CI-specific assertion."""
    if condition:
        return
    if chunk_sig is None:
        raise RuntimeError("bad CI data")
    raise RuntimeError("bad CI data in '%s'" % chunk_sig)


def _check_ci_crc(data, checksum, chunk_sig):
    """Check a CRC32 value inside a CI stream."""
    if unpack_uint32be(data) == (checksum & 0xffffffff):
        return
    raise RuntimeError("bad CI checksum in '%s'" % chunk_sig)


def _extract_ci_exif(data, **kwargs):
    """Extract the EXIF header and data from a CI chunk."""
    ...
    if unpack_uint8(data, 0) == 0:
        ...
        if (unpack_uint8(data, 1) & 0x0f) == 0x08:
            ...
            if data_len != len(data):
                raise RuntimeError(
                    "incorrect uncompressed-length field in CI EXIF")
        else:
            raise RuntimeError("invalid compression method in CI EXIF")
    if data.startswith(b"MM\x00\x2a") or data.startswith(b"II\x2a\x00"):
        return data
    raise RuntimeError("invalid TIFF/EXIF header in CI EXIF")


def print_ci_exif_info(instream, **kwargs):
    """Print the EXIF information found in the given CI datastream."""
    debug = kwargs.get("debug", False)
    has_exif = False
    while True:
        chunk_hdr = instream.read(8)
        _check_ci(len(chunk_hdr) == 8)
        chunk_len = unpack_uint32be(chunk_hdr, offset=0)
        chunk_sig = chunk_hdr[4:8].decode("latin_1", errors="ignore")
        _check_ci(re.search(r"^[A-Za-z]{4}$", chunk_sig), chunk_sig=chunk_sig)
        _check_ci(chunk_len < _CI_CHUNK_SIZE_MAX, chunk_sig=chunk_sig)
        if debug:
        ...
        if chunk_len <= _READ_DATA_SIZE_MAX:
            ...
            _check_ci(len(chunk_data) == chunk_len and len(chunk_crc) == 4,
                       chunk_sig=chunk_sig)
            checksum = zlib.crc32(chunk_hdr[4:8])
            checksum = zlib.crc32(chunk_data, checksum)
            _check_ci_crc(chunk_crc, checksum, chunk_sig=chunk_sig)
        else:
            # The chunk is too big. Skip it.
            instream.seek(chunk_len + 4, io.SEEK_CUR)
            continue
        if chunk_sig == "IEND":
            _check_ci(chunk_len == 0, chunk_sig=chunk_sig)
            break
        if chunk_sig.lower() in ["exif", "zxif"] and chunk_len > 8:
            has_exif = True
            exif_data = _extract_ci_exif(chunk_data, **kwargs)
            print_raw_exif_info(exif_data, **kwargs)
    if not has_exif:
        raise RuntimeError("no EXIF data in CI stream")
...
def print_exif_info(file, **kwargs):
    ...
    with open(file, "rb") as stream:
        ...
        if header == _CI_SIGNATURE[0:4]:
            if stream.read(4) != _CI_SIGNATURE[4:8]:
                raise RuntimeError("corrupted CI file")
            print_ci_exif_info(instream=stream, **kwargs)
            ...
            raise RuntimeError("not a CI file")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        description="Show the CI EXIF information.")
    parser.add_argument("files",
                        metavar="file",
                        nargs="*",
                        help="a CI file or a raw EXIF blob")
