/*---------------------------------------------------------------------------
   ...
   rci2 - progressive-model CI display program                  rci2-x.c

   This program decodes and displays CI files progressively, as if it were
  ...
  ---------------------------------------------------------------------------
   ...
   Changelog:
    ...
    - 2.01:  fixed 64-bit typo in readci2.c; fixed -pause usage description
    - 2.02:  fixed improper display of usage screen on CI error(s); fixed
...
#define PROGNAME  "rci2-x"
#define LONGNAME  "Progressive CI Viewer for X"
#define VERSION   "2.04 of 15 June 2014"
#define RESNAME   "rci2"       /* our X resource application name */
#define RESCLASS  "Rci"       /* our X resource class name */

#include <stdio.h>
#include <stdlib.h>
#include <ctype.h>
#include <string.h>
#include <setjmp.h>       /* for jmpbuf declaration in readci2.h */
#include <time.h>
...
#include "readci2.h"   /* typedefs, common macros, readci2 prototypes */


/* could just include ci.h, but this macro is the only thing we need
...
/* local prototypes */
static void rci2_x_init (void);
static int  rci2_x_create_window (void);
static int  rci2_x_load_bg_image (void);
static void rci2_x_display_row (ulg row);
static void rci2_x_finish_display (void);
static void rci2_x_redisplay_image (ulg startcol, ulg startrow,
                                     ulg width, ulg height);
#ifdef FEATURE_LOOP
static void rci2_x_reload_bg_image (void);
static int  is_number (char *p);
#endif
static void rci2_x_cleanup (void);
static int  rci2_x_msb (ulg u32val);
...
static mainprog_info rci2_info;
...
int main(int argc, char **argv)
{
    ...
    filename = (char *)NULL;
    memset(&rci2_info, 0, sizeof(mainprog_info));
    ...
    if ((p = getenv("SCREEN_GAMMA")) != NULL)
        rci2_info.display_exponent = atof(p);
    else
        rci2_info.display_exponent = default_display_exponent;


    /* Now parse the command line for options and the CI filename. */
    ...
    while (*++argv && !error) {
        if (!strncmp(*argv, "-display", 2)) {
            ...
            if (!*++argv)
                ...
                rci2_info.display_exponent = atof(*argv);
                if (rci2_info.display_exponent <= 0.0)
                    ++error;
            }
    ...
    if (!(infile = fopen(filename, "rb"))) {
        fprintf(stderr, PROGNAME ":  can't open CI file [%s]\n", filename);
        ++error;
    } else {
        incount = fread(inbuf, 1, INBUFSIZE, infile);
        if (incount < 8 || !readci2_check_sig(inbuf, 8)) {
            fprintf(stderr, PROGNAME
              ":  [%s] is not a CI file: incorrect signature\n",
              filename);
            ++error;
        } else if ((rc = readci2_init(&rci2_info)) != 0) {
            switch (rc) {
                case 2:
                    fprintf(stderr, PROGNAME
                      ":  [%s] has bad IHDR (libci longjmp)\n", filename);
                    break;
                case 4:
    ...
    /* set some final rci2_info variables before entering main data loop */

    if (have_bg) {
        unsigned r, g, b;   /* this approach quiets compiler warnings */

        sscanf(bgstr+1, "%2x%2x%2x", &r, &g, &b);
        rci2_info.bg_red   = (uch)r;
        rci2_info.bg_green = (uch)g;
        rci2_info.bg_blue  = (uch)b;
    } else
        rci2_info.need_bgcolor = TRUE;

    rci2_info.state = kPreInit;
    rci2_info.mainprog_init = rci2_x_init;
    rci2_info.mainprog_display_row = rci2_x_display_row;
    rci2_info.mainprog_finish_display = rci2_x_finish_display;


    /* OK, this is the fun part:  call readci2_decode_data() at the start of
     * the loop to deal with our first buffer of data (read in above to verify
     * that the file is a CI image), then loop through the file and continue
     * calling the same routine to handle each chunk of data.  It in turn
     * passes the data to libci, which will invoke one or more of our call-
     * backs as decoded data become available.  We optionally call sleep() for
     * one second per iteration to simulate downloading the image via an analog
     * modem. */

    for (;;) {
        Trace((stderr, "about to call readci2_decode_data()\n"))
        if (readci2_decode_data(&rci2_info, inbuf, incount))
            ++error;
        Trace((stderr, "done with readci2_decode_data()\n"))

        if (error || incount != INBUFSIZE || rci2_info.state == kDone) {
            if (rci2_info.state == kDone) {
                Trace((stderr, "done decoding CI image\n"))
            } else if (ferror(infile)) {
                fprintf(stderr, PROGNAME
                  ":  error while reading CI image file\n");
                exit(3);
            } else if (feof(infile)) {
                fprintf(stderr, PROGNAME ":  end of file reached "
                  "(unexpectedly) while reading CI image file\n");
                exit(3);
            } else /* if (error) */ {
    ...
    /* clean up CI stuff and report any decoding errors */

    fclose(infile);
    Trace((stderr, "about to call readci2_cleanup()\n"))
    readci2_cleanup(&rci2_info);

    if (error) {
        fprintf(stderr, PROGNAME ":  libci error while decoding CI image\n");
        exit(3);
    }
    ...
    if (loop && bg_image) {
        ...
        for (;;) {
            ...
            rci2_x_redisplay_image (0, 0, rci2_info.width, rci2_info.height);
        }
    ...
    Trace((stderr, "about to call rci2_x_cleanup()\n"))
    rci2_x_cleanup();
...
/* this function is called by readci2_info_callback() in readci2.c, which
 * in turn is called by libci after all of the pre-IDAT chunks have been
 * read and processed--i.e., we now have enough info to finish initializing */

static void rci2_x_init(void)
{
    ulg i;
    ulg rowbytes = rci2_info.rowbytes;

    Trace((stderr, "beginning rci2_x_init()\n"))
    Trace((stderr, "  rowbytes = %d\n", rci2_info.rowbytes))
    Trace((stderr, "  width  = %ld\n", rci2_info.width))
    Trace((stderr, "  height = %ld\n", rci2_info.height))

    /* Guard against integer overflow */
    if (rci2_info.height > ((size_t)(-1))/rci2_info.rowbytes) {
        fprintf(stderr, PROGNAME ":  image_data buffer would be too large\n");
        readci2_cleanup(&rci2_info);
        return;
    }

    rci2_info.image_data = (uch *)malloc(rowbytes * rci2_info.height);
    if (!rci2_info.image_data) {
        readci2_cleanup(&rci2_info);
        return;
    }

    rci2_info.row_pointers = (uch **)malloc(rci2_info.height * sizeof(uch *));
    if (!rci2_info.row_pointers) {
        free(rci2_info.image_data);
        rci2_info.image_data = NULL;
        readci2_cleanup(&rci2_info);
        return;
    }

    for (i = 0;  i < rci2_info.height;  ++i)
        rci2_info.row_pointers[i] = rci2_info.image_data + i*rowbytes;
    ...
    if (rci2_x_create_window()) {

        /* GRR TEMPORARY HACK:  this is fundamentally no different from cases
         * above; libci should call our error handler to longjmp() back to us
         * when ci_ptr goes away.  If we/it segfault instead, seems like a
         * libci bug... */

        /* we're here via libci callback, so if window fails, clean and bail */
        readci2_cleanup(&rci2_info);
        rci2_x_cleanup();
        exit(2);
    }

    rci2_info.state = kWindowInit;
}
...
static int rci2_x_create_window(void)
{
    ulg bg_red   = rci2_info.bg_red;
    ulg bg_green = rci2_info.bg_green;
    ulg bg_blue  = rci2_info.bg_blue;
    ulg bg_pixel = 0L;
    ulg attrmask;
    ...
    Trace((stderr, "beginning rci2_x_create_window()\n"))
...
/* GRR:  add/check 8-bit support */
    ...
    if (depth == 15 || depth == 16) {
        RShift = 15 - rci2_x_msb(RMask);    /* these are right-shifts */
        GShift = 15 - rci2_x_msb(GMask);
        BShift = 15 - rci2_x_msb(BMask);
    } else if (depth > 16) {
        RShift = rci2_x_msb(RMask) - 7;     /* these are left-shifts */
        GShift = rci2_x_msb(GMask) - 7;
        BShift = rci2_x_msb(BMask) - 7;
    }
    if (depth >= 15 && (RShift < 0 || GShift < 0 || BShift < 0)) {
        fprintf(stderr, "rci2 internal logic error:  negative X shift(s)!\n");
        return 2;
    }
...
/*---------------------------------------------------------------------------
  ...
  ---------------------------------------------------------------------------*/
    ...
    window = XCreateWindow(display, root, 0, 0, rci2_info.width,
      rci2_info.height, 0, depth, InputOutput, visual, attrmask, &attr);
    ...
    if ((size_hints = XAllocSizeHints()) != NULL) {
        ...
        size_hints->min_width = size_hints->max_width = (int)rci2_info.width;
        size_hints->min_height = size_hints->max_height =
          (int)rci2_info.height;
    }
...
/*---------------------------------------------------------------------------
  ...
  ---------------------------------------------------------------------------*/
    ...
    if (depth == 24 || depth == 32) {
        xdata = (uch *)malloc(4*rci2_info.width*rci2_info.height);
        pad = 32;
    } else if (depth == 16) {
        xdata = (uch *)malloc(2*rci2_info.width*rci2_info.height);
        pad = 16;
    } else /* depth == 8 */ {
        xdata = (uch *)malloc(rci2_info.width*rci2_info.height);
        pad = 8;
    }
    ...
    ximage = XCreateImage(display, visual, depth, ZPixmap, 0,
      (char *)xdata, rci2_info.width, rci2_info.height, pad, 0);
...
/*---------------------------------------------------------------------------
  ...
  ---------------------------------------------------------------------------*/
    ...
    if (bg_image)
        rci2_x_load_bg_image();    /* resets bg_image if fails */
    ...
    if (!bg_image) {
        ...
        XFillRectangle(display, window, gc, 0, 0, rci2_info.width,
          rci2_info.height);
    }
...
} /* end function rci2_x_create_window() */
...
/*---------------------------------------------------------------------------
  ...
  ---------------------------------------------------------------------------*/
    ...
    bg_rowbytes = 3 * rci2_info.width;
    bg_data = (uch *)malloc(bg_rowbytes * rci2_info.height);
    if (!bg_data) {
        fprintf(stderr, PROGNAME
...
/*---------------------------------------------------------------------------
  ...
  ---------------------------------------------------------------------------*/
    ...
    if ((bg[pat].type & 0x07) == 0) {
        ...
        for (row = 0;  row < rci2_info.height;  ++row) {
...
/*---------------------------------------------------------------------------
  ...
  ---------------------------------------------------------------------------*/
        ...
        hh = (int)(rci2_info.height / 2);
        hw = (int)(rci2_info.width / 2);
...
/*---------------------------------------------------------------------------
    Blast background image to display buffer before beginning CI decode.
  ---------------------------------------------------------------------------*/
    ...
    XPutImage(display, window, gc, ximage, 0, 0, 0, 0, rci2_info.width,
      rci2_info.height);

    return 0;

} /* end function rci2_x_load_bg_image() */
...
static void rci2_x_display_row(ulg row)
{
    uch bg_red   = rci2_info.bg_red;
    uch bg_green = rci2_info.bg_green;
    uch bg_blue  = rci2_info.bg_blue;
    uch *src, *src2=NULL;
    char *dest;
    uch r, g, b, a;
    int ximage_rowbytes = ximage->bytes_per_line;
...
/*---------------------------------------------------------------------------
  ...
  ---------------------------------------------------------------------------*/
    ...
    Trace((stderr, "beginning rci2_x_display_row()\n"))

    if (rci2_info.pass != prevpass) {
        if (pause_after_pass && rci2_info.pass > 0) {
            XEvent e;
            KeySym k;
        ...
        fprintf(stderr, "%s:  pass %d of 7\r", PROGNAME, rci2_info.pass + 1);
        fflush(stderr);
        prevpass = rci2_info.pass;
    }
...
/*---------------------------------------------------------------------------
    Aside from the use of the rci2_info struct, the lack of an outer loop
    (over rows) and moving the XPutImage() call outside the "if (depth)"
    tests, this routine is identical to rci_x_display_image() in the non-
  ...
  ---------------------------------------------------------------------------*/
    ...
    if (depth == 24 || depth == 32) {
        ...
        src = rci2_info.image_data + row*rci2_info.rowbytes;
        if (bg_image)
            src2 = bg_data + row*bg_rowbytes;
        dest = ximage->data + row*ximage_rowbytes;
        if (rci2_info.channels == 3) {
            for (i = rci2_info.width;  i > 0;  --i) {
                red   = *src++;
                green = *src++;
                blue  = *src++;
            ...
            for (i = rci2_info.width;  i > 0;  --i) {
                ...
                if (a == 255) {
                    ...
                    /* this macro (from ci.h) composites the foreground
        ...
        src = rci2_info.row_pointers[row];
        if (bg_image)
            src2 = bg_data + row*bg_rowbytes;
        dest = ximage->data + row*ximage_rowbytes;
        if (rci2_info.channels == 3) {
...
/*---------------------------------------------------------------------------
  ...
  ---------------------------------------------------------------------------*/
    ...
    if (demo_timing && (row - firstrow >= 16 || row >= rci2_info.height-2)) {
        XPutImage(display, window, gc, ximage, 0, (int)firstrow, 0,
          (int)firstrow, rci2_info.width, row - firstrow + 1);
        XFlush(display);
        rows = 0;
        usleep(usleep_duration);
    } else
    if (!demo_timing && ((rows & 0xf) == 0 || row >= rci2_info.height-2)) {
        XPutImage(display, window, gc, ximage, 0, (int)firstrow, 0,
          (int)firstrow, rci2_info.width, row - firstrow + 1);
        XFlush(display);
        rows = 0;
...
static void rci2_x_finish_display(void)
{
    Trace((stderr, "beginning rci2_x_finish_display()\n"))

    /* last row has already been displayed by rci2_x_display_row(), so we
     * have nothing to do here except set a flag and let the user know that
     * the image is done */

    rci2_info.state = kDone;
    printf(
                                    ...
                                    ulg width, ulg height)
{
    ...
    Trace((stderr, "   (width = %ld, rowbytes = %d, ximage_rowbytes = %d)\n",
      rci2_info.width, rci2_info.rowbytes, ximage_rowbytes))
    Trace((stderr, "   (bpp = %d)\n", ximage->bits_per_pixel))
    Trace((stderr, "   (byte_order = %s)\n", ximage->byte_order == MSBFirst?
      "MSBFirst" : (ximage->byte_order == LSBFirst? "LSBFirst" : "unknown")))

/*---------------------------------------------------------------------------
    Aside from the use of the rci2_info struct and of src2 (for background
    image), this routine is identical to rci_x_display_image() in the non-
  ...
  ---------------------------------------------------------------------------*/
        ...
        for (lastrow = row = startrow;  row < startrow+height;  ++row) {
            ...
            if (((row+1) & 0xf) == 0) {
                XPutImage(display, window, gc, ximage, 0, (int)lastrow, 0,
                  (int)lastrow, rci2_info.width, 16);
                XFlush(display);
                lastrow = row + 1;
            }
        }
    ...
    if (lastrow < startrow+height) {
        XPutImage(display, window, gc, ximage, 0, (int)lastrow, 0,
          (int)lastrow, rci2_info.width, rci2_info.height-lastrow);
        XFlush(display);
    }
...
} /* end function rci2_x_redisplay_image() */
...
} /* end function rci2_x_reload_bg_image() */
...
static void rci2_x_cleanup(void)
{
    ...
    if (rci2_info.row_pointers) {
        free(rci2_info.row_pointers);
        rci2_info.row_pointers = NULL;
    }
