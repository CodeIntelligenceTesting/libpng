...
#define PROGNAME  "VisualCi"
#define LONGNAME  "Win32 Viewer for CI-files"
...
BOOL BuildCiList (PTSTR pstrPathName, TCHAR **ppFileList, int *pFileCount,
        int *pFileIndex);

BOOL SearchCiList (TCHAR *pFileList, int FileCount, int *pFileIndex,
        PTSTR pstrPrevName, PTSTR pstrNextName);

BOOL LoadImageFile(HWND hwnd, PTSTR pstrPathName,
        ci_byte **ppbImage, int *pxImgSize, int *pyImgSize, int *piChannels,
        ci_color *pBkgColor);
        ...
        LPARAM lParam)
{
    ...
    static ci_color          bkgColor = {127, 127, 127};
    ...
    static TCHAR             *pCiFileList = NULL;
    static int                iCiFileCount;
    static int                iCiFileIndex;
    ...
    case WM_CREATE:
        ...
        if (szCmdFileName[0] != '\0')
        {
            ...
            /* read the other ci-files in the directory for later */
            /* next/previous commands */

            BuildCiList (szImgPathName, &pCiFileList, &iCiFileCount,
                          &iCiFileIndex);
            ...
            /* display the CI into the DIBitmap */
    ...
    case WM_COMMAND:
        ...
        case IDM_FILE_OPEN:
            ...
            if (!CiFileOpenDlg (hwnd, szImgPathName, szTitleName))
                return 0;
        ...
        case IDM_FILE_SAVE:
            ...
            if (!CiFileSaveDlg (hwnd, szImgPathName, szTitleName))
                return 0;

            /* save the CI to a disk file */

            SetCursor (LoadCursor (NULL, IDC_WAIT));
            ShowCursor (TRUE);

            bOk = CiSaveImage (szImgPathName, pDiData, cxWinSize, cyWinSize,
                  bkgColor);
            ...
            if (!bOk)
                MessageBox (hwnd, TEXT ("Error in saving the CI image"),
        ...
        case IDM_FILE_NEXT:
            ...
            if (SearchCiList (pCiFileList, iCiFileCount, &iCiFileIndex,
                NULL, szImgPathName))
            {
...
BOOL LoadImageFile (HWND hwnd, PTSTR pstrPathName,
                ci_byte **ppbImage, int *pxImgSize, int *pyImgSize,
                int *piChannels, ci_color *pBkgColor)
{
    static TCHAR szTmp [MAX_PATH];

    /* if there's an existing CI, free the memory */
    ...
    /* Load the entire CI into memory */

    SetCursor (LoadCursor (NULL, IDC_WAIT));
    ShowCursor (TRUE);

    CiLoadImage (pstrPathName, ppbImage, pxImgSize, pyImgSize, piChannels,
                  pBkgColor);
    ...
    if (*ppbImage != NULL)
    {
        sprintf (szTmp, "VisualCi - %s", strrchr(pstrPathName, '\\') + 1);
        SetWindowText (hwnd, szTmp);
    }
    else
    {
        MessageBox (hwnd, TEXT ("Error in loading the CI image"),
            szProgName, MB_ICONEXCLAMATION | MB_OK);
        return FALSE;
    }
        ...
        BOOL bStretched)
{
    ...
    WORD                        wDIRowBytes;
    ci_color                   bkgBlack = {0, 0, 0};
    ci_color                   bkgGray  = {127, 127, 127};
    ci_color                   bkgWhite = {255, 255, 255};
    ...
    {
        MessageBox (hwnd, TEXT ("Visual CI: image is too big");
    }
    if (!(pDib = (BYTE *) malloc (sizeof(BITMAPINFOHEADER) +
        wDIRowBytes * cyWinSize)))
    {
        MessageBox (hwnd, TEXT ("Error in displaying the CI image"),
            szProgName, MB_ICONEXCLAMATION | MB_OK);
        *ppDib = pDib = NULL;
        return FALSE;
    }
...
/*      if ((cyNewSize / cxNewSize) > (cyImgSize / cxImgSize)) */
        ...
        {
            MessageBox (hwnd, TEXT ("Visual CI: stretched image is too big");
        }
