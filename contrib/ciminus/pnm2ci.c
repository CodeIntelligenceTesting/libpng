/*
 *  pnm2ci.c --- conversion from PBM/PGM/PPM-file to CI-file
...
void usage ();
BOOL pnm2ci (FILE *pnm_file, FILE *ci_file, FILE *alpha_file,
              BOOL interlace, BOOL alpha);
BOOL do_pnm2ci (ci_struct *ci_ptr, ci_info *info_ptr,
...
int fscan_pnm_uint_32 (FILE *pnm_file, ci_uint_32 *num_ptr);
ci_uint_32 get_pnm_data (FILE *pnm_file, int depth);
ci_uint_32 get_pnm_value (FILE *pnm_file, int depth);
...
int main (int argc, char *argv[])
{
  ...
  for (argi = 1; argi < argc; argi++)
  {
    ...
    else if (fp_wr == stdout)
    {
      ...
      {
        fprintf (stderr, "PNM2CI\n");
        fprintf (stderr, "Error:  cannot create CI-file %s\n", argv[argi]);
        exit (1);
      }
    }
    else
  ...
  ret = pnm2ci (fp_rd, fp_wr, fp_al, interlace, alpha);
  ...
  {
    fprintf (stderr, "PNM2CI\n");
    fprintf (stderr, "Error:  unsuccessful converting to CI-image\n");
    if (fname_wr)
...
void usage ()
{
  ...
  fprintf (stderr, "Usage:  pnm2ci [options] <file>.<pnm> [<file>.ci]\n");
  fprintf (stderr, "   or:  ... | pnm2ci [options]\n");
  fprintf (stderr, "Options:\n");
  fprintf (stderr, "   -i[nterlace]   write ci-file with interlacing on\n");
  fprintf (stderr,
      "   -a[lpha] <file>.pgm read CI alpha channel as pgm-file\n");
              ...
              BOOL interlace, BOOL alpha)
{
  ...
  /* initialize the libci context for writing to ci_file */

  ci_ptr = ci_create_write_struct (ci_get_libci_ver(NULL),
                                     NULL, NULL, NULL);
  if (!ci_ptr)
  ...
  ret = do_pnm2ci (ci_ptr, info_ptr, pnm_file, alpha_file, interlace, alpha);
...
/*
 *  do_pnm2ci - does the conversion in a fully-initialized libci context
 */
                 ...
                 BOOL interlace, BOOL alpha)
{
  ...
  char          magic_token[4];
  BOOL          raw;
  ci_uint_32   width, height, maxval;
  ci_uint_32   row_bytes;
  ci_uint_32   row, col;
  ci_uint_32   val16, i;
  ci_uint_32   alpha_width = 0, alpha_height = 0;
  ...
  {
    fprintf (stderr, "PNM2CI can't read PAM (P7) files\n");
    return FALSE;
  }
  else
  {
  ...
  if (alpha)
  {
    ...
    color_type |= CI_COLOR_MASK_ALPHA;
    if (maxval == 0)
      return FALSE;
    else if (maxval == 1)
      alpha_depth = 1;
  ...
  /* allocate the rows using the same memory layout as libci, and transfer
   * their ownership to libci, with the responsibility to clean everything up;
   * please note the use of ci_calloc instead of ci_malloc */
  row_pointers = (ci_byte **)
                 ci_calloc (ci_ptr, height * sizeof (ci_byte *));
  ci_set_rows (ci_ptr, info_ptr, row_pointers);
  ci_data_freer (ci_ptr, info_ptr, CI_DESTROY_WILL_FREE_DATA, CI_FREE_ALL);
  for (row = 0; row < height; row++)
  {
    /* the individual rows should only be allocated after all the previous
     * steps completed successfully, because libci must handle correctly
     * any image allocation left incomplete after an out-of-memory error */
    row_pointers[row] = (ci_byte *) ci_malloc (ci_ptr, row_bytes);
  }
  ...
  for (row = 0; row < height; row++)
  {
    ...
    if (packed_bitmap)
    {
      ...
      {
        /* ci supports this format natively so no conversion is needed */
        ...
        for (i = 0; i < (ci_uint_32) (channels - alpha_present); i++)
        {
          ...
          else
          {
            ...
            else
            {
              ...
              *pix_ptr = (ci_byte) ((val16 >> 8) & 0xFF);
              pix_ptr++;
              *pix_ptr = (ci_byte) (val16 & 0xFF);
              pix_ptr++;
            }
          }
        }
  ...
  /* we're going to write more or less the same CI as the input file */
  ci_set_IHDR (ci_ptr, info_ptr, width, height, bit_depth, color_type,
                (!interlace) ? CI_INTERLACE_NONE : CI_INTERLACE_ADAM7,
...
int fscan_pnm_uint_32 (FILE *pnm_file, ci_uint_32 *num_ptr)
{
  ...
  *num_ptr = (ci_uint_32) token_value;
